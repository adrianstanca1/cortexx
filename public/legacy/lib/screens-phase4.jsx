...
(function() {
  ...
  const SEED = {
    inspections: [
      { id: 1, kind: 'First-fix electrical', projectId: 1, inspector: 'Adrian', date: '2026-05-22', status: 'passed',
        items: [
          { q: 'All circuits tested & dead', ok: true, note: '' },
          { q: 'Earth bonding verified', ok: true, note: '' },
          { q: 'Cable runs match drawings v3', ok: true, note: '' },
          { q: 'Junction boxes accessible', ok: true, note: '' },
          { q: 'No damage to other trades', ok: true, note: '' },
        ] },
      { id: 2, kind: 'Plaster prep', projectId: 1, inspector: 'Tom', date: '2026-05-21', status: 'passed',
        items: [
          { q: 'Surfaces clean & dust-free', ok: true, note: '' },
          { q: 'Beading installed at corners', ok: true, note: '' },
          { q: 'Joints taped', ok: true, note: '' },
          { q: 'Adequate drying time scheduled', ok: true, note: '' },
        ] },
      { id: 3, kind: 'Pre-handover snag walk', projectId: 3, inspector: 'Adrian', date: '2026-05-19', status: 'failed',
        items: [
          { q: 'Paint finish even (all rooms)', ok: false, note: '2 touch-ups needed in front room' },
          { q: 'Tile grout consistent', ok: false, note: 'Kitchen needs re-grouting' },
          { q: 'Doors/windows operate', ok: true, note: '' },
          { q: 'Skirting & architraves snug', ok: false, note: 'WC threshold gap' },
        ] },
      { id: 4, kind: 'Plaster prep', projectId: 2, inspector: 'Marcus', date: null, status: 'scheduled', items: [] },
    ],
    customers: [
      { id: 1, name: 'J. Patterson',    email: 'j.patterson@email.com', phone: '07700 900 111', address: '12 Camden Mews, NW1',  tag: 'Repeat',  projects: 2, totalValue: 222000, lastContact: '2026-05-21', notes: 'Likes USB-C sockets. High spec finishes. Daughter referred us.' },
      { id: 2, name: 'Eve & Mark Lin',  email: 'e.lin@email.com',        phone: '07700 900 222', address: '8 Mare St, E8',          tag: 'Active',  projects: 1, totalValue: 42000,  lastContact: '2026-05-19', notes: 'First-time clients. Eve is architect — likes detail.' },
      { id: 3, name: 'Tonic Café Ltd',  email: 'hello@tonic.cafe',       phone: '07700 900 333', address: 'Brixton, SW9',           tag: 'Commercial', projects: 1, totalValue: 28000,  lastContact: '2026-05-15', notes: 'Pay on time. Open to expansion.' },
      { id: 4, name: 'B. Khoury',       email: 'b.khoury@email.com',     phone: '07700 900 444', address: 'Islington, N1',          tag: 'New',     projects: 0, totalValue: 0,      lastContact: '2026-05-18', notes: 'Quote sent for 2-storey extension £96k. Followed up Mon.' },
      { id: 5, name: 'Park Towers Mgmt', email: 'mgmt@parktowers.co.uk', phone: '07700 900 555', address: 'Streatham, SW16',        tag: 'Commercial', projects: 1, totalValue: 64000,  lastContact: '2026-04-15', notes: '5-block estate. Annual maintenance opportunity.' },
    ],
    leads: [
      { id: 1, name: 'M. Ortiz',       inquiry: 'Kitchen refit · Shoreditch',  value: 22500, source: 'Referral', stage: 'qualified', updated: '2026-05-20' },
      { id: 2, name: 'K. Daniels',     inquiry: 'Loft conversion · Highbury',  value: 58000, source: 'Website',  stage: 'quoted',    updated: '2026-05-02' },
      { id: 3, name: 'TFG Ltd',        inquiry: 'Cafe fit-out · Old Street',   value: 41200, source: 'Linkedin', stage: 'lost',      updated: '2026-04-22' },
      { id: 4, name: 'Jenny Park',     inquiry: 'Garden room · Walthamstow',   value: 18000, source: 'Referral', stage: 'new',       updated: '2026-05-22' },
      { id: 5, name: 'D. Mensah',      inquiry: 'Bathroom refit · Crouch End', value: 9500,  source: 'Website',  stage: 'new',       updated: '2026-05-21' },
      { id: 6, name: 'S. Ahmed',       inquiry: 'Boundary wall · Walthamstow', value: 4200,  source: 'Walk-in',  stage: 'qualified', updated: '2026-05-19' },
    ],
    mileage: [
      { id: 1, date: '2026-05-22', from: 'Office',         to: 'Camden Mews',  miles: 4.2, purpose: 'Site visit',   driver: 'You', amount: 1.89 },
      { id: 2, date: '2026-05-22', from: 'Camden Mews',    to: 'Hackney Loft', miles: 3.4, purpose: 'Multi-site',    driver: 'You', amount: 1.53 },
      { id: 3, date: '2026-05-21', from: 'Office',         to: 'Brixton',      miles: 6.8, purpose: 'Snag walk',     driver: 'You', amount: 3.06 },
      { id: 4, date: '2026-05-20', from: 'Office',         to: 'Islington',    miles: 5.1, purpose: 'Client meeting', driver: 'You', amount: 2.30 },
      { id: 5, date: '2026-05-19', from: 'Camden Mews',    to: 'Travis Perkins', miles: 1.2, purpose: 'Materials',   driver: 'Tom', amount: 0.54 },
    ],
  };
  ...
  Backend.ai.scoreLead = async (lead) => {
    ...
    try {
      ...
      const json = raw.match(/\{[\s\S]*\}/)?.[0];
      return JSON.parse(json);
    } catch (e) { return { score: 5, reasoning: 'Unable to score' }; }
  };
})();
...
// INSPECTIONS
// ═══════════════════════════════════════════════════════════════════
const INSP_STATUS_C = { passed: T.green, failed: T.red, scheduled: T.amber };
const INSP_TEMPLATES = [
  { k: 'first-fix-elec',  l: 'First-fix electrical', i: Ic.zap },
  { k: 'plaster-prep',    l: 'Plaster prep',          i: Ic.layers },
  { k: 'snag-walk',       l: 'Snag walk',             i: Ic.list },
  { k: 'handover',        l: 'Handover sign-off',     i: Ic.check },
  { k: 'health-safety',   l: 'H&S audit',             i: Ic.shield },
  { k: 'pre-pour',        l: 'Pre-pour concrete',     i: Ic.box },
];

function InspectionsScreen({ accent, onOpen }) {
  const inspections = useDB('inspections');
  const projects = useDB('projects');
  const [seg, setSeg] = React.useState('all');
  const [projectFilter, setProjectFilter] = React.useState('all');
  const [showFilter, setShowFilter] = React.useState(false);
  const filtered = (seg === 'all' ? inspections : inspections.filter(i => i.status === seg))
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
          ...
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
            {INSP_TEMPLATES.map(t => (
              <button key={t.k} onClick={() => { toast(`New ${t.l} inspection`, 'success'); window.cortexxNav('addinspection'); }} style={{
                ...
                <span style={{ color: accent }}>{React.cloneElement(t.i, { size: 14 })}</span>
        ...
        <div style={{ padding: '0 16px 14px' }}>
          <SegControl value={seg} onChange={setSeg} options={[
            { k: 'all', l: 'All', n: inspections.length },
            { k: 'passed', l: 'Passed', n: inspections.filter(i => i.status === 'passed').length },
            { k: 'failed', l: 'Failed', n: inspections.filter(i => i.status === 'failed').length },
            { k: 'scheduled', l: 'Due', n: inspections.filter(i => i.status === 'scheduled').length },
          ]}/>
        </div>
              ...
              <div key={insp.id} onClick={() => onOpen && onOpen(insp)} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: SF, fontSize: 14, fontWeight: 600, color: T.t1 }}>{insp.kind}</div>
                    <div style={{ fontFamily: SF, fontSize: 12, color: T.t2, marginTop: 3 }}>
                      {proj?.name?.split(' ').slice(0,2).join(' ')} · {insp.inspector} · {_formatRelDate(insp.date)}
                    </div>
                  </div>
                  <Pill c={INSP_STATUS_C[insp.status]}>{insp.status}</Pill>
                </div>
                {insp.items.length > 0 && (
                  <div style={{ marginTop: 12, paddingTop: 10, borderTop: `0.5px solid ${T.hair}` }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <Bar pct={passed / insp.items.length * 100} c={INSP_STATUS_C[insp.status]} h={3}/>
                      <span style={{ fontFamily: SFMono, fontSize: 11, color: T.t2, fontWeight: 600 }}>{passed}/{insp.items.length}</span>
                    </div>
                  </div>
                )}
              </div>
...
function InspectionDetailSheet({ inspection, onClose, accent }) {
  const projects = useDB('projects');
  ...
  const toggleItem = (i) => setItems(items.map((it, j) => j === i ? { ...it, ok: !it.ok } : it));
  const sign = () => {
    window.cortexxNav('signature', {
      ...
      onSigned: async () => {
        await Backend.db.inspections.update(inspection.id, {
          items, status: allPass ? 'passed' : 'failed', date: '2026-05-22',
        });
    ...
    <Sheet onClose={onClose}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        <div style={{ padding: '4px 20px 16px' }}>
          <Pill c={INSP_STATUS_C[inspection.status]}>{inspection.status}</Pill>
          <div style={{ fontFamily: SF, fontSize: 22, fontWeight: 700, color: T.t1, marginTop: 8, letterSpacing: -0.4 }}>{inspection.kind}</div>
          <div style={{ fontFamily: SF, fontSize: 13, color: T.t2, marginTop: 4 }}>{proj?.name} · Inspector: {inspection.inspector}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 14 }}>
            <Bar pct={passed / Math.max(items.length, 1) * 100} c={allPass ? T.green : T.amber} h={5}/>
            <span style={{ fontFamily: SFMono, fontSize: 13, color: T.t1, fontWeight: 700 }}>{passed}/{items.length}</span>
          </div>
        </div>
          ...
          <GroupedList>
            {items.map((it, i) => (
              <div key={i} onClick={() => toggleItem(i)} style={{
                ...
                }}>{it.ok && <span style={{ color: '#fff' }}>{React.cloneElement(Ic.check, { size: 14, sw: 3 })}</span>}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: SF, fontSize: 14, color: T.t1, fontWeight: 500, lineHeight: 1.4 }}>{it.q}</div>
                  {it.note && <div style={{ fontFamily: SF, fontSize: 12, color: T.amber, marginTop: 4, fontStyle: 'italic' }}>↳ {it.note}</div>}
                </div>
              </div>
            ))}
          </GroupedList>
...
// CUSTOMERS / CRM
// ═══════════════════════════════════════════════════════════════════
const TAG_C = { Active: T.blue, Repeat: T.green, Commercial: T.purple, New: T.amber };
function CustomersScreen({ accent, onOpen }) {
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
        ...
        <div style={{ padding: '4px 16px 14px' }}>
          <div style={{
            ...
            <span style={{ color: T.t3 }}>{React.cloneElement(Ic.search, { size: 14 })}</span>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search customers…"
              style={{ flex: 1, background: 'transparent', border: 'none', color: T.t1, fontFamily: SF, fontSize: 13, outline: 'none' }}/>
          </div>
        </div>
        <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {filtered.map(c => (
            <div key={c.id} onClick={() => onOpen && onOpen(c)} style={{
              ...
              <Avatar name={c.name} size={44} c={TAG_C[c.tag] || accent}/>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <div style={{ fontFamily: SF, fontSize: 14, fontWeight: 600, color: T.t1 }}>{c.name}</div>
                  <Pill c={TAG_C[c.tag] || accent} size="xs">{c.tag}</Pill>
                </div>
                <div style={{ fontFamily: SF, fontSize: 11, color: T.t2, marginTop: 2 }}>{c.email}</div>
                <div style={{ fontFamily: SFMono, fontSize: 10, color: T.t3, marginTop: 3 }}>
                  {c.projects} project{c.projects !== 1 && 's'} · £{(c.totalValue/1000).toFixed(0)}k · last contact {_formatRelDate(c.lastContact)}
                </div>
              </div>
            </div>
          ))}
...
function CustomerDetailSheet({ customer, onClose, accent }) {
  const projects = useDB('projects');
    ...
    <Sheet onClose={onClose}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        <div style={{ padding: '4px 20px 18px', display: 'flex', gap: 16, alignItems: 'center' }}>
          <Avatar name={customer.name} size={64} c={TAG_C[customer.tag] || accent}/>
          <div>
            <Pill c={TAG_C[customer.tag] || accent}>{customer.tag}</Pill>
            <div style={{ fontFamily: SF, fontSize: 22, fontWeight: 700, color: T.t1, letterSpacing: -0.4, marginTop: 6, lineHeight: 1.1 }}>{customer.name}</div>
          </div>
        </div>

        <Section title="Contact">
          <GroupedList>
            <Row icon={Ic.mail} iconBg={T.blue} title={customer.email} sub="Email — tap to compose"
              onClick={() => window.open(`mailto:${customer.email}?subject=${encodeURIComponent(customer.name + ' — Cortexx')}`, '_blank')}/>
            <Row icon={Ic.phone} iconBg={T.green} title={customer.phone} sub="Phone — tap to call"
              onClick={() => window.open(`tel:${customer.phone.replace(/\s+/g, '')}`, '_blank')}/>
            <Row icon={Ic.pin} iconBg={T.amber} title={customer.address} sub="Address — tap for map" isLast
              onClick={() => window.open(`https://maps.google.com/?q=${encodeURIComponent(customer.address)}`, '_blank')}/>
          </GroupedList>
        </Section>

        <Section title={`Projects · ${custProjects.length}`}>
          {custProjects.length === 0 ? (
            <div style={{ padding: 20, fontFamily: SF, fontSize: 13, color: T.t3, textAlign: 'center' }}>No projects yet</div>
          ) : (
            <GroupedList>
              {custProjects.map((p, i, a) => (
                <Row key={p.id} icon={Ic.projects} iconBg={STATUS_C[p.status]}
                  title={p.name} sub={`${fmt(p.value)} · ${p.pct}%`}
                  isLast={i === a.length - 1}
        ...
        <Section title="Notes">
          <div style={{ background: T.bg2, border: `0.5px solid ${T.hair}`, borderRadius: 14, padding: 14, fontFamily: SF, fontSize: 13, color: T.t1, lineHeight: 1.6 }}>
            {customer.notes}
          </div>
        </Section>
      </div>
...
// ═══════════════════════════════════════════════════════════════════
const STAGES = [
  { k: 'new',       l: 'New',       c: T.cyan },
  { k: 'qualified', l: 'Qualified', c: T.amber },
  { k: 'quoted',    l: 'Quoted',    c: T.purple },
  { k: 'won',       l: 'Won',       c: T.green },
  { k: 'lost',      l: 'Lost',      c: T.t3 },
];
function LeadsScreen({ accent }) {
  ...
  const advance = async (lead) => {
    ...
    if (idx < STAGES.length - 2) { // don't advance past won
      const newStage = STAGES[idx + 1].k;
      await Backend.db.leads.update(lead.id, { stage: newStage });
  ...
  return (
    <ScreenBg accent={accent}>
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
          ...
          {STAGES.map(s => {
            ...
            return (
              <button key={s.k} onClick={() => setActiveStage(s.k)} style={{
                ...
                padding: '8px 12px', borderRadius: 14, cursor: 'pointer', whiteSpace: 'nowrap',
                display: 'flex', alignItems: 'center', gap: 5,
              }}>
                <span style={{ width: 6, height: 6, borderRadius: 3, background: s.c }}/>
                {s.l}
                <span style={{ color: T.t3 }}>{n}</span>
              </button>
            );
          })}
        </div>
        ...
        <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {inStage.length === 0 ? (
            <div style={{ padding: 30, fontFamily: SF, fontSize: 13, color: T.t3, textAlign: 'center' }}>No leads in {STAGES.find(s=>s.k===activeStage)?.l}</div>
          ) : inStage.map(l => {
            ...
            return (
              <div key={l.id} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: SF, fontSize: 14, fontWeight: 600, color: T.t1 }}>{l.name}</div>
                    <div style={{ fontFamily: SF, fontSize: 12, color: T.t2, marginTop: 3, lineHeight: 1.3 }}>{l.inquiry}</div>
                    <div style={{ display: 'flex', gap: 5, marginTop: 6 }}>
                      <Pill c={T.cyan} size="xs">{l.source}</Pill>
                      <Pill c={T.t3} size="xs">{_formatRelDate(l.updated)}</Pill>
                    </div>
                  </div>
                  <div style={{ fontFamily: SFMono, fontSize: 16, color: T.t1, fontWeight: 700 }}>£{(l.value/1000).toFixed(0)}k</div>
                </div>
            ...
            {Array.from({ length: grid }).map((_, i) => {
              ...
              return (
                <div key={i} onClick={() => toast(`Photo ${i+1} opened`, 'info')} style={{
                  ...
                  <svg width="100%" height="100%" viewBox="0 0 60 60" style={{ opacity: 0.25 }}>
                    <rect x="6" y="14" width="48" height="36" fill="none" stroke="#fff" strokeWidth="0.5"/>
                    <line x1="6" y1="32" x2="54" y2="32" stroke="#fff" strokeWidth="0.5"/>
                    <line x1="30" y1="14" x2="30" y2="50" stroke="#fff" strokeWidth="0.3" strokeDasharray="2 2"/>
                  </svg>
                  {tag && (
                    <div style={{
                      position: 'absolute', top: 4, left: 4,
                      background: accent, color: '#fff',
                      fontFamily: SF, fontSize: 8, fontWeight: 700,
                  ...
                  <div style={{
                    ...
                    fontFamily: SFMono, fontSize: 8,
                    padding: '1px 4px', borderRadius: 3,
                  }}>{Math.floor(i/3) + 7}:{(i*7)%60 < 10 ? '0' : ''}{(i*7)%60}</div>
                </div>
              );
            })}
          </div>
          <div style={{ textAlign: 'center', padding: '20px 0', fontFamily: SF, fontSize: 12, color: T.t3 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
        ...
        <div style={{ padding: '4px 16px 14px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <div style={{ background: T.bg2, borderRadius: 12, padding: 12, border: `0.5px solid ${T.hair}` }}>
            <div style={{ fontFamily: SF, fontSize: 10, color: T.t2, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 }}>This week</div>
            <div style={{ fontFamily: SFMono, fontSize: 24, color: T.t1, fontWeight: 700, marginTop: 4, letterSpacing: -0.5 }}>{totalMiles.toFixed(1)} mi</div>
          </div>
          <div style={{ background: T.bg2, borderRadius: 12, padding: 12, border: `0.5px solid ${T.hair}` }}>
            <div style={{ fontFamily: SF, fontSize: 10, color: T.t2, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 }}>Reimburse @ 45p</div>
            <div style={{ fontFamily: SFMono, fontSize: 24, color: T.green, fontWeight: 700, marginTop: 4, letterSpacing: -0.5 }}>£{reimburse.toFixed(2)}</div>
          </div>
        </div>
            ...
            {trips.map((t, i, a) => (
              <div key={t.id} style={{
                ...
                <div style={{ width: 32, height: 32, borderRadius: 8, background: `${accent}22`, color: accent, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  {React.cloneElement(Ic.pin, { size: 16 })}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontFamily: SF, fontSize: 13, fontWeight: 600, color: T.t1 }}>{t.from} → {t.to}</div>
                  <div style={{ fontFamily: SF, fontSize: 11, color: T.t2, marginTop: 2 }}>{t.purpose} · {t.driver} · {_formatRelDate(t.date)}</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontFamily: SFMono, fontSize: 13, color: T.t1, fontWeight: 700 }}>{t.miles}mi</div>
                  <div style={{ fontFamily: SFMono, fontSize: 10, color: T.green, marginTop: 1 }}>£{t.amount.toFixed(2)}</div>
                </div>
              </div>
            ))}
          </GroupedList>
...
Object.assign(window, {
  InspectionsScreen, InspectionDetailSheet,
  CustomersScreen, CustomerDetailSheet,
  LeadsScreen, PhotosScreen, MileageScreen,
});
