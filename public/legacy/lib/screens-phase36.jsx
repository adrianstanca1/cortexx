...
(function() {
  ...
  window.cortexxPhotoStore = {
    async save(file, meta = {}) {
      ...
      const record = {
        blob, name: meta.name || 'photo.jpg', type: blob.type || 'image/jpeg',
        size: blob.size, projectId: meta.projectId || null,
        when: new Date().toISOString().slice(0, 16), tags: meta.tags || [],
      };
    ...
    async listSync() {
      try { return await this.list(); } catch (e) { return []; }
    },
    async remove(id) {
...
function PhotosV2Screen({ accent }) {
  ...
  const handleFile = async (e) => {
    ...
    for (const f of files) {
      await window.cortexxPhotoStore.save(f, { name: f.name, projectId: 1 });
    }
    await refresh();
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
        ...
        {photos.length === 0 ? (
          <div onClick={() => fileInput.current?.click()} style={{
            ...
            <div style={{ color: accent, fontSize: 36, marginBottom: 8 }}>{React.cloneElement(Ic.camera, { size: 36 })}</div>
            <div style={{ fontFamily: SF, fontSize: 14, color: T.t1, fontWeight: 600 }}>Tap to take or pick photos</div>
            <div style={{ fontFamily: SF, fontSize: 11, color: T.t2, marginTop: 4 }}>Stored locally · works offline</div>
          </div>
        ) : (
              ...
              {photos.map(p => {
                ...
                return (
                  <div key={p.id} onClick={() => setViewing({...p, url})} style={{
                    ...
                    <div style={{ position: 'absolute', bottom: 4, right: 4, background: 'rgba(0,0,0,0.7)', color: '#fff', fontFamily: SFMono, fontSize: 8, padding: '1px 4px', borderRadius: 3 }}>
        ...
        {viewing && (
          <div onClick={() => setViewing(null)} style={{
            ...
            <div style={{ padding: 16, color: '#fff', fontFamily: SF, fontSize: 12, textAlign: 'center' }}>
              {viewing.name} · {new Date(viewing.when).toLocaleString('en-GB')}
            </div>
          </div>
        )}
      </div>
...
Object.assign(window, { usePhotos, PhotosV2Screen });
