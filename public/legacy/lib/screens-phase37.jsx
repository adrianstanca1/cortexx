...
// ═══════════════════════════════════════════════════════════════════
function InlineField({ value, onSave, placeholder, type = 'text', size = 'md', accent }) {
  ...
  if (editing) {
    return <input ref={ref} value={draft} type={type}
      onChange={e => setDraft(e.target.value)}
      onBlur={commit}
      onKeyDown={e => { if (e.key === 'Enter') commit(); if (e.key === 'Escape') { setDraft(value); setEditing(false); }}}
      style={{
        background: T.bg3, border: `1px solid ${accent || T.blue}`, borderRadius: 6,
        padding: '4px 8px', color: T.t1, fontFamily: SF, fontSize, fontWeight: 600,
...
// ═══════════════════════════════════════════════════════════════════
function ResponsiveSidebar({ tab, setTab, accent }) {
  ...
  return (
    <div style={{
      position: 'fixed', left: 0, top: 0, bottom: 0, width: 220, zIndex: 5,
      background: T.bg1, borderRight: `0.5px solid ${T.hair}`,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '0 8px 24px' }}>
        <div style={{ width: 32, height: 32, borderRadius: 8, background: `linear-gradient(135deg, ${accent}, ${T.purple})`, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {React.cloneElement(Ic.spark, { size: 17 })}
        </div>
        <div style={{ fontFamily: SF, fontSize: 16, fontWeight: 700, color: T.t1, letterSpacing: -0.3 }}>Cortexx</div>
      </div>
      {tabs.map(t => (
      ...
      }}>{React.cloneElement(Ic.search, { size: 14 })} <span style={{ flex: 1 }}>Quick jump</span> <span style={{ fontFamily: SFMono, fontSize: 9, background: T.bg2, padding: '2px 5px', borderRadius: 3 }}>⌘K</span></button>
    </div>
  );
}

Object.assign(window, { InlineField, ResponsiveSidebar });
