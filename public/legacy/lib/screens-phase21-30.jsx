...
function APIScreen({ accent }) {
  return <ScreenBg accent={accent}><div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
    <MobileHeader title="API & developer" subtitle="Build on Cortexx · webhooks · keys"/>
...
function RolesScreen({ accent }) {
  ...
  return <ScreenBg accent={accent}><div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
    <MobileHeader title="Roles & permissions" subtitle={`${ROLES.length} roles · ${ROLES.reduce((s, r) => s + r.members, 0)} seats`}
      right={<HeaderBtn icon={Ic.plus} accent={accent} onClick={() => window.open('mailto:hello@cortexx.app?subject=Custom%20role%20request', '_blank')}/>}/>
    {ROLES.map((r, i) => (
...
function TourSheet({ onClose, accent }) {
  ...
  const STEPS = [
    { t: 'Welcome to Cortexx', d: 'The construction OS built for UK SMB contractors. Let me show you around.', i: Ic.spark, c: T.purple },
