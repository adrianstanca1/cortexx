                ...
                {projectInvoices.map((iv, i, a) => {
                  ...
                  return (
                    <Row key={iv.id}
                      ...
                      right={<span style={{ fontFamily: SFMono, fontSize: 11, color: c, fontWeight: 700, textTransform: 'uppercase' }}>{iv.status}</span>}
                      isLast={i === a.length - 1}/>
                  );
                })}
              </GroupedList>
        ...
        {appGroups.map((g, gi) => (
          <div key={gi} style={{ marginBottom: 16 }}>
            ...
            <div style={{ padding: '0 16px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
              {g.items.map((a) => <AppTile key={a.k} a={a}/>)}
            </div>
          </div>
        ))}
      </div>
...
function AISheet({ onClose, accent }) {
  ...
  const send = async (text) => {
    ...
    try {
      ...
      setMessages(m => [...m, { who: 'ai', t: "Hmm, I couldn't connect just now. Try again?" }]);
    }
    setThinking(false);
  };
    ...
    <Sheet onClose={onClose} fullscreen>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderBottom: `0.5px solid ${T.hair}` }}>
        ...
        <div style={{ flex: 1, textAlign: 'center' }}>
          ...
          <div style={{ fontFamily: SF, fontSize: 11, color: T.green, fontWeight: 500, marginTop: 1 }}>● Online</div>
        </div>
        <div style={{ width: 50 }}/>
      </div>
