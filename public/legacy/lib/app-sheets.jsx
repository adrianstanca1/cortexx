...
function CaptureSheet({
  ...
  }, "All apps"), appGroups.map((g, gi) => React.createElement("div", {
    key: gi,
    style: {
      marginBottom: 16
    }
  ...
  }, g.items.map(a => React.createElement(AppTile, {
    key: a.k,
    a: a
  })))))));
}
function AISheet({
  ...
  const send = async text => {
    ...
    setThinking(true);
    try {
      const response = await Backend.ai.ask(text);
      setMessages(m => [...m, {
        who: 'ai',
        t: response
      }]);
      ...
      setMessages(m => [...m, {
        who: 'ai',
        t: "Hmm, I couldn't connect just now. Try again?"
      }]);
    }
    setThinking(false);
  };
  ...
  }, m.t)), thinking && React.createElement("div", {
    style: {
