  ...
  window.cortexxUniversalUpload = async (opts = {}) => {
    ...
    return new Promise((resolve) => {
      input.onchange = async (e) => {
        ...
        if (files.length === 0) { resolve([]); document.body.removeChild(input); return; }
        const results = [];
        for (const f of files) {
          ...
          if (isImage && window.cortexxPhotoStore) {
            await window.cortexxPhotoStore.save(f, { name: f.name, projectId: opts.projectId || null });
            results.push({ kind: 'photo', name: f.name });
          } else if (isDwg && window.Backend?.db?.drawings) {
            await Backend.db.drawings.create({
              name: f.name.replace(/\.[a-z]+$/i, ''),
              projectId: opts.projectId || 1, version: 'v1',
              updated: new Date().toISOString().slice(0,10), markups: 0, type: 'plan',
            });
            results.push({ kind: 'drawing', name: f.name });
          } else if (window.Backend?.db?.documents) {
            await Backend.db.documents.create({
              name: f.name, type: ext, size: Math.round(f.size / 1024),
              projectId: opts.projectId || 1, folder: opts.folder || 'Uploads',
              uploaded: new Date().toISOString().slice(0,10), updatedBy: 'You',
            });
            results.push({ kind: 'document', name: f.name });
          }
        }
        if (window.cortexxToast) {
          const grouped = results.reduce((a, r) => ({ ...a, [r.kind]: (a[r.kind] || 0) + 1 }), {});
...
function FloatingUploadPill({ accent, bottom = 158 }) {
  return (
    <button onClick={() => window.cortexxUniversalUpload()} style={{
      ...
      <span style={{ color: accent || T.cyan }}>{React.cloneElement(Ic.upload, { size: 14 })}</span>
      <span>Upload</span>
    </button>
  );
}

Object.assign(window, { FloatingUploadPill });
