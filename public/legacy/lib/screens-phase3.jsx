    ...
    <Sheet onClose={onClose} fullscreen>
      <div style={{ flex: 1, padding: '60px 28px', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ fontFamily: SF, fontSize: 24, fontWeight: 700, color: T.t1, letterSpacing: -0.5 }}>Cortexx</div>
        </div>
...
function SettingsScreen({ accent }) {
  ...
  const ROOT_SECTIONS = [
    ...
    { k: 'about',        l: 'About Cortexx',   d: 'v2.0 · build 247',     i: Ic.spark,    c: T.purple },
  ];
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
        ...
        {section === 'billing' && <>
          ...
          <Section title="Upgrade">
            <GroupedList>
              <Row icon={Ic.star} iconBg={T.amber} title="Cortexx Pro" sub="£29/mo · 11–50 staff · advanced AI" onClick={async () => { await Backend.db.user.update({ plan: 'Pro · £29/mo' }); toast('Upgraded to Pro · 30-day free trial started', 'success'); }}/>
              <Row icon={Ic.layers} iconBg={T.purple} title="Cortexx Enterprise" sub="Custom · 51+ staff · SSO + audit" isLast
                onClick={() => window.open('mailto:sales@cortexx.app?subject=Cortexx%20Enterprise%20enquiry', '_blank')}/>
            </GroupedList>
          </Section>
        </>}
          ...
          <Section title="Your data">
            <GroupedList>
              ...
              <Row icon={Ic.trash} iconBg={T.red} title="Delete workspace" sub="GDPR — permanent" danger isLast
                onClick={() => window.open('mailto:hello@cortexx.app?subject=GDPR%20deletion%20request&body=Please%20delete%20my%20Cortexx%20workspace%20and%20all%20personal%20data.', '_blank')}/>
            </GroupedList>
          </Section>
        )}
        ...
        {section === 'about' && <>
          <Section>
            <div style={{ textAlign: 'center', padding: '30px 0' }}>
              ...
              <div style={{ fontFamily: SF, fontSize: 22, fontWeight: 700, color: T.t1, marginTop: 14, letterSpacing: -0.4 }}>Cortexx</div>
          ...
          <Section title="Links">
            <GroupedList>
              ...
              <Row icon={Ic.mail} iconBg={T.green} title="Contact support" sub="hello@cortexx.app"
                onClick={() => window.open('mailto:hello@cortexx.app', '_blank')}/>
              <Row icon={Ic.share} iconBg={T.purple} title="Share Cortexx" isLast
...
const FAQ = [
  ...
  { q: 'Does Cortexx handle CIS deductions?',
  ...
  { q: 'Is there a desktop version?',
    a: 'Cortexx is mobile-first but works on tablets and desktops via web browser. A native desktop app is on our roadmap for Q3.' },
    ...
    <ScreenBg accent={accent}>
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
        ...
        <Section title="Get in touch">
          <GroupedList>
            <Row icon={Ic.mail} iconBg={T.blue} title="Email support" sub="hello@cortexx.app · ~2 hours"
              onClick={() => window.open('mailto:hello@cortexx.app?subject=Cortexx%20support', '_blank')}/>
...
function ClientPortalScreen({ accent }) {
  ...
  const project = projects.find(p => p.id == 1) || projects[0]; // Camden as default
    ...
    <ScreenBg accent={accent}>
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
            ...
            {projInvoices.map((iv, i, a) => {
              ...
              return (
                <Row key={iv.id} icon={Ic.doc} iconBg={c}
                  ...
                  right={<Pill c={c} size="xs">{iv.status}</Pill>}
                  isLast={i === a.length - 1}/>
              );
            })}
          </GroupedList>
        ...
        <div style={{ padding: '20px 20px 0', textAlign: 'center', fontFamily: SF, fontSize: 11, color: T.t3 }}>
          Powered by <span style={{ color: T.purple, fontWeight: 700 }}>Cortexx</span>
        </div>
      </div>
    </ScreenBg>
  );
