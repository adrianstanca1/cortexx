(function () {
  ...
  const RFI_SEED = [{
    id: 'RFI-008',
    projectId: 1,
    subject: 'Kitchen socket layout — confirm spec',
    from: 'Aisha Begum',
    status: 'open',
    priority: 'high',
    opened: '2026-05-22',
    messages: [{
      who: 'Aisha Begum',
      t: 'Hi — we\'re first-fixing the kitchen today. Spec drawing v2 shows 6 doubles + 2 USB-C, but the kitchen designer\'s latest layout shows 8 doubles + 4 USB-C. Which is correct?',
      when: '2026-05-22T07:42'
    }, {
      who: 'You',
      t: 'Going with the kitchen designer\'s — 8 doubles, 4 USB-C. Will email confirmation.',
      when: '2026-05-22T08:01'
    }]
  }, {
    id: 'RFI-007',
    projectId: 1,
    subject: 'Plasterboard joint detail at coving',
    from: 'Tom Reilly',
    status: 'open',
    priority: 'med',
    opened: '2026-05-21',
    messages: [{
      who: 'Tom Reilly',
      t: 'Coving spec is 90mm but ceilings have variable drop (10mm in places). Shadow gap detail or push up tight?',
      when: '2026-05-21T15:20'
    }]
  }, {
    id: 'RFI-006',
    projectId: 2,
    subject: 'Loft stair handrail height',
    from: 'Marcus Webb',
    status: 'answered',
    priority: 'low',
    opened: '2026-05-19',
    messages: [{
      who: 'Marcus Webb',
      t: '1.1m or 900mm for the loft staircase handrail?',
      when: '2026-05-19T10:30'
    }, {
      who: 'You',
      t: '900mm — domestic, single dwelling, Doc K applies.',
      when: '2026-05-19T11:05'
    }]
  }, {
    id: 'RFI-005',
    projectId: 3,
    subject: 'Snag list sign-off',
    from: 'Tonic Café Ltd',
    status: 'closed',
    priority: 'low',
    opened: '2026-05-15',
    messages: [{
      who: 'Tonic Café Ltd',
      t: 'Snag list looks good. Couple of additions on the kitchen tiles.',
      when: '2026-05-15T14:00'
    }, {
      who: 'You',
      t: 'Noted — we\'ll re-do those grout lines this week.',
      when: '2026-05-15T14:22'
    }, {
      who: 'Tonic Café Ltd',
      t: 'Perfect. Signing off when those are done.',
      when: '2026-05-16T09:00'
    }]
  }];
  const MESSAGE_SEED = [{
    id: 1,
    kind: 'team',
    name: 'Camden site crew',
    members: ['Tom', 'Aisha', 'Jack', 'Sara'],
    lastMsg: 'Plasterboard ready for 2nd run',
    lastWho: 'Tom',
    when: '2026-05-22T09:14',
    unread: 2,
    thread: [{
      who: 'Tom Reilly',
      t: 'Morning all — plan for today: I\'m on board prep ground floor.',
      when: '2026-05-22T07:20'
    }, {
      who: 'Aisha Begum',
      t: 'I\'m finishing kitchen first-fix. Should wrap by 11.',
      when: '2026-05-22T07:22'
    }, {
      who: 'Tom Reilly',
      t: 'Plasterboard ready for 2nd run',
      when: '2026-05-22T09:14'
    }]
  }, {
    id: 2,
    ...
    name: 'J. Patterson',
    members: [],
    lastMsg: 'Can we add a radiator to the utility?',
    lastWho: 'J. Patterson',
    when: '2026-05-21T17:30',
    unread: 0,
    thread: [{
      who: 'J. Patterson',
      t: 'Can we add a radiator to the utility?',
      when: '2026-05-21T17:30'
    }, {
      who: 'You',
      t: 'Yes — costed at £680. Variation submitted (CO-002).',
      when: '2026-05-21T18:00'
    }]
  }, {
    id: 3,
    kind: 'sub',
    name: 'Spark Electricals (Vik)',
    members: [],
    lastMsg: 'Available next Thursday morning',
    lastWho: 'Vik',
    when: '2026-05-20T11:00',
    unread: 0,
    thread: [{
      who: 'You',
      t: 'Vik, need a second-fix sparky session at Hackney Loft next week.',
      when: '2026-05-20T10:30'
    }, {
      who: 'Vik Patel',
      t: 'Available next Thursday morning',
      when: '2026-05-20T11:00'
    }]
  }];
  if (!snap.rfis) {
...
function RFIsScreen({
  ...
  return React.createElement(ScreenBg, {
    ...
    options: [{
      ...
      k: 'answered',
      l: 'Answered',
      n: rfis.filter(r => r.status === 'answered').length
    }, {
      k: 'closed',
      l: 'Closed',
    ...
    }, r.messages.length, " ", r.messages.length === 1 ? 'message' : 'messages', " \xB7 ", _formatRelDate(r.opened))));
  }))));
}
function RFIDetailSheet({
  rfi,
  onClose,
  accent
}) {
  ...
  return React.createElement(Sheet, {
    ...
    return React.createElement("div", {
      ...
      style: {
        ...
        paddingLeft: !mine ? 12 : 0,
        paddingRight: mine ? 4 : 0
      }
    }, new Date(m.when).toLocaleTimeString('en-GB', {
      hour: '2-digit',
      minute: '2-digit'
    ...
    style: {
      ...
      height: 36,
      borderRadius: 18,
      background: reply.trim() ? accent : T.bg3,
      border: 'none',
      color: '#fff',
...
function MessagesScreen({
  ...
  return React.createElement(ScreenBg, {
    ...
    right: React.createElement("button", {
      onClick: async () => {
        await Backend.db.messages.create({
          ...
          name: 'New thread',
          members: [],
          lastMsg: 'You started a new thread',
          lastWho: 'You',
          when: new Date().toISOString().slice(0, 16),
          unread: 0,
          thread: [{
            who: 'You',
            t: 'New thread started',
            when: new Date().toISOString().slice(0, 16)
          }]
        });
    ...
    }, _formatRelDate(m.when.slice(0, 10)), " ", new Date(m.when).toLocaleTimeString('en-GB', {
      hour: '2-digit',
      minute: '2-digit'
    }))));
  }))));
...
function MessageThreadSheet({
  ...
  return React.createElement(Sheet, {
    ...
    style: {
      ...
      height: 36,
      borderRadius: 18,
      background: input.trim() ? accent : T.bg3,
      border: 'none',
      color: '#fff',
...
function ReportsScreen({
  ...
  const reports = [{
    k: 'business',
    l: 'Business overview',
    d: 'Cash, pipeline, margins',
    i: Ic.trend,
    c: T.green
  }, {
    k: 'project',
    l: 'Active projects',
    d: 'Health, timelines, blockers',
    i: Ic.layers,
    c: T.blue
  }, {
    k: 'safety',
    l: 'Safety & H&S',
    d: 'Score, snags, compliance',
    i: Ic.shield,
    c: T.amber
  }];
  return React.createElement(ScreenBg, {
    ...
    right: React.createElement("button", {
      onClick: () => {
        window.print();
        toast('PDF dialog opened', 'success');
      },
      style: {
        background: accent,
        color: '#fff',
    ...
    onClick: async () => {
      try {
        ...
        try {
          ...
          toast('Report ready to share', 'info');
        }
      }
    },
    style: {
  ...
  return React.createElement(ScreenBg, {
    ...
    return React.createElement("div", {
      ...
      style: {
        ...
        width: (bar.end - bar.start) * dayWidth,
        top: 4,
        height: 20,
        borderRadius: 5,
