...
function ProjectsScreen({
  ...
  return React.createElement(ScreenBg, {
    ...
    l: 'Pipeline',
    v: '£' + (projects.reduce((s, p) => s + p.value, 0) / 1000).toFixed(0) + 'k',
    c: T.t1
  }, {
    l: 'In progress',
    v: counts.active,
...
const formatTaskWhen = iso => {
  ...
  if (days < 7) return new Date(iso).toLocaleDateString('en-GB', {
    weekday: 'long'
  });
...
function TasksScreen({
  ...
  React.useEffect(() => {
    ...
    return () => {
      if (window.__cortexxTaskSel?.ids === selected) window.__cortexxTaskSel = null;
    };
  }, [selected]);
  ...
  const toggleTask = (id, current) => Backend.db.tasks.update(id, {
    done: !current
  });
  ...
  const bulkComplete = async () => {
    for (const id of selected) await Backend.db.tasks.update(id, {
      done: true
    });
  ...
  return React.createElement(ScreenBg, {
    ...
    return React.createElement("div", {
      key: task.id,
      onClick: () => selected.size > 0 ? toggleSelect(task.id) : toggleTask(task.id, task.done),
      onContextMenu: e => {
        e.preventDefault();
      ...
      style: {
        ...
        borderBottom: i === list.length - 1 ? 'none' : `0.5px solid ${T.hair}`,
