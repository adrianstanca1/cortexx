        ...
        <Section title={seg === 'todo' ? 'Your queue' : 'Completed'}>
          <GroupedList>
            {list.map((task, i) => {
              ...
              return (
              <div key={task.id} onClick={() => selected.size > 0 ? toggleSelect(task.id) : toggleTask(task.id, task.done)}
                onContextMenu={(e) => { e.preventDefault(); toggleSelect(task.id); }}
                style={{
                display: 'flex', alignItems: 'center', gap: 12,
                ...
                <div onClick={(e) => { e.stopPropagation(); toggleSelect(task.id); }} style={{
