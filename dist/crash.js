...
(function () {
  ...
  function capture(err, ctx) {
    ...
    if (!DSN) {
      try {
        console.error('[CortexCrash]', message, ctx || '');
      } catch (e) {}
      return;
    }
    const payload = {
      ...
      contexts: {
        app: {
          app_version: REL,
          app_name: 'CortexBuild Pro'
        },
        runtime: {
          name: 'browser',
          version: (navigator.userAgent || '').slice(0, 200)
        }
      },
      extra: Object.assign({}, ctx || {}, {
        url: location.href
      })
    };
    send(payload);
  ...
  function init(opts) {
    opts = opts || {};
    DSN = parseDSN(opts.dsn || function () {
      try {
        return localStorage.getItem('cortexx_sentry_dsn');
      } catch (e) {
        return null;
      }
    }());
    ...
    if (opts.dsn) {
      try {
        localStorage.setItem('cortexx_sentry_dsn', opts.dsn);
      } catch (e) {}
    }
    return !!DSN;
  }
  ...
  window.addEventListener('error', e => {
    if (e && e.error) capture(e.error, {
      source: 'window.error',
      file: e.filename,
      line: e.lineno
    });else capture(new Error(e.message || 'window error'), {
      source: 'window.error'
    });
  });
  window.addEventListener('unhandledrejection', e => {
    const r = e && e.reason;
    capture(r instanceof Error ? r : new Error(String(r)), {
  ...
  try {
    if (localStorage.getItem('cortexx_sentry_dsn')) init({});
  } catch (e) {}
  window.CortexCrash = {
    init: init,
    capture: capture,
    ...
    status: () => ({
      enabled: !!DSN,
      dsn: DSN ? DSN.host + '/' + DSN.projectId : null,
      sent,
      lastError,
      env: ENVT,
      release: REL
    ...
    clearDSN: () => {
      try {
        localStorage.removeItem('cortexx_sentry_dsn');
      } catch (e) {}
      DSN = null;
    }
  };
