  ...
  B.map = {
    ...
    async downloadPack(lat, lng, zMin, zMax, spanTiles, onProgress) {
      ...
      for (let z = zMin; z <= zMax; z++) {
        const cx = Math.floor(lon2x(lng, z)),
          cy = Math.floor(lat2y(lat, z));
        for (let dx = -spanTiles; dx <= spanTiles; dx++) for (let dy = -spanTiles; dy <= spanTiles; dy++) urls.push(tileUrl(z, cx + dx, cy + dy));
      }
      let done = 0,
        failed = 0;
      for (const url of urls) {
        try {
          ...
          if (!hit) {
            ...
            if (r.ok) await cache.put(url, r.clone());else failed++;
          }
        } catch (e) {
          failed++;
        }
        done++;
        onProgress && onProgress(done, urls.length, failed);
      }
      return {
        ok: true,
        total: urls.length,
...
function OfflineMapScreen({
  ...
  React.useEffect(() => {
    Backend.map.cachedCount().then(setCached);
  }, [dl]);
  React.useEffect(() => {
  ...
  const saveMarks = async next => {
    ...
    const all = Backend.db.siteMaps && Backend.db.siteMaps.listSync ? Backend.db.siteMaps.listSync() : [];
    const rec = all.find(m => m.projectId == pid);
    if (rec) await Backend.db.siteMaps.update(rec.id, {
      marks: next
    });else await Backend.db.siteMaps.create({
      projectId: pid,
      marks: next
    });
  };
  ...
  const toScreen = (lat, lng) => ({
    x: Backend.map.lon2x(lng, zoom) * TS - centerPx.x + W / 2,
    y: Backend.map.lat2y(lat, zoom) * TS - centerPx.y + H / 2
  });
  const toLatLng = (sx, sy) => ({
  ...
  for (let dx = -span; dx <= span; dx++) for (let dy = -span; dy <= span; dy++) {
    const tx = Math.floor(ctx) + dx,
      ty = Math.floor(cty) + dy;
    if (tx < 0 || ty < 0 || tx >= 2 ** zoom || ty >= 2 ** zoom) continue;
    const px = (tx - ctx) * TS + W / 2,
      py = (ty - cty) * TS + H / 2;
    tiles.push({
      tx,
      ty,
  ...
  const onMove = e => {
    ...
    const sy = (e.touches ? e.touches[0].clientY : e.clientY) - r.top;
    if (tool === 'pan' && dragStart.current) {
      const dxp = sx - dragStart.current.sx,
        dyp = sy - dragStart.current.sy;
      const nx = Backend.map.lon2x(dragStart.current.lng, zoom) * TS - dxp;
      const ny = Backend.map.lat2y(dragStart.current.lat, zoom) * TS - dyp;
      setCenter({
        lng: Backend.map.x2lon(nx / TS, zoom),
        lat: Backend.map.y2lat(ny / TS, zoom)
      });
  ...
  const onUp = () => {
    if (tool === 'draw' && drawing.current) {
      saveMarks([...marks.filter(x => x.id !== drawing.current.id), drawing.current]);
  ...
  const download = async () => {
    ...
    const r = await Backend.map.downloadPack(center.lat, center.lng, 15, 18, 2, (done, total, failed) => setDl({
      done,
      total,
      failed
    }));
  ...
  return /*#__PURE__*/React.createElement(ScreenBg, {
    ...
    style: {
      flex: 1,
      background: tool === t.k ? accent : T.bg2,
      color: tool === t.k ? '#fff' : T.t2,
      border: `0.5px solid ${tool === t.k ? accent : T.hairMid}`,
      borderRadius: 11,
      padding: '11px 6px',
      fontFamily: SF,
      fontSize: 12,
  ...
  }, "Caches zoom 15\u201318 around the current view (~", Backend.map.estimateTiles(0, 0, 15, 18, 2), " tiles). Markup saves to this project and syncs when you're online.")))));
}
const zoomBtn = {
  width: 34,
  height: 34,
