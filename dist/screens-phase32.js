...
function CommandPalette({
  ...
  const results = query ? ALL.filter(a => a.l.toLowerCase().includes(query)).slice(0, 12) : ALL.slice(0, 8);
  const run = item => {
    if (item.action) {
      item.action();
      onClose();
    ...
    if (item.k === 'tab') window.cortexxNav('tab', item.payload);else window.cortexxNav(item.k);
    onClose();
  };
  const onKey = e => {
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault();
      results[activeIdx] && run(results[activeIdx]);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    style: {
      width: '100%',
      background: i === activeIdx ? T.bg3 : 'transparent',
      border: 'none',
      padding: '8px 12px',
      cursor: 'pointer',
      display: 'flex',
