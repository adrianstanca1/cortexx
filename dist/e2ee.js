...
(function () {
  ...
  if (typeof crypto === 'undefined' || !crypto.subtle) {
    window.CortexE2EE = { available: false };
    return;
  }
  ...
  function getOrCreateSalt() {
    let s; try { s = localStorage.getItem(SALT_KEY); } catch (e) {}
    if (s) return b64ToBuf(s);
    const fresh = randomBytes(16);
    try { localStorage.setItem(SALT_KEY, bufToB64(fresh)); } catch (e) {}
    return fresh;
  }
  ...
  async function unlock(passphrase) {
    ...
    let canary; try { canary = localStorage.getItem(CANARY_KEY); } catch (e) {}
    if (canary) {
      try {
        const env = JSON.parse(canary);
        const pt = await subtle.decrypt({ name: 'AES-GCM', iv: b64ToBuf(env.iv) }, key, b64ToBuf(env.ct));
        if (new TextDecoder().decode(pt) !== 'cortexbuild-pro-e2ee-v1') { return false; }
      } catch (e) { return false; }
    } else {
      // First time — write the canary under this passphrase
      const iv = randomBytes(12);
      const ct = await subtle.encrypt({ name: 'AES-GCM', iv }, key, new TextEncoder().encode('cortexbuild-pro-e2ee-v1'));
      try { localStorage.setItem(CANARY_KEY, JSON.stringify({ iv: bufToB64(iv), ct: bufToB64(ct) })); } catch (e) {}
    }
    cachedKey = key;
  ...
  async function encrypt(plain) {
    ...
    const iv = randomBytes(12);
    const enc = new TextEncoder();
    const ct = await subtle.encrypt({ name: 'AES-GCM', iv }, cachedKey, enc.encode(typeof plain === 'string' ? plain : JSON.stringify(plain)));
    return { v: 1, iv: bufToB64(iv), ct: bufToB64(ct), e: 1 };
  }
  ...
  async function decrypt(env) {
    ...
    const pt = await subtle.decrypt({ name: 'AES-GCM', iv: b64ToBuf(env.iv) }, cachedKey, b64ToBuf(env.ct));
  ...
  async function encryptRecords(records, sensitiveKeys) {
    ...
    for (const r of records) {
      const id = r.id, rev = r._rev;
      let payload;
      if (keys) {
        payload = {};
        for (const k of keys) if (k in r) payload[k] = r[k];
      } else {
        payload = Object.assign({}, r); delete payload.id; delete payload._rev;
      }
      const env = await encrypt(payload);
      out.push({ id, _rev: rev, _enc: env });
    }
    return out;
  }
    ...
    for (const r of records) {
      if (r && r._enc) {
        try {
          ...
          out.push(Object.assign({ id: r.id, _rev: r._rev }, pt));
        } catch (e) { out.push(r); } // can't decrypt — surface as-is
      } else out.push(r);
    }
    return out;
  }
  ...
  function forget() {
    try { localStorage.removeItem(SALT_KEY); localStorage.removeItem(ENABLED_KEY); localStorage.removeItem('cortexx_e2ee_canary'); } catch (e) {}
    cachedKey = null;
  }

  window.CortexE2EE = {
    available: true, isEnabled, isUnlocked, lock, unlock,
    encrypt, decrypt, encryptRecords, decryptRecords, forget,
    iterations: ITERS,
  };
})();
