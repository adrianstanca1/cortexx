...
(function () {
  ...
  const CATEGORIES = {
    death: { code: 'death', label: 'Work-related death', immediate: true, form: 'F2508' },
    specifiedInjury: { code: 'specifiedInjury', label: 'Specified injury (fracture, amputation, eye damage, crush, scalp, internal organ)', immediate: false, form: 'F2508' },
    over7day: { code: 'over7day', label: 'Over-7-day incapacitation', immediate: false, form: 'F2508' },
    disease: { code: 'disease', label: 'Reportable occupational disease (Schedule 8)', immediate: false, form: 'F2508A' },
    dangerousOccurrence: { code: 'dangerousOccurrence', label: 'Dangerous occurrence (Schedule 2)', immediate: false, form: 'F2508' },
    gasIncident: { code: 'gasIncident', label: 'Gas-related incident', immediate: true, form: 'F2508G' },
  };
  ...
  const SPECIFIED_INJURIES = [
    ...
    'Amputation',
    'Permanent loss of sight / reduction',
    'Crush injury → brain or internal organ damage',
    'Serious burn (>10% body, or to face, vital areas)',
    'Scalping requiring hospital treatment',
    'Loss of consciousness (head injury or asphyxia)',
    'Confined-space injury → loss of consciousness/hypothermia/heat-induced illness',
  ];
...
<body>
  ...
  ${section('Incident', [
    ...
    ['Postcode',        report.gridRef],
    ['Kind',            report.kind],
    ['Description',     report.description],
    ['Action taken',    report.actionTaken],
  ])}
  ${report.injuredPersonName ? section('Injured person', [
    ['Name',            report.injuredPersonName],
    ['Age',             report.injuredPersonAge],
    ['Sex',             report.injuredPersonSex],
    ['Role',            report.injuredPersonRole],
    ['Employment',      report.employmentStatus],
    ['Body part',       report.bodyPartAffected],
    ['Nature',          report.natureOfInjury],
    ['Days off',        report.daysOff],
  ]) : ''}
  ${section('Reporter', [
  ...
  ${section('Company', [
    ...
    ['Postcode', report.companyPostcode],
    ['Type',     report.companyType],
    ['Employees',report.companyEmployeeCount],
  ])}
