...
(function () {
  ...
  const PLANS = [
    { id: 'cbp_pro_monthly',  name: 'Pro · monthly',  price: 24, period: 'month', currency: 'GBP', apple: 'cbp.pro.monthly',  stripe: 'price_pro_monthly' },
    { id: 'cbp_pro_yearly',   name: 'Pro · yearly',   price: 220, period: 'year',  currency: 'GBP', apple: 'cbp.pro.yearly',   stripe: 'price_pro_yearly',  savePct: 24 },
    { id: 'cbp_team_monthly', name: 'Team · monthly', price: 80, period: 'month', currency: 'GBP', apple: 'cbp.team.monthly', stripe: 'price_team_monthly' },
  ];
  ...
  function saveStatus(s) {
    try { localStorage.setItem(ENTITLEMENT_KEY, JSON.stringify(s)); } catch (e) {}
    cachedStatus = s;
  }
  ...
  function status() {
    ...
    return cachedStatus || { entitled: false, plan: null, expires: null, source: 'none' };
  }
  ...
  if (isCapacitor()) {
    const init = () => {
      ...
      try {
        PLANS.forEach(p => store.register({ id: p.apple, type: store.PAID_SUBSCRIPTION, platform: store.APPLE_APPSTORE }));
        store.ready(() => {
          // On launch, ask Apple for current entitlement
          PLANS.forEach(p => {
            const prod = store.get(p.apple);
            if (prod && prod.owned) {
              verifyServer({ platform: 'ios', productId: p.apple, receipt: prod.transaction && prod.transaction.appStoreReceipt }).then(v => { if (v && v.entitled) saveStatus(v); });
            }
          });
        });
        store.initialize();
  ...
  window.CortexIAP = {
    plans: () => PLANS.slice(),
    subscribe, restore, status, cancel,
