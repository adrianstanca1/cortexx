...
(function () {
  ...
  const format = {
    currency: (n, opts) => {
      try {
        return new Intl.NumberFormat(currentLocale, {
          style: 'currency',
          currency: opts && opts.currency || 'GBP'
        }).format(n);
      } catch (e) {
    ...
    relative: d => {
      ...
      let value, unit;
      if (abs < 60000) {
        value = Math.round(abs / 1000) * sign;
        unit = 'second';
      } else if (abs < 3600000) {
        value = Math.round(abs / 60000) * sign;
        unit = 'minute';
      } else if (abs < day) {
        value = Math.round(abs / 3600000) * sign;
        unit = 'hour';
      } else if (abs < day * 30) {
        value = Math.round(abs / day) * sign;
        unit = 'day';
      } else if (abs < day * 365) {
        value = Math.round(abs / (day * 30)) * sign;
        unit = 'month';
      } else {
        value = Math.round(abs / (day * 365)) * sign;
        unit = 'year';
      }
      try {
        ...
        return Math.abs(value) + ' ' + unit + (Math.abs(value) === 1 ? '' : 's') + (sign < 0 ? ' ago' : '');
      }
    }
  };
