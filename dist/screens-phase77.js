    ...
    if (hasSpeech) {
      ...
      sr.onresult = event => {
        ...
        for (let i = event.resultIndex; i < event.results.length; i++) {
          ...
          if (r.isFinal) finalPiece += r[0].transcript;else interim += r[0].transcript;
        }
      ...
      try {
        ...
        setErrMsg('Speech recognition unavailable');
      }
    } else if (!hasMedia) {
      setStage('error');
...
function ReceiptScanSheetReal({
  ...
  const onFile = async e => {
    ...
    try {
      ...
      setData({
        ...parsed,
        category: parsed.category || aiCat.category,
        suggestedProjectId: aiCat.projectId
      });
...
function GlobalSearchSheet({
  ...
  const results = React.useMemo(() => {
    if (!norm) return [];
    const out = [];
    const push = (group, label, sub, action, icon, color) => out.push({
      group,
      label,
      sub,
      action,
  ...
  }, React.cloneElement(Ic.chevR, {
    size: 14
  }))))))));
}
Object.assign(window, {
  VoiceMemoSheetReal,
  ReceiptScanSheetReal,
  GlobalSearchSheet
});