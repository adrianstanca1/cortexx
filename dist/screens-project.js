...
function InboxScreen({
  ...
  const [portalMsgs, setPortalMsgs] = React.useState(() => {
    try {
      return JSON.parse(localStorage.getItem('cortexx_portal_messages') || '[]');
    } catch (e) {
      return [];
    }
  });
  ...
  const markPortalRead = id => {
    const next = portalMsgs.map(m => m.id === id ? {
      ...m,
      read: true
    } : m);
    setPortalMsgs(next);
    try {
      localStorage.setItem('cortexx_portal_messages', JSON.stringify(next));
    } catch (e) {}
  ...
  return React.createElement(ScreenBg, {
    ...
    onClick: () => {
      markPortalRead(m.id);
      if (window.cortexxNav) window.cortexxNav('clientmsgs');
    },
    style: {
