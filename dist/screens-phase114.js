...
(function () {
  ...
  const moneyK = n => {
    n = Number(n) || 0;
    return Math.abs(n) >= 1000 ? '£' + (n / 1000).toFixed(n % 1000 === 0 ? 0 : 0) + 'k' : '£' + Math.round(n);
  };
  const STAGES = [{
    k: 'identified',
    l: 'Identified',
  ...
  window.BidManagementScreen = function ({
    accent
  }) {
    ...
    const advance = async bid => {
      ...
      await Backend.db.bids.update(bid.id, {
        stage: next,
        submittedOn: next === 'submitted' ? new Date().toISOString().slice(0, 10) : bid.submittedOn
      });
    ...
    const Pipeline = () => React.createElement('div', null, STAGES.map(stage => {
      ...
      }, stage.l), React.createElement('span', {
        style: {
          fontSize: 11,
          color: T.t3
        }
      }, items.length + ' · ' + moneyK(items.reduce((a, b) => a + b.value, 0)))), items.map(b => React.createElement('div', {
        key: b.id,
      ...
      }, moneyK(b.value))), React.createElement('div', {
        style: {
          display: 'flex',
          justifyContent: 'space-between',
          alignItems: 'center'
      ...
      }, b.ref + ' · ' + b.client), !['won', 'lost'].includes(b.stage) && React.createElement('span', {
        style: {
          fontSize: 11,
          color: acc,
          fontWeight: 700
        }
      }, b.probability + '% likely')), !['won', 'lost'].includes(b.stage) && React.createElement('div', {
        style: {
          display: 'flex',
          gap: 6,
          marginTop: 10
    ...
    const Takeoff = () => {
      ...
      return React.createElement('div', null, submittedBids.length === 0 && React.createElement('p', {
      ...
      }, 'No takeoff measurements yet.'), submittedBids.map(b => {
        ...
        }, b.title), items.map(t => React.createElement('div', {
          key: t.id,
          style: {
            display: 'flex',
            justifyContent: 'space-between',
        ...
        }, t.item), React.createElement('div', {
          style: {
            fontSize: 11,
            color: T.t3
          }
        }, t.qty + ' ' + t.unit + ' × ' + money(t.rate))), React.createElement('span', {
          style: {
            fontSize: 13,
            fontWeight: 700,
            color: T.t1
          }
        }, money(t.total)))), React.createElement('div', {
          style: {
            display: 'flex',
            justifyContent: 'space-between',
            paddingTop: 10,
        ...
        }, 'Measured total'), React.createElement('span', {
          style: {
            fontSize: 16,
            fontWeight: 800,
            color: acc
    ...
    const RFQ = () => {
      const byPackage = {};
      rfqs.forEach(r => {
        (byPackage[r.package] = byPackage[r.package] || []).push(r);
      });
      return React.createElement('div', null, Object.keys(byPackage).length === 0 && React.createElement('p', {
        style: {
          color: T.t2,
          fontSize: 13,
          textAlign: 'center',
      ...
      }, 'No supplier quotes to compare yet.'), Object.entries(byPackage).map(([pkg, rows]) => {
        ...
        return React.createElement('div', {
        ...
        }, pkg), rows.sort((a, b) => a.price - b.price).map(r => React.createElement('div', {
          key: r.id,
          style: {
            display: 'flex',
            alignItems: 'center',
            ...
            borderRadius: 10,
            marginBottom: 6,
            background: r.selected ? 'rgba(16,185,129,0.08)' : T.bg2,
            border: '1px solid ' + (r.selected ? T.green : T.hair)
          }
        }, React.createElement('div', {
          style: {
            flex: 1
        ...
        }, r.supplier, r.price === cheapest && React.createElement('span', {
          style: {
            marginLeft: 6,
            fontSize: 10,
            color: T.green,
        ...
        }, 'LOWEST')), React.createElement('div', {
          style: {
            fontSize: 11,
            color: T.t3
          }
        }, 'Lead ' + r.lead + ' · ★ ' + r.rating)), React.createElement('span', {
          style: {
            fontSize: 14,
            fontWeight: 800,
            color: T.t1
          ...
          onClick: async () => {
            ...
            window.cortexxToast && window.cortexxToast('Selected ' + r.supplier, 'success');
          },
          style: {
            padding: '5px 10px',
            borderRadius: 7,
            background: r.selected ? T.green : 'transparent',
            border: '1px solid ' + (r.selected ? T.green : T.hair),
            color: r.selected ? '#fff' : T.t2,
            fontSize: 11,
            fontWeight: 700,
            cursor: 'pointer'
          }
    ...
    const Analytics = () => {
      ...
      return React.createElement('div', null, React.createElement('div', {
      ...
      }, winRate + '%'), React.createElement('div', {
        style: {
          fontSize: 11,
          color: T.t3
        }
      ...
      }, moneyK(pipelineValue)), React.createElement('div', {
        style: {
          fontSize: 11,
          color: T.t3
        }
      }, 'Weighted pipeline'))), React.createElement('div', {
        style: card({
          marginBottom: 12
        })
      }, React.createElement('div', {
      ...
      }, 'WON vs LOST VALUE'), React.createElement('div', {
        style: {
          display: 'flex',
          height: 24,
          borderRadius: 8,
        ...
        style: {
          width: wonVal / (wonVal + lostVal || 1) * 100 + '%',
          background: T.green,
          display: 'flex',
          alignItems: 'center',
          paddingLeft: 8
      ...
      }, moneyK(wonVal))), React.createElement('div', {
        style: {
          flex: 1,
          background: T.red,
          display: 'flex',
      ...
      }, moneyK(lostVal)))), React.createElement('div', {
        style: {
          display: 'flex',
          justifyContent: 'space-between',
          marginTop: 8
      ...
      }, lost.length + ' lost'))), lost.length > 0 && React.createElement('div', {
        style: card()
      }, React.createElement('div', {
        style: {
          fontSize: 12,
      ...
      }, 'LOSS REASONS'), lost.map(b => React.createElement('div', {
        key: b.id,
        style: {
          padding: '8px 0',
          borderBottom: '1px solid ' + T.hair
    ...
    }, '+ New bid'), openBid && React.createElement(BidDetail, {
      bid: openBid,
  ...
  function BidDetail({
    ...
    }, bid.ref)), React.createElement('div', {
      style: {
        padding: '0 20px'
      }
    }, React.createElement('div', {
    ...
    }, bid.title), React.createElement('span', {
      style: {
    ...
    }, stage.l), [['Client', bid.client], ['Value', money(bid.value)], ['Probability', bid.probability + '%'], ['Due', bid.due], ['Estimator', bid.estimator]].map(([k, v]) => React.createElement('div', {
      key: k,
      style: {
        display: 'flex',
        justifyContent: 'space-between',
