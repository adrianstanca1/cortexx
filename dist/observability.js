...
(function () {
  ...
  const counters = { fetch: 0, click: 0, nav: 0, error: 0, txSlow: 0, txTotal: 0 };
  const vitals = { ttfb: null, fcp: null, lcp: null, inp: null, longTasks: 0, cls: 0 };

  function nowISO() { return new Date().toISOString(); }
  function push(arr, item, cap) { arr.push(item); while (arr.length > cap) arr.shift(); }

  // ── Breadcrumbs ────────────────────────────────────────────────────
  function breadcrumb(category, message, data, level) {
    const b = { ts: Date.now(), category, message: String(message || '').slice(0, 200), data, level: level || 'info' };
    push(breadcrumbs, b, MAX_BREADCRUMBS);
    push(events, { ...b, kind: 'breadcrumb' }, MAX_EVENTS);
    return b;
  }
  ...
  function startSpan(name, meta) {
    ...
    activeSpans.set(id, { id, name, start: performance.now(), meta: meta || {} });
    counters.txTotal++;
    return id;
  }
  function finishSpan(id, extra) {
    ...
    const ev = { ts: Date.now(), kind: 'span', name: span.name, ms, slow: ms > SLOW_TX_MS, ...span.meta, ...(extra || {}) };
    push(events, ev, MAX_EVENTS);
    if (ev.slow) {
      counters.txSlow++;
      breadcrumb('span.slow', span.name + ' · ' + ms + 'ms', { name: span.name, ms }, 'warning');
    }
    return ev;
  }
  function span(name, fn, meta) {
    ...
    try {
      ...
      if (out && typeof out.then === 'function') {
        return out.then(
          v => { finishSpan(id, { ok: true }); return v; },
          e => { finishSpan(id, { ok: false, error: e && e.message }); throw e; }
        );
      }
      finishSpan(id, { ok: true });
      return out;
    } catch (e) {
      finishSpan(id, { ok: false, error: e.message });
      throw e;
    }
  }
  ...
  function instrumentClicks() {
    document.addEventListener('click', (e) => {
      ...
      breadcrumb('ui.click', label, { tag: t.tagName });
      counters.click++;
    }, true);
  }
  function instrumentNav() {
    const orig = history.pushState;
    history.pushState = function (...args) { breadcrumb('nav', args[2] || '(pushState)'); counters.nav++; return orig.apply(this, args); };
  ...
  function instrumentFetch() {
    ...
    window.fetch = async function (input, init) {
      ...
      try {
        ...
        breadcrumb('http', (init && init.method ? init.method : 'GET') + ' ' + url + ' → ' + r.status, { status: r.status, ms });
        return r;
      } catch (e) {
        const ms = Math.round(performance.now() - t0);
        breadcrumb('http', (init && init.method ? init.method : 'GET') + ' ' + url + ' → FAIL', { error: e.message, ms }, 'error');
        throw e;
      }
    };
  }
  function instrumentConsole() {
    ['warn', 'error'].forEach(level => {
      ...
      console[level] = function (...args) {
        try { breadcrumb('console.' + level, args.map(a => typeof a === 'string' ? a : JSON.stringify(a)).join(' ').slice(0, 200), null, level); } catch (e) {}
  ...
  function instrumentErrors() {
    window.addEventListener('error', (e) => {
      counters.error++;
      breadcrumb('error', (e && e.message) || 'window.error', { file: e.filename, line: e.lineno }, 'error');
    });
  ...
  function captureVitals() {
    ...
    try {
      ...
      const po = new PerformanceObserver((list) => {
        ...
        if (e) { last = Math.round(e.startTime); vitals.lcp = last; }
      });
    ...
    try {
      const po = new PerformanceObserver((list) => {
        for (const entry of list.getEntries()) {
          vitals.longTasks++;
          breadcrumb('perf.longtask', Math.round(entry.duration) + 'ms', { duration: entry.duration }, 'warning');
        }
      });
  ...
  // ── Public API ────────────────────────────────────────────────────
  window.CortexObs = {
    breadcrumb, span, startSpan, finishSpan,
    counters: () => ({ ...counters }),
    vitals: () => ({ ...vitals }),
    recent: (n) => events.slice(-(n || 50)),
    crumbs: (n) => breadcrumbs.slice(-(n || 50)),
    clear: () => { breadcrumbs.length = 0; events.length = 0; },
  };
  ...
  function init() {
    ...
    pollMemory();
    wireCrashEnrichment();
    breadcrumb('app', 'CortexObs initialised', { build: window.__cortexxVersion || '1.0.0' });
  }
