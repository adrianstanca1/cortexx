...
(function () {
  ...
  const persist = () => {
    try {
      localStorage.setItem(KEY, JSON.stringify(Backend.db.snapshot()));
    } catch (e) {}
  ...
  const makeCorrect = name => ({
    ...
    create: async data => {
      ...
      if (window.cortexxCloud) {
        try {
          window.cortexxCloud.push(name, 'create', id, item);
        } catch (e) {}
      }
    ...
    remove: async id => {
      ...
      if (window.cortexxCloud) {
        try {
          window.cortexxCloud.push(name, 'delete', id);
        } catch (e) {}
      }
    }
  });
  ...
  const NON_TABLE = new Set(['snapshot', 'subscribe', 'user', 'settings', 'table', 'mergeRemote', 'pullRemote', 'reset', 'export', 'import', 'seed']);
  let repaired = 0,
    checked = 0;
