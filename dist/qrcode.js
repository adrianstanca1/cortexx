...
(function () {
  ...
  function encodeData(text, v) {
    ...
    let pi = 0;
    while (cw.length < dataCodewords(v)) {
      cw.push(pads[pi++ % 2]);
    }
    return cw;
  }
  ...
  function buildCodewords(text, v) {
    ...
    const blocks = [],
      ecBlocks = [];
    for (let i = 0; i < nb; i++) {
      const blk = cw.slice(i * per, (i + 1) * per);
      blocks.push(blk);
  ...
  function buildMatrix(text) {
    ...
    const m = Array.from({
      length: n
    }, () => new Array(n).fill(null)); // null = unset
    ...
    const penalty = mat => {
      ...
      for (let r = 0; r < n - 1; r++) for (let c = 0; c < n - 1; c++) {
        const x = mat[r][c];
        if (x === mat[r][c + 1] && x === mat[r + 1][c] && x === mat[r + 1][c + 1]) p += 3;
      }
      let dark = 0;
  ...
  function toCanvas(text, opts = {}) {
    ...
    if (!mat) return null;
    const n = mat.length,
      q = opts.quiet != null ? opts.quiet : 4,
      scale = opts.scale || 6;
