...
(function () {
  ...
  const SEED = {
    ...
    allocations: [{
      ...
      id: 6,
      resourceType: 'crew',
      resourceName: '1st-fix carpenters',
      resourceId: 'C3',
      projectId: 4,
      start: addDays(1),
      end: addDays(7),
    ...
    bids: [{
      id: 1,
      ref: 'BID-3041',
      title: 'Hackney school refurbishment',
      client: 'LB Hackney',
      value: 480000,
      stage: 'submitted',
      due: addDays(9),
      probability: 55,
      submittedOn: addDays(-3),
      estimator: 'You',
      notes: 'Two-stage tender, ITT received.'
    }, {
      id: 2,
      ref: 'BID-3042',
      title: 'Riverside apartments — fit-out',
      ...
      submittedOn: null,
      estimator: 'You',
      notes: 'PQQ passed, awaiting ITT.'
    }, {
      id: 3,
      ref: 'BID-3043',
      title: 'Office Cat-B fit-out, Shoreditch',
      client: 'Apex Capital',
      value: 340000,
      stage: 'won',
      due: addDays(-10),
      probability: 100,
      submittedOn: addDays(-25),
      estimator: 'You',
      notes: 'Awarded — convert to project.'
    }, {
      id: 4,
      ref: 'BID-3044',
      title: 'Care home extension, Enfield',
      client: 'Sunrise Care Group',
      value: 720000,
      stage: 'lost',
      due: addDays(-18),
      probability: 0,
      submittedOn: addDays(-30),
      estimator: 'You',
      notes: 'Lost on price — 6% above winner.'
    }, {
      id: 5,
      ref: 'BID-3045',
      title: 'Retail unit strip-out & CAT-A',
      client: 'Brookfield AM',
      value: 210000,
      stage: 'identified',
      due: addDays(28),
      probability: 20,
    ...
    rfqs: [{
      id: 1,
      bidId: 1,
      package: 'M&E sub-package',
      supplier: 'Volt Building Services',
      price: 92000,
      lead: '6 wks',
      rating: 4.5,
      selected: true
    ...
    handoverItems: [{
      ...
      title: 'Electrical EIC / NICEIC',
      status: 'received',
      responsible: 'Crown Elec',
      due: addDays(5)
    }, {
      id: 4,
      projectId: 1,
      category: 'Test cert',
      title: 'Pressure test certificates',
      status: 'in-review',
      responsible: 'Volt BS',
      due: addDays(7)
    }, {
      ...
      projectId: 2,
      category: 'Warranty',
      title: 'Roofing 20yr warranty',
      status: 'outstanding',
      responsible: 'Apex Roofing',
      due: addDays(20)
    }, {
      id: 6,
      projectId: 1,
      category: 'H&S File',
      title: 'CDM Health & Safety file',
      status: 'in-review',
      responsible: 'PD',
      due: addDays(12)
    }],
    ...
    satisfaction: [{
      id: 1,
      projectId: 1,
      client: 'Oakwood Developments',
      score: 9,
      surveyedOn: addDays(-7),
      comment: 'Excellent communication, minor delay handled well.'
    }, {
      id: 2,
      projectId: 2,
      client: 'Meridian Homes',
      score: 7,
      surveyedOn: addDays(-14),
      comment: 'Good progress, would like more frequent photo updates.'
    }, {
      id: 3,
      projectId: 4,
      client: 'B. Khoury',
      score: 10,
      surveyedOn: addDays(-3),
      comment: 'Outstanding work, will recommend.'
    }],
    progressFeed: [{
      id: 1,
      projectId: 1,
      when: addDays(0),
      title: 'Foundations poured — Block A',
      body: 'Concrete pour completed and signed off by BCO.',
      photo: null,
      clientVisible: true
    }, {
      id: 2,
      projectId: 1,
      when: addDays(-1),
      title: 'Steel frame delivered',
      body: 'Primary steel delivered to site, offload complete.',
      photo: null,
      clientVisible: true
    }, {
      id: 3,
      projectId: 2,
      when: addDays(-2),
      title: 'First-floor slab complete',
      body: 'Slab cured, propping to remain 7 days.',
      photo: null,
      clientVisible: true
    }]
  };
  ...
  C.cashflowForecast = (weeks = 8) => {
    ...
    for (let w = 0; w < weeks; w++) {
      ...
      wkStart.setDate(today.getDate() + w * 7);
      const wkEnd = new Date(wkStart);
      wkEnd.setDate(wkStart.getDate() + 7);
      const inflow = invoices.filter(i => i.due && new Date(i.due) >= wkStart && new Date(i.due) < wkEnd && i.status !== 'paid').reduce((a, i) => a + num(i.amount), 0);
      const outflow = pos.filter(p => p.deliveryDate && new Date(p.deliveryDate) >= wkStart && new Date(p.deliveryDate) < wkEnd).reduce((a, p) => a + num(p.total), 0);
      out.push({
        week: w,
        label: 'Wk ' + (w + 1),
        inflow,
  ...
  C.projectPnL = projectId => {
    ...
    const labour = (s.timesheets || []).filter(t => t.projectId == projectId).reduce((a, t) => a + (num(t.hours) || num(t.mon) + num(t.tue) + num(t.wed) + num(t.thu) + num(t.fri) + num(t.sat) + num(t.sun)) * num(t.rate || 22), 0);
    ...
    return {
      ...
      cost,
      labour,
      materials: receipts + po,
      profit: inv - cost,
      marginPct: inv ? Math.round((inv - cost) / inv * 100) : 0
    };
  };
  C.marginAlerts = (threshold = 15) => {
    ...
    return (s.projects || []).map(p => ({
      project: p,
      pnl: C.projectPnL(p.id)
    })).filter(x => x.pnl.revenue > 0 && x.pnl.marginPct < threshold).sort((a, b) => a.pnl.marginPct - b.pnl.marginPct);
  };
  C.wipValue = () => {
    const s = B.db.snapshot();
  ...
  C.bidPipelineValue = () => (B.db.snapshot().bids || []).filter(b => !['won', 'lost'].includes(b.stage)).reduce((a, b) => a + num(b.value) * (num(b.probability) / 100), 0);
  C.winRate = () => {
