...
(function () {
  ...
  Backend.brain = {
    snapshot() {
      ...
      return {
        company: {
          name: s.user?.company || 'CortexBuild Ltd',
          plan: s.user?.plan,
          score: s.user?.safetyScore
        },
        money: {
          cash: c.cashBalance(),
...
function VeraScreen({
  ...
  const runEstimate = async () => {
    ...
    const results = await Backend.vera.autoEstimateNewLeads();
    if (results && results.length) {
      const total = results.reduce((s, r) => s + (r.est?.total || 0), 0);
      toast(`Vera drafted ${results.length} quote${results.length === 1 ? '' : 's'} · £${(total / 1000).toFixed(0)}k pipeline`, 'success');
    } else {
      toast('No new leads to quote right now', 'info');
    }
    setLoading(l => ({
      ...l,
      estimate: false
  ...
  return /*#__PURE__*/React.createElement(ScreenBg, {
    ...
    style: {
      flex: 1,
      background: j === 0 ? accent : 'transparent',
      color: j === 0 ? '#fff' : T.t1,
