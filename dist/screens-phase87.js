function DataExportScreen({
  ...
  const sources = [{
    ...
    k: 'photos',
    l: 'Photos',
    d: 'Metadata only (no blobs)',
    i: Ic.camera,
    c: T.cyan
  }];
  const collect = () => {
    const out = {
      workspace: tenant ? tenant.name : 'CortexBuild Pro',
  ...
  return React.createElement(ScreenBg, {
    ...
    style: {
      background: T.bg2,
      border: `0.5px solid ${scope[s.k] ? s.c : T.hair}`,
      borderRadius: 12,
      padding: 12,
      cursor: 'pointer',
      display: 'flex',
    ...
    style: {
      ...
      height: 22,
      borderRadius: 6,
      border: `1.5px solid ${scope[s.k] ? s.c : T.hairMid}`,
      background: scope[s.k] ? s.c : 'transparent',
      display: 'flex',
      alignItems: 'center',
      justifyContent: 'center'
    }
...
function OnboardWizard({
  ...
  const steps = [{
    t: 'Company name',
    d: 'What should we call your workspace?'
  }, {
    t: 'Your trade',
    d: 'We tailor templates to your sector'
  }, {
    t: 'Team size',
    d: 'Helps us pick the right plan'
  }, {
    t: 'Brand colour',
    d: 'Personalise your CortexBuild Pro'
  }];
  const finish = () => {
    try {
  ...
  return React.createElement(Sheet, {
    ...
    onClick: () => set('trade', t),
    style: {
      background: data.trade === t ? `${accent}14` : T.bg2,
      border: `0.5px solid ${data.trade === t ? accent : T.hair}`,
      borderRadius: 12,
      padding: '16px 12px',
      cursor: 'pointer',
      fontFamily: SF,
    ...
    onClick: () => set('size', s),
    style: {
      background: data.size === s ? `${accent}14` : T.bg2,
      border: `0.5px solid ${data.size === s ? accent : T.hair}`,
      borderRadius: 12,
      padding: '16px',
      cursor: 'pointer',
      fontFamily: SF,
    ...
    style: {
      ...
      borderRadius: 26,
      background: c,
      border: data.color === c ? '3px solid #fff' : '3px solid transparent',
    ...
    style: {
      flex: 1,
      background: canNext ? accent : T.bg3,
      color: '#fff',
      border: 'none',
      borderRadius: 12,
      padding: '15px',
