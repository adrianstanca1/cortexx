...
(function () {
  ...
  if (!c.dueInvoices) {
    c.dueInvoices = () => (Backend.db.snapshot().invoices || []).filter(i => ['due', 'overdue'].includes(i.status)).length;
  }
  if (!c.failedInspectionCount) {
    c.failedInspectionCount = () => (Backend.db.snapshot().inspections || []).filter(i => i.status === 'failed').length;
  }
  if (!c.openIncidents) {
...
Project: ${rfi.projectName || ''}`;
    try {
      return (await Backend.ai.ask('', {
        system: prompt
      })).trim();
    } catch (e) {
      return 'Acknowledged — reviewing and will respond shortly.';
    }
  };
})();
...
function AddInspectionSheet({
  ...
  return /*#__PURE__*/React.createElement(FormSheet, {
    ...
    options: [{
      v: Backend.db.user?.getSync?.()?.name?.split(' ')[0] || 'You',
      l: 'You'
    }, ...team.map(t => ({
      v: t.n.split(' ')[0],
      l: t.n
