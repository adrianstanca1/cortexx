...
(function() {
  ...
  if (!snap.veraActions) {
    snap.veraActions = [
      { id: 1, when: '2026-05-22T06:30', action: 'Drafted Camden chase email', impact: '£8,420', status: 'pending-review' },
      { id: 2, when: '2026-05-22T05:00', action: 'Flagged margin slip on Brixton', impact: '−1.2%', status: 'completed' },
      { id: 3, when: '2026-05-21T22:00', action: 'Generated 2 leads from local data', impact: '£24k pipeline', status: 'completed' },
      { id: 4, when: '2026-05-21T16:00', action: 'Suggested crew reallocation', impact: '+3 days saved', status: 'pending-review' },
    ];
  ...
  const mk = (n) => ({
    ...
    create: async (d) => {
      ...
      s[n] = [{...d, id: Math.max(0, ...ids) + 1}, ...s[n]].slice(0, 100);
  ...
  Backend.vera.autoOrderMaterials = async () => {
    ...
    return { items: lowStock, forecast };
  };
...
function VeraActionsScreen({ accent }) {
  ...
  const run = async (kind, label) => {
    setRunning(kind);
    try {
      if (kind === 'leads') await Backend.vera.autoHuntLeads(3);
      else if (kind === 'chase') await Backend.vera.autoChaseOverdue();
      else if (kind === 'health') await Backend.vera.autoHealthCheck();
      else if (kind === 'materials') await Backend.vera.autoOrderMaterials();
      else if (kind === 'all') await Backend.vera.runAll();
      toast(`Vera ran: ${label}`, 'ai');
    } catch (e) { toast('Vera hit an issue — see log', 'error'); }
    setRunning(null);
  };
  ...
  return (
    <ScreenBg accent={accent}>
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
        <MobileHeader title="Vera · autopilot" subtitle="Autonomous actions on your business"/>
        ...
        <div style={{ padding: '4px 16px 14px' }}>
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              ...
              <span style={{ fontFamily: SFMono, fontSize: 11, color: T.green, fontWeight: 700 }}>AUTOPILOT ACTIVE</span>
            </div>
            ...
            }}>{React.cloneElement(Ic.spark, { size: 16 })} {running === 'all' ? 'Vera working…' : 'Run full sweep now'}</button>
          </div>
        </div>

        {/* Individual actions */}
        <Section title="◆ Individual jobs">
          <GroupedList>
            {[
              { k: 'leads',     l: 'Hunt new leads',         d: '3 AI-generated qualified leads',         i: Ic.trend },
              { k: 'chase',     l: 'Chase overdue invoices', d: 'Draft polite chase emails',              i: Ic.mail },
              { k: 'health',    l: 'Health-check projects',  d: 'AI risk scoring on every active job',    i: Ic.shield },
        ...
        {/* Action log */}
        <Section title="◆ Vera's action log">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {actions.length === 0 ? (
              <div style={{ padding: 20, textAlign: 'center', fontFamily: SF, fontSize: 13, color: T.t3 }}>No actions yet · trigger one above</div>
            ) : actions.map(a => {
              ...
              return (
                <div key={a.id} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: SF, fontSize: 13, color: T.t1, fontWeight: 600 }}>{a.action}</div>
                    <div style={{ fontFamily: SFMono, fontSize: 10, color: T.t3, marginTop: 2 }}>
                      {new Date(a.when).toLocaleString('en-GB', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <Pill c={sc} size="xs">{a.status}</Pill>
                    <div style={{ fontFamily: SFMono, fontSize: 11, color: T.t1, fontWeight: 700, marginTop: 4 }}>{a.impact}</div>
                  </div>
                </div>
              );
            })}
        ...
        {/* Schedule */}
        <Section title="◆ Scheduled runs">
          <GroupedList>
            <Row icon={Ic.sun} iconBg={T.amber} title="Morning briefing" sub="Every weekday 06:30"
              right={<Pill c={T.green} size="xs">ON</Pill>}/>
            <Row icon={Ic.trend} iconBg={T.cyan} title="Lead hunt" sub="Every Mon/Thu 09:00"
              right={<Pill c={T.green} size="xs">ON</Pill>}/>
            <Row icon={Ic.mail} iconBg={T.red} title="Overdue chase sweep" sub="Daily 16:00"
              right={<Pill c={T.green} size="xs">ON</Pill>}/>
            <Row icon={Ic.shield} iconBg={T.blue} title="Project health check" sub="Every Friday 14:00"
              right={<Pill c={T.green} size="xs">ON</Pill>} isLast/>
          </GroupedList>
        </Section>
...
Object.assign(window, { VeraActionsScreen });
