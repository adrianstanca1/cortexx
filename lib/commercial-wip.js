...
function commercialSummary(input) {
  ...
  const costControl = input.costControl || null
  ...
  const committedPOs = costControl ? money(costControl.committedNet) : sum(purchaseOrders.filter(po => !['draft', 'cancelled'].includes(po.status)), 'subtotal')
  ...
  const recordedCost = costControl ? money(costControl.actualNet) : money(project.spent)
  const openCommitments = costControl
    ? money(costControl.openCommitments)
    : money(Math.max(0, committedPOs - recordedCost) + Math.max(0, approvedSubcontract - paidSubcontract))
  const uncodedCost = costControl ? money(costControl.uncodedNet) : 0
  const costCodingPct = costControl ? money(costControl.codedPct) : (recordedCost > 0 ? 0 : 100)
  ...
  return {
    ...
    paidSubcontract, recordedCost, uncodedCost, costCodingPct, openCommitments, forecastCost, earnedValue,
