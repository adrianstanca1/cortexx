...
// ═══════════════════════════════════════════════════════════════════
// CHECKOUT — real Stripe payment intent
// ═══════════════════════════════════════════════════════════════════
function CheckoutSheet({ accent, plan, price, onClose }) {
  ...
  React.useEffect(() => {
    fetch('/api/iap/create-payment-intent', {
      ...
      body: JSON.stringify({ plan, price: parseFloat(price.replace(/[^0-9.]/g, '')) })
    })
      .then(r => r.json())
      .then(data => {
  ...
  const pay = async () => {
    ...
    try {
      const res = await fetch('/api/iap/confirm-payment', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ clientSecret, plan })
      });
  ...
  return (
    <Sheet onClose={onClose}>
      <div style={{ padding: '20px 20px 30px' }}>
        ...
        {!clientSecret && !error && (
          <div style={{ textAlign: 'center', padding: 20, color: T.t3, fontFamily: SF, fontSize: 14 }}>Loading secure payment form…</div>
        )}
