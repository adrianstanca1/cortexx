      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
        ...
        <div style={{ padding: '4px 16px 14px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <button onClick={() => window.cortexxNav('clock')} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ width: 8, height: 8, borderRadius: 4, background: onSite ? T.green : T.t3 }}/>
              <span style={{ fontFamily: SF, fontSize: 10, color: onSite ? T.green : T.t3, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 }}>{onSite ? 'On site' : 'Off site'}</span>
            </div>
          ...
          <button onClick={() => window.cortexxNav('inbox')} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <span style={{ color: unreadMsg + openRFIs > 0 ? T.amber : T.t3 }}>{React.cloneElement(Ic.bell, { size: 12 })}</span>
              <span style={{ fontFamily: SF, fontSize: 10, color: unreadMsg + openRFIs > 0 ? T.amber : T.t3, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 }}>Inbox</span>
            </div>
            <div style={{ fontFamily: SF, fontSize: 13, color: T.t1, fontWeight: 600, marginTop: 6 }}>{unreadMsg + openRFIs} unread</div>
          </button>
        </div>

        {/* My tasks */}
        <Section title={`Your queue · ${myTasks.length}`}>
          <GroupedList>
            {myTasks.length === 0 && <div style={{ padding: 20, textAlign: 'center', fontFamily: SF, fontSize: 13, color: T.t3 }}>Nothing on your plate</div>}
            {myTasks.map((t, i, a) => (
              <Row key={t.id}
                icon={Ic.tasks} iconBg={PRIO_C[t.prio]}
                title={t.t}
                sub={formatTaskWhen(t.due)}
                right={<Pill c={PRIO_C[t.prio]} size="xs">{t.prio}</Pill>}
                isLast={i === a.length - 1}
                onClick={() => Backend.db.tasks.update(t.id, { done: !t.done })}/>
            ))}
          </GroupedList>
        </Section>
        ...
        <Section title="Quick capture">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
            {[
              { k: 'task',    l: 'Task',     i: Ic.tasks,   c: T.purple },
              ...
              <button key={o.k} onClick={() => window.cortexxNav(o.k === 'task' ? 'addtask' : o.k === 'voice' ? 'voice' : 'scan')} style={{
                ...
                <div style={{ width: 32, height: 32, borderRadius: 8, background: `${o.c}22`, color: o.c, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {React.cloneElement(o.i, { size: 17 })}
                </div>
                <span style={{ fontFamily: SF, fontSize: 11, fontWeight: 600, color: T.t1 }}>{o.l}</span>
              </button>
            ))}
          </div>
        </Section>
...
// WORKSPACE SWITCHER
// ═══════════════════════════════════════════════════════════════════
function WorkspaceSheet({ onClose, accent }) {
  const user = useDB('user');
  const ws = [
    { name: 'CortexBuild Ltd', sub: 'Director · 7 staff · Pro', current: true, c: accent },
    { name: 'Camden Joinery',  sub: 'Co-owner · 2 staff · Free', current: false, c: T.amber },
    { name: 'Hackney Refurb',  sub: 'Contractor · 4 staff · Free', current: false, c: T.green },
  ];
  return (
    <Sheet onClose={onClose} height="auto">
      <div style={{ padding: '8px 20px 14px', textAlign: 'center', fontFamily: SF, fontSize: 17, fontWeight: 600, color: T.t1 }}>Switch workspace</div>
      <div style={{ padding: '0 16px 28px' }}>
        <GroupedList>
          {ws.map((w, i, a) => (
            <Row key={w.name}
              icon={Ic.briefcase} iconBg={w.c}
              title={w.name} sub={w.sub}
              right={w.current ? <Pill c={T.green} size="xs">CURRENT</Pill> : null}
              isLast={i === a.length - 1}
        ...
        }}>{React.cloneElement(Ic.plus, { size: 14 })} Create new workspace</button>
      </div>
    </Sheet>
  );
}

Object.assign(window, { MyDayScreen, WorkspaceSheet });
