...
const Backend = (function() {
  ...
  // Separate cached company data on shared devices. A new cloud session
  // starts empty rather than inheriting another company's records or demo data.
  function activateScope(scope) {
    const key = scope ? 'cortexx_db_v1:' + scope : 'cortexx_db_v1';
    ...
    else if (!scope) state = JSON.parse(JSON.stringify(SEED));
    else {
      state = Object.fromEntries(Object.keys(state).map(k => [k, Array.isArray(state[k]) ? [] : {}]));
      state.settings = JSON.parse(JSON.stringify(SEED.settings));
    }
    persist();
  }
  ...
  const db = {
    ...
    user: {
      ...
      updateSync: (patch) => { state.user = { ...state.user, ...patch }; persist(); return state.user; },
    },
    settings: {
    ...
    mergeRemote: (collections, options = {}) => {
      ...
      let touched = false;
      const names = options.fullSnapshot ? Object.keys(state).filter(k => Array.isArray(state[k])) : Object.keys(collections);
      names.forEach(remoteName => {
        const local = MAP[remoteName] || remoteName;
        if (!Array.isArray(state[local])) return;   // only merge known collections
        const incoming = collections[remoteName] || (options.fullSnapshot ? [] : null);
        if (!Array.isArray(incoming)) return;
        const byId = new Map((options.fullSnapshot ? [] : state[local]).map(x => [String(x.id), x]));
        ...
        for (const o of options.pending || []) {
          if ((MAP[o.collection] || o.collection) !== local) continue;
          if (o.op === 'delete') byId.delete(String(o.id));
          else if (o.data) byId.set(String(o.id), { ...o.data, id: o.id });
        }
