...
(function() {
  ...
  const origPersist = () => {
    try { localStorage.setItem('cortexx_db_v1', JSON.stringify(Backend.db.snapshot())); } catch (e) {}
  };
  const debouncedPersist = () => {
    if (pendingWrite) clearTimeout(pendingWrite);
    pendingWrite = setTimeout(() => { origPersist(); pendingWrite = null; }, 100);
  };
  // Expose for tables that want to force-flush
  Backend.flush = () => { if (pendingWrite) { clearTimeout(pendingWrite); origPersist(); pendingWrite = null; } };
  window.addEventListener('beforeunload', Backend.flush);
  window.addEventListener('visibilitychange', () => { if (document.visibilityState === 'hidden') Backend.flush(); });
  ...
  Backend.db.subscribe = (fn) => {
    const wrapped = (state) => { cacheVersion++; computedCache = new WeakMap(); fn(state); };
    return origSubscribe(wrapped);
  };
  Object.entries(Backend.computed).forEach(([k, fn]) => {
    let lastVer = -1, lastResult;
