...
(function() {
  ...
  if (_sd6) { try { localStorage.setItem('cortexx_db_v1', JSON.stringify(snap)); } catch (e) {} }
  ...
  const mk = (n) => ({
    ...
    create: async (d) => {
      ...
      const item = {...d, id: d.id ?? (Math.max(0, ...ids) + 1)};
      s[n] = [item, ...arr6(n)];
    ...
    update: async (id, p) => {
      const s = Backend.db.snapshot();
      s[n] = arr6(n).map(x => x.id == id ? {...x, ...p} : x);
      try { localStorage.setItem('cortexx_db_v1', JSON.stringify(s)); } catch (e) {}
      Backend.db.user.update({});
...
// ═══════════════════════════════════════════════════════════════════
// VOICE MEMO — real transcription via Web Speech API
// ═══════════════════════════════════════════════════════════════════
function VoiceMemoSheet({ onClose, accent }) {
  ...
  const save = async () => {
    ...
    await Backend.db.notes.create({ type: 'voice', text: transcript.trim(), createdAt: new Date().toISOString() });
      ...
      <div style={{ padding: '0 24px 30px', textAlign: 'center' }}>
        {!recording && !transcript && !transcribing && (
          <>
            ...
            {recorderError && (
              <div style={{ background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.3)', color: '#ef4444', borderRadius: 10, padding: '10px 14px', fontFamily: SF, fontSize: 13, marginBottom: 16 }}>
                {recorderError}
              </div>
            )}
