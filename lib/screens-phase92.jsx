...
(function () {
  ...
  function save(l) { try { localStorage.setItem(KEY, JSON.stringify(l.slice(0, 200))); } catch (e) {} }
  window.CortexPortalMsgs = {
    list() { return load(); },
    unreadCount() { return load().filter(m => !m.read).length; },
    markRead(id) { const l = load(); const m = l.find(x => x.id === id); if (m) { m.read = true; save(l); } return l; },
    markReplied(id) { const l = load(); const m = l.find(x => x.id === id); if (m) { m.read = true; m.replied = true; save(l); } return l; },
    remove(id) { save(load().filter(x => x.id !== id)); return load(); },
    // Dev/demo seed so the screen isn't empty on first open.
    seedIfEmpty() {
      if (load().length) return;
      save([
        { id: Date.now() - 3600e3, projectId: 1, project: 'Camden Mews Refurb', client: 'J. Patterson', text: 'The kitchen is looking great! Quick question — are we still on track for the worktop install next week?', when: new Date(Date.now() - 3600e3).toISOString(), read: false },
        { id: Date.now() - 9e7, projectId: 3, project: 'Brixton Shopfront', client: 'Tonic Café Ltd', text: 'Happy with the snag progress. Can you send the final invoice once the clean is done?', when: new Date(Date.now() - 9e7).toISOString(), read: true, replied: true },
      ]);
    },
  };
})();

function ClientMessagesScreen({ accent }) {
  ...
  React.useEffect(() => {
    ...
    const pull = async () => {
      ...
      const mapped = rows
        ...
        .map(r => ({ id: 'srv-' + r.id, projectId: r.project_id, project: r.project || 'Project', client: r.client,
          text: r.body, when: r.created_at, read: r.read, replied: r.replied, kind: r.kind, _server: true }));
      const fresh = mapped.filter(m => !seen.has(m.id));
      if (fresh.length) {
        const merged = [...fresh, ...local].sort((a, b) => new Date(b.when) - new Date(a.when));
        try { localStorage.setItem('cortexx_portal_messages', JSON.stringify(merged.slice(0, 200))); } catch (e) {}
        setMsgs(window.CortexPortalMsgs.list());
      }
    };
    pull();
    const onRemote = (e) => { if (e.detail && (e.detail.type === 'portal_message' || e.detail.type === 'portal_approval')) pull(); };
    window.addEventListener('cortexx-remote', onRemote);
    return () => { alive = false; window.removeEventListener('cortexx-remote', onRemote); };
  }, []);

  const open = (m) => { if (!m.read) { window.CortexPortalMsgs.markRead(m.id); refresh(); } setReply(m); setDraft(''); };
  const sendReply = () => {
    ...
    if (reply._server && window.cortexxCloud && window.cortexxCloud.status().authed) {
      ...
      try {
        fetch(`${api}/api/portal-inbox/${srvId}/reply`, {
          method: 'POST',
          headers: { 'content-type': 'application/json', authorization: 'Bearer ' + (localStorage.getItem('cortexx_token') || '') },
    ...
    setReply(null); setDraft(''); refresh();
  };
  const remove = (id) => { window.CortexPortalMsgs.remove(id); refresh(); };
  ...
  return (
    <ScreenBg accent={accent}>
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
        <MobileHeader title="Client messages" subtitle={(unread ? `${unread} unread · ` : '') + (cloudOn ? 'synced from cloud + portals' : 'from your client portals')} ws/>

        <div style={{ padding: '0 16px' }}>
          {msgs.length === 0 ? (
            <div style={{ padding: '40px 16px', textAlign: 'center', fontFamily: SF, fontSize: 13, color: T.t3 }}>
              No client messages yet. When a client sends a note from their portal, it lands here.
            </div>
          ) : (
              ...
              {msgs.map(m => (
                <div key={m.id} onClick={() => open(m)} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                    <Avatar name={m.client} size={32} c={accent}/>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontFamily: SF, fontSize: 14, fontWeight: 600, color: T.t1 }}>{m.client}</div>
                      <div style={{ fontFamily: SF, fontSize: 11, color: T.t3 }}>{m.project} · {fmtWhen(m.when)}</div>
                    </div>
                    {!m.read && <div style={{ width: 9, height: 9, borderRadius: 5, background: accent, flexShrink: 0 }}/>}
                    {m.kind === 'approval' && <Pill c={T.green} size="xs">APPROVED</Pill>}
                    {m.replied && <Pill c={T.green} size="xs">REPLIED</Pill>}
                  </div>
                  <div style={{ fontFamily: SF, fontSize: 13, color: T.t1, lineHeight: 1.45, opacity: 0.92 }}>{m.text}</div>
                </div>
              ))}
            </div>
          )}
      ...
      {reply && (
        <Sheet onClose={() => setReply(null)}>
          <div style={{ padding: '20px 20px 30px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
              <Avatar name={reply.client} size={40} c={accent}/>
              <div style={{ flex: 1 }}>
            ...
            }}>{React.cloneElement(Ic.send, { size: 15 })} Send reply</button>
          </div>
        </Sheet>
      )}
    </ScreenBg>
...
Object.assign(window, { ClientMessagesScreen });
