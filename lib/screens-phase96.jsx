...
(function () {
  ...
  const geo = () => new Promise((res) => {
    ...
    navigator.geolocation.getCurrentPosition(
      (p) => { clearTimeout(t); res({ lat: p.coords.latitude, lng: p.coords.longitude, acc: Math.round(p.coords.accuracy) }); },
      () => { clearTimeout(t); res(null); }, { enableHighAccuracy: true, timeout: 2500 });
  });
  ...
  B.nfc.isOnSite = (name = 'You') => {
    const e = (B.db.clockEntries.listSync() || [])
      .filter(x => x.name === name || x.userId === 0 && name === 'You')
  ...
  B.nfc.clock = async ({ projectId, name = 'You', userId = 0, action } = {}) => {
    ...
    await B.db.clockEntries.create({
      userId, name, projectId: projectId != null ? projectId : null,
      action: act, time: new Date().toISOString().slice(0, 16),
      method: 'nfc', gps, location: proj?.name || 'Site',
    });
    if (B.db.activity) await B.db.activity.create({
      who: name, what: act === 'in' ? 'checked in (NFC)' : 'checked out (NFC)',
      where: proj?.name || 'Site', when: new Date().toISOString().slice(0, 16),
      icon: 'pin', color: act === 'in' ? '#1f8a5b' : '#63748a',
    });
    if (window.CortexAudit) window.CortexAudit.log(name, `${act === 'in' ? 'checked in' : 'checked out'} via NFC at ${proj?.name || 'site'}`, 'Timesheets');
    return { action: act, project: proj?.name || 'Site' };
  };
  ...
  B.nfc.attendance = () => {
    const entries = (B.db.clockEntries.listSync() || []).slice()
      .sort((a, b) => String(a.time).localeCompare(String(b.time)));
    const byPerson = new Map();
    entries.forEach(e => {
      const key = e.name || ('user-' + e.userId);
      byPerson.set(key, { name: key, last: e, action: e.action, projectId: e.projectId, location: e.location, method: e.method, time: e.time });
    });
    const all = [...byPerson.values()];
    const onSite = all.filter(p => p.action === 'in' || p.action === 'break-in');
    return { onSite, all, count: onSite.length };
  };
})();
...
(function () {
  function handle() {
    ...
    setTimeout(() => {
      try { window.dispatchEvent(new CustomEvent('cortexx-nfc-checkin', { detail: { projectId: pid } })); } catch (e) {}
...
// ═══════════════════════════════════════════════════════════
function NfcCheckinConfirm({ projectId, onDone, accent }) {
  ...
  const confirm = async () => {
    setBusy(true);
    const r = await Backend.nfc.clock({ projectId, action: nextAction });
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.6)', zIndex: 4000, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={onDone}>
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 420, background: T.bg1, borderRadius: '20px 20px 0 0', padding: '26px 22px calc(26px + env(safe-area-inset-bottom))', boxShadow: '0 -8px 40px rgba(0,0,0,.5)' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 6 }}>
          ...
          {done ? (
            <>
              <div style={{ fontFamily: SF, fontSize: 19, fontWeight: 700, color: T.green }}>{done.action === 'in' ? '✓ Checked in' : '✓ Checked out'}</div>
              <div style={{ fontFamily: SF, fontSize: 14, color: T.t2 }}>{done.project} · {new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' })}</div>
            </>
          ) : (
            <>
...
Object.assign(window, { NfcCheckinConfirm });
