      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 150 }}>
        <MobileHeader
          ...
          right={<div style={{ display: 'flex', gap: 8 }}>
            <HeaderBtn icon={Ic.money} accent={accent} onClick={() => window.cortexxNav('payments')}/>
            <HeaderBtn icon={Ic.plus} accent={accent} onClick={() => window.cortexxNav('estimator')}/>
          </div>}
        />
    ...
    <ScreenBg accent={accent}>
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 150 }}>
        ...
        <div style={{ textAlign: 'center', fontFamily: SFMono, fontSize: 10, color: T.t3, padding: '14px 0' }}>
          CortexBuild Pro · v2.0 (build 247) · {user.company}
        </div>
      </div>
    </ScreenBg>
  );
    ...
    <Sheet onClose={onClose}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 16px 20px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
          ...
          }}>{React.cloneElement(Ic.download, { size: 15 })} PDF</button>
        </div>
      </div>
    </Sheet>
  );
