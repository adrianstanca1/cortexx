          ...
          <GroupedList>
            <div style={{ padding: '12px 14px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: SF, fontSize: 14, color: T.t1, fontWeight: 600 }}>On-device AI model</div>
                  <div style={{ fontFamily: SF, fontSize: 11, color: T.t2, marginTop: 1 }}>
                    {aiProgress || (navigator.gpu ? 'Llama 3.2 1B · runs fully offline' : 'Needs WebGPU — not available here')}
                  </div>
                </div>
                {!navigator.gpu ? (
                  <Pill c={T.t3} size="xs">N/A</Pill>
                ) : aiReady ? (
                  ...
                  <button onClick={async () => {
                    ...
                    try {
                      ...
                      if (st.webllm) { setAiReady(true); setAiProgress('Model ready · works offline'); toast('On-device AI ready', 'success'); }
                      else { setAiProgress('Couldn\'t load — cloud + local engine still active'); }
                    } catch (e) { setAiProgress('Load failed — fallback engine active'); }
                    setDownloading(false);
                  }} style={{
                    background: downloading ? T.bg3 : T.purple, color: '#fff', border: 'none',
                    borderRadius: 14, padding: '6px 12px', cursor: downloading ? 'default' : 'pointer',
                    fontFamily: SF, fontSize: 11, fontWeight: 700,
                  }}>{downloading ? '…' : 'Download'}</button>
                )}
              </div>
              <div style={{ marginTop: 10, paddingTop: 10, borderTop: `0.5px solid ${T.hair}`, fontFamily: SF, fontSize: 11, color: T.t2, lineHeight: 1.5 }}>
                Cortex always responds — even fully offline. It cascades: cloud AI → on-device model → live-data engine that never fails.
              </div>
            </div>
          </GroupedList>
        </Section>
