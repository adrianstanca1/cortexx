...
(function () {
  ...
  Backend.db.table = (name, opts = {}) => {
    ...
    return {
      ...
      create: async (d) => {
        const s = Backend.db.snapshot();
        if (!s[name]) s[name] = [];
        s[name] = [{ ...d, id: idGen(s[name]) }, ...s[name]];
      ...
      update: async (id, p) => {
        const s = Backend.db.snapshot();
        if (!s[name]) return;
        s[name] = s[name].map(x => x.id == id ? { ...x, ...p } : x);
        try { localStorage.setItem(KEY, JSON.stringify(s)); } catch (e) {}
        Backend.db.user.update({});
      },
      remove: async (id) => {
  ...
  if (!s.tags) {
    s.tags = [
      ...
      { id: 2, label: 'On hold',    color: '#f59e0b' },
      { id: 3, label: 'For review', color: '#8b5cf6' },
      { id: 4, label: 'Snagged',    color: '#06b6d4' },
    ];
  }
  if (!s.savedViews) {
    s.savedViews = [
      { id: 1, name: 'My overdue', icon: 'flag', filter: 'status:overdue assignee:me' },
      { id: 2, name: 'This week',  icon: 'clock', filter: 'due:this-week' },
    ];
  }
  if (!s.templates) {
    s.templates = [
      { id: 1, name: 'Kitchen renovation', kind: 'project', used: 12 },
      { id: 2, name: 'Loft conversion',    kind: 'project', used: 8 },
      { id: 3, name: 'Snag report',        kind: 'doc',     used: 22 },
    ];
  }
  if (!s.costItems) {
    s.costItems = [
      { id: 1, name: 'Plasterer',       cat: 'Labour',    unit: 'day', rate: 280 },
      { id: 2, name: 'Plasterboard 9mm', cat: 'Materials', unit: 'sheet', rate: 12 },
      { id: 3, name: 'Skip 6 yard',     cat: 'Plant',     unit: 'wk',  rate: 180 },
    ];
  }
  try { localStorage.setItem(KEY, JSON.stringify(s)); } catch (e) {}
...
// ─────────────────────────────────────────────────────────
function EditFieldSheet({ params, onClose, accent }) {
  const p = params || {};
  ...
  return (
    <FormSheet title={`Edit ${p.label || 'value'}`} onClose={onClose} accent={accent} onSave={save}>
      {p.kind === 'textarea' ? (
        <FormTextarea label={p.label || 'Value'} v={val} onChange={setVal} placeholder={p.placeholder || ''}/>
      ) : (
        <FormInput label={p.label || 'Value'} v={val} onChange={setVal} placeholder={p.placeholder || ''} type={p.type || 'text'}/>
      )}
      {p.help && <div style={{ fontFamily: SF, fontSize: 11.5, color: T.t3, padding: '0 2px' }}>{p.help}</div>}
    </FormSheet>
  );
}
...
// ─────────────────────────────────────────────────────────
function StartTripSheet({ onClose, accent }) {
  ...
  const begin = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => { setStart({ lat: pos.coords.latitude, lng: pos.coords.longitude }); setNow({ lat: pos.coords.latitude, lng: pos.coords.longitude }); },
      () => { toast('Location permission denied', 'error'); setTracking(false); },
      { enableHighAccuracy: true }
    );
    watchRef.current = navigator.geolocation.watchPosition(
      (pos) => setNow({ lat: pos.coords.latitude, lng: pos.coords.longitude }),
    ...
    <Sheet onClose={onClose} fullscreen>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px 16px 24px' }}>
        ...
        <div style={{
          ...
          <div style={{ fontFamily: SF, fontSize: 11, color: T.t3, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 8 }}>
            {tracking ? 'Tracking GPS · live' : 'Ready to track'}
          </div>
          <div style={{ fontFamily: SFMono, fontSize: 56, fontWeight: 700, color: tracking ? T.green : T.t1, letterSpacing: -2, lineHeight: 1 }}>
            {miles.toFixed(1)}<span style={{ fontSize: 22, color: T.t3 }}> mi</span>
          </div>
          <div style={{ fontFamily: SF, fontSize: 13, color: T.t2, marginTop: 8 }}>
            Reimbursement @ 45p: <span style={{ color: T.green, fontWeight: 600 }}>£{(miles * 0.45).toFixed(2)}</span>
          </div>
        </div>
        ...
        {!tracking && (
          <>
            ...
            <div style={{ marginTop: 14 }}>
              <FormSelect label="Purpose" v={purpose} onChange={setPurpose}
                options={[
                  { v: 'Site visit',       l: 'Site visit' },
                  { v: 'Client meeting',   l: 'Client meeting' },
                  { v: 'Material run',     l: 'Material run' },
                  { v: 'Inspection',       l: 'Inspection' },
                  { v: 'Other business',   l: 'Other business' },
                ]}/>
            </div>
          </>
        )}
        ...
        <button onClick={tracking ? stop : begin} style={{
          ...
          padding: '16px', fontFamily: SF, fontSize: 15, fontWeight: 700, cursor: 'pointer',
          boxShadow: `0 10px 26px ${tracking ? T.red : T.green}55`,
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
        }}>
          {tracking ? '◼  Stop & log trip' : '●  Start trip'}
        </button>
        {!tracking && (
          <div style={{ fontFamily: SF, fontSize: 11.5, color: T.t3, textAlign: 'center', marginTop: 10, lineHeight: 1.5 }}>
...
function AddTagSheet({ onClose, accent }) {
  const [label, setLabel] = React.useState('');
  const [color, setColor] = React.useState(TAG_COLORS[0]);
  const save = async () => {
    if (!label.trim()) { toast('Label required', 'error'); return; }
    await Backend.db.tags.create({ label: label.trim(), color });
    toast('Tag added', 'success');
    onClose();
  };
  return (
    <FormSheet title="New tag" onClose={onClose} accent={accent} onSave={save}>
      <FormInput label="Label" v={label} onChange={setLabel} placeholder="e.g. Phase-2"/>
      <div>
...
function AddTemplateSheet({ onClose, accent }) {
  const [name, setName] = React.useState('');
  const [kind, setKind] = React.useState('project');
  const save = async () => {
    if (!name.trim()) { toast('Name required', 'error'); return; }
    await Backend.db.templates.create({ name: name.trim(), kind, used: 0 });
    toast('Template added', 'success');
    onClose();
  };
  return (
    <FormSheet title="New template" onClose={onClose} accent={accent} onSave={save}>
      <FormInput label="Name" v={name} onChange={setName} placeholder="e.g. Bathroom fit-out"/>
      <FormSelect label="Kind" v={kind} onChange={setKind} options={[
        { v: 'project', l: 'Project template' },
        { v: 'doc',     l: 'Document template' },
        { v: 'task',    l: 'Task checklist' },
        { v: 'quote',   l: 'Quote template' },
      ]}/>
    </FormSheet>
  );
}

function AddViewSheet({ onClose, accent }) {
  const [name, setName] = React.useState('');
  const [filter, setFilter] = React.useState('');
  const save = async () => {
    if (!name.trim()) { toast('Name required', 'error'); return; }
    await Backend.db.savedViews.create({ name: name.trim(), filter: filter.trim() || '*', icon: 'flag' });
    toast('View saved', 'success');
    onClose();
  };
  return (
    <FormSheet title="Save current view" onClose={onClose} accent={accent} onSave={save}>
      <FormInput label="Name" v={name} onChange={setName} placeholder="e.g. Overdue invoices"/>
      <FormInput label="Filter" v={filter} onChange={setFilter} placeholder="e.g. status:overdue type:invoice"/>
    </FormSheet>
  );
}

function AddCostItemSheet({ onClose, accent }) {
  const [name, setName] = React.useState('');
  const [cat, setCat] = React.useState('Materials');
  const [unit, setUnit] = React.useState('each');
  const [rate, setRate] = React.useState('');
  const save = async () => {
    if (!name.trim() || !rate) { toast('Name and rate required', 'error'); return; }
    await Backend.db.costItems.create({ name: name.trim(), cat, unit, rate: parseFloat(rate) });
    toast('Cost item added', 'success');
    onClose();
  };
  return (
    <FormSheet title="New cost item" onClose={onClose} accent={accent} onSave={save}>
      <FormInput label="Name" v={name} onChange={setName} placeholder="e.g. MDF skirting"/>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        <FormSelect label="Category" v={cat} onChange={setCat} options={[
          { v: 'Materials', l: 'Materials' },
          { v: 'Labour',    l: 'Labour' },
          { v: 'Plant',     l: 'Plant' },
        ]}/>
        <FormInput label="Unit" v={unit} onChange={setUnit} placeholder="m / sheet / day"/>
      </div>
      <FormInput label="Rate (£)" type="number" v={rate} onChange={setRate} placeholder="12.50"/>
    </FormSheet>
  );
}

Object.assign(window, {
  EditFieldSheet, StartTripSheet,
  AddTagSheet, AddTemplateSheet, AddViewSheet, AddCostItemSheet,
});
