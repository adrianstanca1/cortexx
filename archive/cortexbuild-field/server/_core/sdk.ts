...
class SDKServer {
  ...
  async createSessionToken(
    ...
    const safeName = options.name?.trim() || openId;
  ...
  async authenticateRequest(req: Request): Promise<User> {
    // Two valid token shapes reach this dispatcher:
    //   - RS256 Supabase JWT (bearer or cookie) — verified against JWKS
    //   - HS256 local session cookie minted by sdk.createSessionToken
    //     (the auth.login / auth.requestPasswordReset path)
    // Dispatch on the JWT alg header so each path surfaces its own errors
    ...
    try {
      ...
      throw ForbiddenError("Malformed JWT");
    }
    ...
    throw ForbiddenError(`Unsupported JWT algorithm: ${alg ?? "unknown"}`);
  }
