...
**Gate C — Commercial/procurement backbone:** unify commercial ledger, complete procurement lifecycle, supplier scorecards, immutable audit events, approval matrices and accounting adapter boundary. The first procurement-control slice is implemented on the active feature line: tenant-local PO numbering, manager-gated approval/rejection, immutable issued POs, linked suppliers, auditable partial/full goods receipts, approved-only commitment accounting, persisted three-way match snapshots, and invoice approval/payment blocking with an explicitly audited financial-admin override. Requisition/RFQ/comparison, supplier-performance scoring and the external accounting adapter boundary remain.
