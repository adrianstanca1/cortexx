(function () {
  ...
  window.QualityHandoverScreen = function ({
    accent
  }) {
    ...
    const cycleStatus = async item => {
      ...
      await Backend.db.handoverItems.update(item.id, {
        status: next
      });
    };
    ...
    const Pack = () => React.createElement('div', null, React.createElement('div', {
      ...
      r: 48,
      fill: 'none',
      stroke: readiness === 100 ? T.green : acc,
      strokeWidth: 10,
      strokeLinecap: 'round',
    ...
    }, '📦 Issue handover pack')), items.length === 0 ? React.createElement('p', {
      style: {
        color: T.t2,
        fontSize: 13,
        textAlign: 'center',
    ...
    }, 'No handover items for this project yet.') : items.map(item => {
      ...
      }, CAT_ICON[item.category] || '📄'), React.createElement('div', {
        style: {
          flex: 1
        }
      }, React.createElement('div', {
      ...
      }, item.category + ' · ' + item.responsible + ' · due ' + item.due)), React.createElement('span', {
        style: {
          display: 'inline-flex',
          alignItems: 'center',
          gap: 4,
    ...
    const Snagging = () => {
      ...
      return React.createElement('div', null, React.createElement('div', {
      ...
      }, 'Snag completion'), React.createElement('span', {
        style: {
          fontSize: 14,
          fontWeight: 800,
          color: completion === 100 ? T.green : acc
        }
        ...
        style: {
          width: completion + '%',
          height: '100%',
          background: completion === 100 ? T.green : acc
        }
      })), React.createElement('div', {
        style: {
          fontSize: 11,
      ...
      }, closed.length + ' closed · ' + openSnags.length + ' open')), openSnags.length === 0 && closed.length > 0 && React.createElement('div', {
        style: {
          textAlign: 'center',
          padding: 30,
          color: T.green
      ...
      }, 'All snags closed — ready for completion')), openSnags.map(s => React.createElement('div', {
        key: s.id,
        style: card({
          marginBottom: 8,
          padding: 14
      ...
      }, s.title || s.desc || 'Snag'), React.createElement('span', {
        style: {
          fontSize: 10,
          fontWeight: 700,
          color: s.priority === 'high' ? T.red : s.priority === 'med' ? '#f59e0b' : T.t3
        }
      }, (s.priority || 'low').toUpperCase())), React.createElement('div', {
        style: {
          fontSize: 11,
          color: T.t3,
          marginBottom: 10
        ...
        onClick: async () => {
          ...
          window.cortexxToast && window.cortexxToast('Snag closed', 'success');
        },
        style: {
          width: '100%',
          padding: 9,
    ...
    }, React.createElement(ProjectPicker), React.createElement(TabBar), tab === 'pack' && React.createElement(Pack), tab === 'snagging' && React.createElement(Snagging));
  };
})();