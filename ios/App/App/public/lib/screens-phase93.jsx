...
(function () {
  ...
  const taxCode = {
    ...
    sage: { standard: 'T1', zero: 'T0', cis: 'T21' },           // T21 = CIS reverse charge (Sage)
  ...
  function lines(opts) {
    ...
    if (opts.sales) {
      (s.invoices || []).forEach(inv => {
        ...
        const { net, vat } = splitVat(inv.amount, opts.vat);
        out.push({ kind: 'sale', date: inv.issued, due: inv.due, ref: inv.id, contact: inv.client,
          desc: `${projName(inv.projectId) || 'Project'} — works`, net, vat, gross: inv.amount, status: inv.status, project: projName(inv.projectId) });
      });
    }
    if (opts.purchases) {
      (s.receipts || []).forEach(r => {
        if (!inRange(r.date)) return;
        const { net, vat } = splitVat(r.amount, opts.vat);
        out.push({ kind: 'purchase', date: r.date, due: r.date, ref: 'RCT-' + r.id, contact: r.vendor,
          desc: `${r.category || 'expense'}${projName(r.projectId) ? ' — ' + projName(r.projectId) : ''}`, net, vat, gross: r.amount, status: r.assigned ? 'assigned' : 'unassigned', project: projName(r.projectId) });
      });
    }
  ...
  function toCSV(format, opts) {
    ...
    return { csv: [row(header)].concat(body).join('\r\n'), count: L.length, lines: L };
  }
  ...
  function summary(opts) {
    ...
    return {
      count: L.length, salesCount: sales.length, purchCount: purch.length,
      salesNet: sum(sales, 'net'), salesVat: sum(sales, 'vat'), salesGross: sum(sales, 'gross'),
      purchNet: sum(purch, 'net'), purchVat: sum(purch, 'vat'), purchGross: sum(purch, 'gross'),
      vatDue: sum(sales, 'vat') - sum(purch, 'vat'),
    };
  }

  function download(format, opts) {
    const { csv, count } = toCSV(format, opts);
  ...
  window.CortexLedger = { toCSV, summary, download, lines };
})();

function LedgerExportScreen({ accent }) {
  ...
  const rangeBounds = React.useMemo(() => {
    ...
    if (range === 'month') return { from: new Date(now.getFullYear(), now.getMonth(), 1).toISOString().slice(0,10), to: null };
    if (range === 'quarter') { const q = Math.floor(now.getMonth()/3)*3; return { from: new Date(now.getFullYear(), q, 1).toISOString().slice(0,10), to: null }; }
    ...
    return { from: fyStart.toISOString().slice(0,10), to: null };
  }, [range]);

  const opts = { ...rangeBounds, vat, sales, purchases };
  ...
  const FORMATS = [
    { k: 'xero', l: 'Xero', d: 'Sales invoice import template' },
    { k: 'qb', l: 'QuickBooks', d: 'Transaction list with VAT' },
    { k: 'sage', l: 'Sage 50', d: 'Audit-trail import layout' },
    { k: 'generic', l: 'Generic', d: 'Plain columns, any tool' },
  ];
  const VATS = [
    { k: 'standard', l: 'Standard 20%' },
    { k: 'zero', l: 'Zero-rated' },
    { k: 'cis', l: 'CIS reverse charge' },
  ];
  const RANGES = [
  ...
  const doExport = () => {
    if (!sales && !purchases) { window.cortexxToast && window.cortexxToast('Select sales or purchases', 'error'); return; }
    if (sum.count === 0) { window.cortexxToast && window.cortexxToast('No records in this range', 'error'); return; }
    setBusy(true);
    setTimeout(() => {
  ...
  return (
    <ScreenBg accent={accent}>
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
        <MobileHeader title="Ledger export" subtitle="Accountant-ready CSV · Xero · QuickBooks · Sage"/>
        <div style={{ padding: '0 16px' }}>
            ...
            {FORMATS.map(f => (
              <button key={f.k} onClick={() => setFormat(f.k)} style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontFamily: SF, fontSize: 11, color: T.t2 }}>{f.d}</div>
                </div>
                <div style={{ width: 20, height: 20, borderRadius: 10, border: `1.5px solid ${format === f.k ? accent : T.hairMid}`, background: format === f.k ? accent : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {format === f.k && <div style={{ width: 8, height: 8, borderRadius: 4, background: '#fff' }}/>}
                </div>
              </button>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {[{ k: 'sales', on: sales, set: setSales, l: 'Sales invoices', c: sum.salesCount }, { k: 'purch', on: purchases, set: setPurchases, l: 'Purchase receipts', c: sum.purchCount }].map(x => (
              <button key={x.k} onClick={() => x.set(v => !v)} style={{
                ...
                <div style={{ fontFamily: SF, fontSize: 13, fontWeight: 600, color: x.on ? T.t1 : T.t2 }}>{x.l}</div>
                <div style={{ fontFamily: SFMono, fontSize: 11, color: x.on ? accent : T.t3, marginTop: 2 }}>{x.c} record{x.c === 1 ? '' : 's'}</div>
              </button>
            ))}
          </div>

          {/* VAT + Range */}
          <div style={{ fontFamily: SF, fontSize: 11, fontWeight: 700, color: T.t3, textTransform: 'uppercase', letterSpacing: 0.6, margin: '18px 2px 8px' }}>VAT treatment</div>
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 2 }}>
            {VATS.map(v => <Chip key={v.k} active={vat === v.k} onClick={() => setVat(v.k)}>{v.l}</Chip>)}
          </div>
          <div style={{ fontFamily: SF, fontSize: 11, fontWeight: 700, color: T.t3, textTransform: 'uppercase', letterSpacing: 0.6, margin: '18px 2px 8px' }}>Period</div>
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 2 }}>
            {RANGES.map(r => <Chip key={r.k} active={range === r.k} onClick={() => setRange(r.k)}>{r.l}</Chip>)}
          </div>

          {/* Summary card */}
          <div style={{ marginTop: 18, background: T.bg2, border: `0.5px solid ${T.hair}`, borderRadius: 14, padding: 16 }}>
            <div style={{ fontFamily: SF, fontSize: 11, color: T.t3, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 12 }}>Export summary</div>
            {sales && (
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: `0.5px solid ${T.hair}` }}>
                <span style={{ fontFamily: SF, fontSize: 13, color: T.t2 }}>Sales (net / VAT)</span>
                <span style={{ fontFamily: SFMono, fontSize: 13, color: T.t1, fontWeight: 600 }}>{gbp(sum.salesNet)} / {gbp(sum.salesVat)}</span>
              </div>
            )}
            {purchases && (
              <div style={{ display: 'flex', justifyContent: 'space-between', padding: '7px 0', borderBottom: `0.5px solid ${T.hair}` }}>
                <span style={{ fontFamily: SF, fontSize: 13, color: T.t2 }}>Purchases (net / VAT)</span>
                <span style={{ fontFamily: SFMono, fontSize: 13, color: T.t1, fontWeight: 600 }}>{gbp(sum.purchNet)} / {gbp(sum.purchVat)}</span>
              </div>
            )}
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0 2px' }}>
              <span style={{ fontFamily: SF, fontSize: 13, color: T.t1, fontWeight: 700 }}>{vat === 'cis' ? 'VAT (reverse charge)' : 'Net VAT due to HMRC'}</span>
              <span style={{ fontFamily: SFMono, fontSize: 14, color: vat === 'cis' ? T.t2 : (sum.vatDue >= 0 ? T.amber : T.green), fontWeight: 700 }}>{vat === 'cis' ? '£0.00' : gbp(sum.vatDue)}</span>
            </div>
          </div>

          {/* Preview */}
          <div style={{ marginTop: 14, background: T.bg0, border: `0.5px solid ${T.hair}`, borderRadius: 12, padding: 12, overflowX: 'auto' }}>
            <div style={{ fontFamily: SFMono, fontSize: 10, color: T.t3, whiteSpace: 'pre', lineHeight: 1.7 }}>
              {preview.csv.split('\r\n').slice(0, 4).join('\n') || 'No data'}
              {preview.count > 3 ? `\n… +${preview.count - 3} more line${preview.count - 3 === 1 ? '' : 's'}` : ''}
            </div>
          </div>
          ...
          }}>{React.cloneElement(Ic.download, { size: 16 })} {busy ? 'Generating…' : `Export ${preview.count} line${preview.count === 1 ? '' : 's'} · ${FORMATS.find(f => f.k === format).l}`}</button>
          ...
          <div style={{ marginTop: 12, padding: 12, background: T.bg2, borderRadius: 10, fontFamily: SF, fontSize: 11, color: T.t2, lineHeight: 1.5, display: 'flex', gap: 8 }}>
            ...
            <span>UTF-8 BOM + CRLF so Excel and accounting tools parse columns cleanly. CIS reverse charge sets output VAT to £0 and tags lines with the reverse-charge code your package expects.</span>
          </div>
        </div>
      </div>
    </ScreenBg>
...
Object.assign(window, { LedgerExportScreen });
