...
(function () {
  ...
  async function status() {
    try { return await req('/api/hmrc/status'); }
  ...
  async function submitCIS300(irEnvelope, periodEnd) {
    return req('/api/hmrc/cis300/submit', {
      method: 'POST',
      body: JSON.stringify({ irEnvelope, periodEnd }),
    });
  }
  ...
  async function pollUntilDone(correlationId, opts) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      onUpdate({ phase: result.status, attempt, elapsed: Math.round((Date.now() - t0) / 1000), result });
  ...
  async function history() {
    try { return await req('/api/hmrc/cis300/history'); }
    catch (e) { return { submissions: [], error: e.message }; }
  }

  async function discard(correlationId) {
    return req('/api/hmrc/cis300/submission?correlationId=' + encodeURIComponent(correlationId), { method: 'DELETE' });
  }

  window.CortexHMRC = { status, submitCIS300, checkStatus, pollUntilDone, history, discard };
})();
