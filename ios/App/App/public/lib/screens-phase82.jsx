...
(function () {
  ...
  const defaults = [
    { id: 'cortexbuild', name: 'CortexBuild Ltd', plan: 'Pro', role: 'Director', color: '#2563eb' },
    { id: 'camden-joinery', name: 'Camden Joinery', plan: 'Free', role: 'Co-owner', color: '#f59e0b' },
    { id: 'hackney-refurb', name: 'Hackney Refurb', plan: 'Free', role: 'Contractor', color: '#10b981' },
  ];

  function loadTenants() {
    try { const r = localStorage.getItem(LIST_KEY); if (r) return JSON.parse(r); } catch (e) {}
    try { localStorage.setItem(LIST_KEY, JSON.stringify(defaults)); } catch (e) {}
    return defaults;
  }

  window.CortexTenant = {
    list() { return loadTenants(); },
    active() {
      try { return localStorage.getItem(TKEY) || 'cortexbuild'; } catch (e) { return 'cortexbuild'; }
    },
    activeRecord() { return this.list().find(t => t.id === this.active()) || this.list()[0]; },
    // Namespaced DB key — each tenant gets its own data store
    dbKey() { return 'cortexx_db_v1__' + this.active(); },
    photoDb() { return 'cortexx_photos__' + this.active(); },
    switch(id) {
    ...
    create(name) {
      ...
      tenants.push({ id, name, plan: 'Free', role: 'Owner', color: '#8b5cf6' });
      try { localStorage.setItem(LIST_KEY, JSON.stringify(tenants)); } catch (e) {}
      return id;
    },
    setPlan(plan) {
      const tenants = this.list();
      const t = tenants.find(x => x.id === this.active());
      if (t) { t.plan = plan; try { localStorage.setItem(LIST_KEY, JSON.stringify(tenants)); } catch (e) {} }
      return t;
    },
  };
...
// ═══════════════════════════════════════════════════════════════════
function PhotoReviewScreen({ accent }) {
  ...
  const setStatus = async (id, status) => {
    ...
    setPhotos(ps => ps.map(p => p.id === id ? { ...p, reviewStatus: status } : p));
    if (window.cortexxToast) window.cortexxToast(`Photo ${status}`, status === 'approved' ? 'success' : status === 'rejected' ? 'error' : 'info');
    if (viewing && viewing.id === id) setViewing(v => ({ ...v, reviewStatus: status }));
  };

  const handleFile = async (e) => {
    const files = Array.from(e.target.files || []);
    for (const f of files) {
      await window.cortexxPhotoStore.save(f, { name: f.name, projectId: 1, reviewStatus: 'pending' });
    }
    await load();
  ...
  const tagged = photos.map(p => ({ ...p, reviewStatus: p.reviewStatus || 'pending' }));
  const counts = {
  ...
  const statusC = { pending: T.amber, approved: T.green, rejected: T.red };
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
        ...
        {shown.length === 0 ? (
          <div onClick={() => fileInput.current && fileInput.current.click()} style={{
            ...
            <div style={{ color: accent, fontSize: 34, marginBottom: 8 }}>{React.cloneElement(Ic.camera, { size: 34 })}</div>
            <div style={{ fontFamily: SF, fontSize: 14, color: T.t1, fontWeight: 600 }}>
              {filter === 'pending' ? 'No photos awaiting review' : `No ${filter} photos`}
            </div>
            <div style={{ fontFamily: SF, fontSize: 11, color: T.t2, marginTop: 4 }}>Tap to capture or upload site photos</div>
          </div>
        ) : (
                  ...
                  <div key={p.id} style={{ background: T.bg2, borderRadius: 12, overflow: 'hidden', border: `0.5px solid ${T.hair}` }}>
                    <div onClick={() => setViewing({ ...p, url })} style={{
                      ...
                      <div style={{ position: 'absolute', top: 6, left: 6 }}>
                        <Pill c={statusC[p.reviewStatus]} size="xs">{p.reviewStatus}</Pill>
                      </div>
                    </div>
                    {filter === 'pending' && (
...
function TenantScreen({ accent }) {
  ...
  return (
    <ScreenBg accent={accent}>
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 30 }}>
        <MobileHeader title="Workspaces" subtitle={`${tenants.length} organisations · isolated data`}/>
        <div style={{ padding: '4px 16px 14px' }}>
          <div style={{ background: `linear-gradient(135deg, ${T.purple}22, ${accent}11)`, border: `0.5px solid ${T.purple}44`, borderRadius: 12, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ color: T.purple }}>{React.cloneElement(Ic.shield, { size: 16 })}</div>
            <div style={{ flex: 1, fontFamily: SF, fontSize: 12, color: T.t1, lineHeight: 1.4 }}>
              Each workspace has fully isolated data — projects, money, photos. Switching reloads into that tenant's namespace.
            </div>
          </div>
        </div>
        <Section title="Your workspaces">
          <GroupedList>
            {tenants.map((t, i) => (
              <Row key={t.id}
                icon={React.cloneElement(<Avatar name={t.name} size={32} c={t.color}/>)}
                iconBg={null}
                title={t.name}
                sub={`${t.role} · ${t.plan}`}
        ...
        <Section title="Create workspace">
          <div style={{ background: T.bg2, border: `0.5px solid ${T.hair}`, borderRadius: 12, padding: 12 }}>
            ...
            }} style={{ width: '100%', marginTop: 8, background: accent, color: '#fff', border: 'none', borderRadius: 10, padding: '11px', fontFamily: SF, fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>Create & switch</button>
          </div>
        </Section>
      </div>
    </ScreenBg>
...
Object.assign(window, { PhotoReviewScreen, TenantScreen });
