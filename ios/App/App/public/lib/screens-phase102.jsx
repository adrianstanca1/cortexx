...
(function () {
  ...
  function mapHeaders(headers) {
    const idx = { date: -1, desc: -1, amount: -1, in: -1, out: -1 };
  ...
  function parseStatement(text) {
    ...
    if (!rows.length) return { credits: [], debits: [], headers: null };
    ...
    const idx = looksHeader ? mapHeaders(headers) : { date: 0, desc: 2, amount: 3, in: -1, out: -1 };
    ...
    for (const r of data) {
      ...
      let amount = 0;
      if (idx.in >= 0 && parseAmount(r[idx.in]) > 0) amount = parseAmount(r[idx.in]);
      else if (idx.out >= 0 && parseAmount(r[idx.out]) > 0) amount = -parseAmount(r[idx.out]);
      else amount = parseAmount(r[idx.amount]);
      if (!date || !desc) continue;
      const rec = { date, desc, amount: Math.abs(amount), raw: r.join(' | ') };
      if (amount > 0) credits.push(rec);
      else if (amount < 0) debits.push(rec);
    }
    return { credits, debits, headers };
  }
  ...
  function scoreMatch(invoice, tx) {
    ...
    if (Math.abs(invoice.amount - tx.amount) < 0.011) score += 60;
    else return 0; // skip — non-matching amounts can't be the same payment
    // Reference fuzzy: invoice id appears in description
    const descUC = tx.desc.toUpperCase();
    if (descUC.includes(invoice.id)) score += 30;
    else if (descUC.includes(invoice.id.replace(/^INV-/, ''))) score += 15;
    // Client name appears
    const clientWords = String(invoice.client || '').toUpperCase().split(/[\s,.]+/).filter(w => w.length > 3);
    for (const w of clientWords) if (descUC.includes(w)) { score += 10; break; }
    ...
    if (invDate && tx.date) {
      ...
      if (diff <= 3) score += 8;
      else if (diff <= 14) score += 4;
      else if (diff <= 60) score += 1;
      else score -= 5; // suspicious if very far
    }
    return score;
  }
    ...
    try {
      if (window.Backend.db.activity && window.Backend.db.activity.create) {
        await window.Backend.db.activity.create({
          ...
          t: 'Bank reconciliation', sub: done + ' invoice' + (done === 1 ? '' : 's') + ' marked paid', when: 'now', icon: '🏦',
        });
      }
    } catch (e) {}
    return done;
  ...
  window.CortexBankRec = { parseStatement, autoMatch, reconcile };
})();

function BankRecScreen({ accent }) {
  ...
  const onFile = async (file) => {
    ...
    const sel = {};
    m.matches.forEach(x => { if (x.confidence !== 'low') sel[x.invoice.id] = true; });
    setSelected(sel);
    setStage('review');
  ...
  const pullFromBank = async () => {
    if (!window.CortexBanking) { if (window.cortexxToast) window.cortexxToast('Banking client not loaded', 'error'); return; }
    ...
    try {
      ...
      setParsed({ credits, debits, source: 'open-banking' });
      const m = window.CortexBankRec.autoMatch(invoices, credits);
      setMatches(m.matches); setUnmatchedTx(m.unmatchedTx); setUnmatchedInv(m.unmatchedInv);
      const sel = {}; m.matches.forEach(x => { if (x.confidence !== 'low') sel[x.invoice.id] = true; });
      setSelected(sel);
      setStage('review');
  ...
  const restart = () => { setStage('upload'); setParsed(null); setMatches([]); setUnmatchedTx([]); setUnmatchedInv([]); setSelected({}); setCount(0); };

  const Pill = ({ c, label }) => <span style={{ padding: '3px 8px', borderRadius: 999, background: c + '20', color: c, fontSize: 10, fontFamily: SFMono, fontWeight: 700, letterSpacing: 0.4 }}>{label}</span>;

  return (
    <ScreenBg accent={accent}>
      <MobileHeader title="Bank reconciliation" subtitle="Upload statement · auto-match · one-tap reconcile"/>
      <div style={{ padding: '0 18px 110px', fontFamily: SF }}>
        {stage === 'upload' && <>
          <div style={{ marginTop: 18, padding: 18, borderRadius: 14, background: T.bg2, border: '1px solid ' + T.hair, textAlign: 'center' }}>
            <div style={{ fontSize: 36, marginBottom: 8 }}>🏦</div>
            <div style={{ fontSize: 14, fontWeight: 700, color: T.t1, marginBottom: 4 }}>Drop a bank statement CSV</div>
            <div style={{ fontSize: 12, color: T.t2, lineHeight: 1.5, marginBottom: 14 }}>
              Auto-detects columns from Barclays, Lloyds, HSBC, NatWest, Monzo, Starling, Tide, Revolut and most others.
            </div>
          ...
          <div style={{ marginTop: 14, padding: 12, borderRadius: 10, background: T.bg2, border: '1px solid ' + T.hair }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: T.t2, letterSpacing: 0.6, marginBottom: 6 }}>OUTSTANDING NOW</div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
              <span style={{ color: T.t1 }}>{invoices.filter(i => i.status !== 'paid').length} invoices</span>
              <span style={{ fontWeight: 700, color: T.t1, fontFamily: SFMono }}>£{invoices.filter(i => i.status !== 'paid').reduce((s, i) => s + (i.amount || 0), 0).toLocaleString()}</span>
            </div>
          </div>
        </>}

        {stage === 'review' && <>
          <div style={{ marginTop: 14, padding: 12, borderRadius: 10, background: T.bg2, border: '1px solid ' + T.hair, display: 'flex', justifyContent: 'space-around', textAlign: 'center', fontSize: 12, color: T.t2 }}>
            <div><div style={{ fontSize: 18, fontWeight: 700, color: T.green }}>{matches.length}</div>matches</div>
            <div><div style={{ fontSize: 18, fontWeight: 700, color: T.amber }}>{unmatchedTx.length}</div>orphan tx</div>
            <div><div style={{ fontSize: 18, fontWeight: 700, color: T.red }}>{unmatchedInv.length}</div>unpaid</div>
          </div>
          <div style={{ marginTop: 16, fontSize: 11, fontWeight: 700, color: T.t2, letterSpacing: 0.6 }}>MATCHES</div>
          {matches.length === 0 && <div style={{ padding: 14, fontSize: 13, color: T.t2 }}>No automatic matches found. You can still review unmatched items below.</div>}
          {matches.map(m => (
            <label key={m.invoice.id}
              style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 8, padding: 12, borderRadius: 10,
                background: T.bg2, border: '1px solid ' + (selected[m.invoice.id] ? T.green : T.hair) }}>
              <input type="checkbox" checked={!!selected[m.invoice.id]}
                onChange={e => setSelected({ ...selected, [m.invoice.id]: e.target.checked })}
                style={{ width: 18, height: 18, accentColor: T.green }}/>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: T.t1 }}>{m.invoice.id} · £{m.invoice.amount.toLocaleString()}</div>
                <div style={{ fontSize: 11, color: T.t2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {m.invoice.client} ← {m.tx.desc.slice(0, 50)}
                </div>
              </div>
              <Pill c={m.confidence === 'high' ? T.green : m.confidence === 'medium' ? T.amber : T.t2} label={m.confidence}/>
            </label>
          ))}

          {unmatchedTx.length > 0 && <>
            <div style={{ marginTop: 18, fontSize: 11, fontWeight: 700, color: T.t2, letterSpacing: 0.6 }}>ORPHAN CREDITS</div>
            {unmatchedTx.slice(0, 8).map((tx, i) => (
              <div key={i} style={{ marginTop: 6, padding: 10, borderRadius: 8, background: T.bg2, border: '1px solid ' + T.hair }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12 }}>
                  <span style={{ color: T.t1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '70%' }}>{tx.desc}</span>
                  <span style={{ fontWeight: 700, fontFamily: SFMono, color: T.green }}>£{tx.amount.toLocaleString()}</span>
                </div>
                <div style={{ fontSize: 10, color: T.t2, marginTop: 2 }}>{tx.date}</div>
              </div>
            ))}
            {unmatchedTx.length > 8 && <div style={{ marginTop: 6, fontSize: 11, color: T.t2 }}>… and {unmatchedTx.length - 8} more</div>}
          </>}
        ...
        {stage === 'done' && <div style={{ marginTop: 60, textAlign: 'center', padding: 20 }}>
...
Object.assign(window, { BankRecScreen });
