...
(function () {
  ...
  function getMonthRange(monthEnd) {
    ...
    start.setDate(6); start.setHours(0, 0, 0, 0);
    return { start, end };
  }
  ...
  function compute(monthEnd) {
    const snap = (window.Backend && window.Backend.db && window.Backend.db.snapshot) ? window.Backend.db.snapshot() : { cisPayments: [], cisSubs: [] };
    const { start, end } = getMonthRange(monthEnd);
    ...
    const subRows = [];
    let totalPayments = 0, totalDeductions = 0, totalMaterials = 0;
    for (const key of Object.keys(bySub)) {
      const sub = subs.find(s => s.id === key || s.utr === key || s.name === key) || { name: key };
      const pays = bySub[key];
      let gross = 0, deduction = 0, materials = 0;
      for (const p of pays) {
    ...
    return {
      monthEnd, start: start.toISOString().slice(0, 10), end: end.toISOString().slice(0, 10),
      subs: subRows,
      totals: {
        subs: subRows.length,
...
${subs}
  ...
  function toCSV(ret) {
    ...
    const rows = ret.subs.map(s => [
      s.utr, '"' + (s.name || '').replace(/"/g, '""') + '"', '"' + (s.trading || '').replace(/"/g, '""') + '"',
      s.verified ? 'Y' : 'N', fmtAmount(s.gross), fmtAmount(s.materials), fmtAmount(s.gross - s.materials), fmtAmount(s.deduction),
    ].join(',')).join('\n');
  ...
  window.CortexCIS300 = { compute, toHMRCXml, toCSV, download, ukTaxMonthLabel };
})();
