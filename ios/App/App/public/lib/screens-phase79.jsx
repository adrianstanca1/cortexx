...
(function () {
  ...
  const writeEntry = (what, where, color, icon) => {
    try {
      ...
      t.create({ who, what, where, when, sig, prev, icon, color });
    } catch (e) {/* silent */}
  };
  ...
  const phraseFor = (table, op, before, after) => {
    ...
    const colors = { create: '#10b981', update: '#2563eb', remove: '#ef4444' };
    const icons  = { create: 'plus', update: 'edit', remove: 'trash' };
    ...
    const friendlyTable = {
      tasks: 'task', projects: 'project', customers: 'customer',
      quotes: 'quote', invoices: 'invoice', receipts: 'receipt',
      timesheets: 'timesheet', team: 'team member', subs: 'subcontractor',
      documents: 'document', incidents: 'incident', holidays: 'leave',
      claims: 'insurance claim', improvements: 'improvement',
      mileage: 'mileage trip', clockEntries: 'clock entry',
      tags: 'tag', savedViews: 'saved view', templates: 'template',
      costItems: 'cost item', rfis: 'RFI', snags: 'snag',
      messages: 'message', purchaseOrders: 'PO',
    };
    ...
    return { what: `${verb} ${noun}${refId}${titleStr}`, where, color: colors[op], icon: icons[op] };
  };

  // Wrap every existing CRUD table (and any registered later via Backend.db.table).
  const skipTables = new Set([
    'user', 'settings', 'activity', 'auditLog', 'docGens',
    'computed', 'aiHistory', // ephemeral / derived
  ]);

  function wrapTable(name, tbl) {
    if (!tbl || tbl._audited || skipTables.has(name)) return;
    tbl._audited = true;
    const origC = tbl.create, origU = tbl.update, origR = tbl.remove;
    if (origC) tbl.create = async (d) => {
      const r = await origC.call(tbl, d);
      const { what, where, color, icon } = phraseFor(name, 'create', null, d);
    ...
    if (origU) tbl.update = async (id, p) => {
      ...
      const { what, where, color, icon } = phraseFor(name, 'update', before, { id, ...p });
      writeEntry(what, where, color, icon);
      return r;
    };
    if (origR) tbl.remove = async (id) => {
      ...
      const { what, where, color, icon } = phraseFor(name, 'remove', before, null);
      writeEntry(what, where, color, icon);
      return r;
    };
  }
...
function PhotoToSnagSheet({ onClose, accent }) {
  ...
  const onFile = async (e) => {
    ...
    setStage('scanning'); setErr(''); setPicked(new Set());
  ...
  const fileSelected = async () => {
    ...
    if (blob && window.cortexxPhotoStore) {
      try { photoId = await window.cortexxPhotoStore.save(blob, { name: `snag_evidence.jpg`, projectId, tags: ['snag', 'evidence'] }); }
      catch (e) {}
    }
        ...
        {stage === 'pick' && (
          <>
            <button onClick={pick} style={{
              ...
              }}>{React.cloneElement(Ic.alert, { size: 32 })}</div>
              <div style={{ fontFamily: SF, fontSize: 15, fontWeight: 700 }}>Photograph the defect</div>
              <div style={{ fontFamily: SF, fontSize: 11, color: T.t3, textAlign: 'center', lineHeight: 1.5, padding: '0 30px' }}>
                Cortex vision will list every defect it can see — cracked tiles, paint runs, gaps, alignment issues — and file each as a separate snag.
              </div>
            </button>
          ...
          <>
            <div style={{ position: 'relative', borderRadius: 14, overflow: 'hidden', marginBottom: 14, background: '#000' }}>
              ...
              <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                ...
                <div style={{ fontFamily: SF, fontSize: 13, color: '#fff', fontWeight: 600 }}>Scanning for defects…</div>
              </div>
            </div>
        ...
        {stage === 'review' && result && (
          <>
            ...
            <div style={{ fontFamily: SF, fontSize: 13, color: T.t1, lineHeight: 1.45, marginBottom: 14 }}>
              {result.summary || 'Defects detected:'}
            </div>

            {result.snags.length === 0 && (
              <div style={{ padding: 28, textAlign: 'center', fontFamily: SF, fontSize: 13, color: T.t3 }}>
                Nothing to file — this looks clean ✓
              </div>
            )}
                ...
                return (
                  <div key={i} onClick={() => toggle(i)} style={{
                    ...
                    <div style={{
                      ...
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                    }}>{sel && <span style={{ color: '#fff' }}>{React.cloneElement(Ic.check, { size: 12, sw: 3 })}</span>}</div>
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                        <Pill c={priColor(sn.priority)} size="xs">{sn.priority}</Pill>
                        {sn.area && <span style={{ fontFamily: SF, fontSize: 10.5, color: T.t3 }}>{sn.area}</span>}
                      </div>
                      <div style={{ fontFamily: SF, fontSize: 13.5, color: T.t1, lineHeight: 1.35 }}>{sn.title}</div>
                    </div>
                  </div>
                );
              })}
            ...
            {result.snags.length > 0 && (
              <>
                ...
                <button onClick={fileSelected} disabled={picked.size === 0} style={{
                  ...
                  {React.cloneElement(Ic.check, { size: 15 })}
                ...
                }}>Re-scan with different photo</button>
              </>
            )}
          </>
        )}
...
Object.assign(window, { PhotoToSnagSheet });
