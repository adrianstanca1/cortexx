...
(function () {
  ...
  window.ClientExperienceScreen = function ({ accent }) {
    ...
    // ── Live progress feed ─────────────────────────────────────────
    const Feed = () => React.createElement('div', null,
      React.createElement('div', { style: { fontSize: 12, color: T.t2, marginBottom: 14, display: 'flex', alignItems: 'center', gap: 6 } },
        React.createElement('span', { style: { width: 8, height: 8, borderRadius: 4, background: T.green, display: 'inline-block' } }),
        'This feed is what your clients see in their branded portal'),
      feed.length === 0
        ? React.createElement('p', { style: { color: T.t2, fontSize: 13, textAlign: 'center', padding: 30 } }, 'No updates posted yet.')
        : feed.sort((a, b) => new Date(b.when) - new Date(a.when)).map((f, i) => React.createElement('div', { key: f.id, style: { display: 'flex', gap: 12, marginBottom: 4 } },
            React.createElement('div', { style: { display: 'flex', flexDirection: 'column', alignItems: 'center' } },
              React.createElement('div', { style: { width: 12, height: 12, borderRadius: 6, background: acc, marginTop: 4 } }),
              i < feed.length - 1 && React.createElement('div', { style: { width: 2, flex: 1, background: T.hair, minHeight: 30 } })),
    ...
    // ── Variation approvals ────────────────────────────────────────
    const Variations = () => React.createElement('div', null,
      changes.length === 0
        ? React.createElement('p', { style: { color: T.t2, fontSize: 13, textAlign: 'center', padding: 30 } }, 'No variations raised.')
        : changes.map(c => {
            const status = c.status || 'pending';
            const col = status === 'approved' ? T.green : status === 'rejected' ? T.red : '#f59e0b';
            return React.createElement('div', { key: c.id, style: card({ marginBottom: 10 }) },
              React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', marginBottom: 4 } },
                React.createElement('span', { style: { fontSize: 14, fontWeight: 700, color: T.t1 } }, c.ref || c.title || 'Variation'),
                React.createElement('span', { style: { fontSize: 15, fontWeight: 800, color: T.t1 } }, money(c.value || c.amount || 0))),
              React.createElement('div', { style: { fontSize: 12, color: T.t2, marginBottom: 4 } }, c.desc || c.description || c.title || ''),
    ...
    // ── Satisfaction / NPS ─────────────────────────────────────────
    const Satisfaction = () => React.createElement('div', null,
      React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 } },
        React.createElement('div', { style: card({ padding: 18, textAlign: 'center' }) },
          React.createElement('div', { style: { fontSize: 32, fontWeight: 800, color: Number(avgSat) >= 8 ? T.green : Number(avgSat) >= 6 ? '#f59e0b' : T.red } }, avgSat),
          React.createElement('div', { style: { fontSize: 11, color: T.t3 } }, 'Avg satisfaction / 10')),
        React.createElement('div', { style: card({ padding: 18, textAlign: 'center' }) },
          React.createElement('div', { style: { fontSize: 32, fontWeight: 800, color: nps >= 50 ? T.green : nps >= 0 ? '#f59e0b' : T.red } }, (nps > 0 ? '+' : '') + nps),
          React.createElement('div', { style: { fontSize: 11, color: T.t3 } }, 'NPS score'))),
      satisfaction.length === 0
        ? React.createElement('p', { style: { color: T.t2, fontSize: 13, textAlign: 'center', padding: 30 } }, 'No surveys completed yet.')
        : satisfaction.sort((a, b) => new Date(b.surveyedOn) - new Date(a.surveyedOn)).map(s => React.createElement('div', { key: s.id, style: card({ marginBottom: 10 }) },
            React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 } },
              React.createElement('div', null,
                React.createElement('div', { style: { fontSize: 13, fontWeight: 700, color: T.t1 } }, s.client),
    ...
    return React.createElement('div', { style: { height: '100%', overflowY: 'auto', padding: '12px 16px 120px' } },
      React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 14 } },
        React.createElement('div', { style: card({ padding: 12 }) },
          React.createElement('div', { style: { fontSize: 20, fontWeight: 800, color: acc } }, avgSat),
          React.createElement('div', { style: { fontSize: 10, color: T.t3 } }, 'Satisfaction')),
        React.createElement('div', { style: card({ padding: 12 }) },
          React.createElement('div', { style: { fontSize: 20, fontWeight: 800, color: nps >= 0 ? T.green : T.red } }, (nps > 0 ? '+' : '') + nps),
          React.createElement('div', { style: { fontSize: 10, color: T.t3 } }, 'NPS')),
        React.createElement('div', { style: card({ padding: 12, borderColor: pendingChanges.length ? 'rgba(245,158,11,0.4)' : T.hair }) },
          React.createElement('div', { style: { fontSize: 20, fontWeight: 800, color: pendingChanges.length ? '#f59e0b' : T.green } }, pendingChanges.length),
          React.createElement('div', { style: { fontSize: 10, color: T.t3 } }, 'Pending'))),
      React.createElement(TabBar),
      tab === 'feed' && React.createElement(Feed),
      tab === 'variations' && React.createElement(Variations),
      tab === 'satisfaction' && React.createElement(Satisfaction)
    );
  };

  // ── Post progress update sheet ───────────────────────────────────
  window.PostUpdateSheet = function ({ onClose, accent }) {
    const projects = window.useDB('projects');
    const [form, setForm] = React.useState({ projectId: (projects[0] || {}).id || '', title: '', body: '', clientVisible: true });
    ...
    const save = async () => {
      ...
      setSaving(true);
      await Backend.db.progressFeed.create({ projectId: parseInt(form.projectId) || form.projectId, title: form.title, body: form.body, when: new Date().toISOString().slice(0, 10), clientVisible: form.clientVisible, photo: null });
    ...
    return React.createElement('div', { style: { position: 'fixed', inset: 0, background: T.bg1, zIndex: 1100, overflowY: 'auto', paddingBottom: 100 } },
      ...
      React.createElement('div', { style: { padding: '20px 20px 0' } },
        ...
        React.createElement(Field, { label: 'Headline' }, React.createElement('input', { style: inp, placeholder: 'e.g. Roof structure complete', value: form.title, onChange: e => set('title', e.target.value) })),
        React.createElement(Field, { label: 'Detail' }, React.createElement('textarea', { style: { ...inp, minHeight: 90, resize: 'vertical' }, placeholder: 'What progress has been made?', value: form.body, onChange: e => set('body', e.target.value) })),
        React.createElement('label', { style: { display: 'flex', alignItems: 'center', gap: 10, color: T.t1, fontSize: 14, cursor: 'pointer' } },
          React.createElement('input', { type: 'checkbox', checked: form.clientVisible, onChange: e => set('clientVisible', e.target.checked) }), 'Visible to client in portal'))
    );
  };
