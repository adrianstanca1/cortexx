...
(function () {
  ...
  const grabJSON = (raw, arr) => {
    try { const m = raw.match(arr ? /\[[\s\S]*\]/ : /\{[\s\S]*\}/); return m ? JSON.parse(m[0]) : null; }
    catch (e) { return null; }
  };
  ...
  Backend.vera.estimateLead = async (lead) => {
    ...
    try { await Backend.db.leads.update(lead.id, { stage: 'quoted', value: est.total || lead.value, updated: new Date().toISOString().slice(0, 10) }); } catch (e) {}
  ...
  const toImg = (blob) => new Promise((res, rej) => {
    ...
    r.onload = () => res({ media_type: blob.type || 'image/jpeg', data: String(r.result || '').split(',')[1] || '' });
  ...
  const downscale = (blob, maxDim = 1280) => new Promise((res) => {
    ...
    img.onload = () => { try {
      ...
      const c = document.createElement('canvas'); c.width = Math.round(img.naturalWidth * ratio); c.height = Math.round(img.naturalHeight * ratio);
      c.getContext('2d').drawImage(img, 0, 0, c.width, c.height);
      c.toBlob((o) => { URL.revokeObjectURL(url); res(o || blob); }, 'image/jpeg', 0.82);
    } catch (e) { URL.revokeObjectURL(url); res(blob); } };
  ...
  Backend.vision.extractActions = async (blob) => {
    ...
    try {
      ...
      return grabJSON(raw) || { summary: '', items: [] };
    } catch (e) { return { summary: '', items: [] }; }
  };
...
Email: """${text.slice(0, 4000)}"""`;
  ...
  Backend.ai.fileTriage = async (t) => {
    ...
    if (t.category === 'lead' || t.category === 'enquiry') {
      const lead = await Backend.db.leads.create({ name: e.name || 'New enquiry', inquiry: e.inquiry || t.summary, value: val, source: 'Email', stage: 'new', updated: new Date().toISOString().slice(0, 10) });
      if (window.CortexAudit) window.CortexAudit.log('You', `filed email as lead: ${lead.name}`, 'Inbox');
      return { kind: 'lead', record: lead };
    }
    if (t.category === 'invoice') {
      const task = await Backend.db.tasks.create({ title: `Process invoice: ${e.name || t.summary}`, assignee: 'You', prio: 'high', due: e.due || null });
      return { kind: 'task', record: task };
    }
    const task = await Backend.db.tasks.create({ title: t.suggestedAction || t.summary, assignee: 'You', prio: 'med' });
    return { kind: 'task', record: task };
  };
})();
...
// ═══════════════════════════════════════════════════════════
function PhotoMentionSheet({ onClose, accent }) {
  ...
  const pick = (e) => {
    ...
    if (!f) return;
    setBlob(f); setResult(null); setFiled({});
  ...
  const file = async (item, i) => {
    if (filed[i]) return;
    if (item.type === 'snag') await Backend.db.snags?.create?.({ title: item.title, priority: item.priority, status: 'open', created: new Date().toISOString().slice(0,10) });
    else if (item.type === 'rfi') await Backend.db.rfis?.create?.({ subject: item.title, status: 'open', priority: item.priority });
    else await Backend.db.tasks.create({ title: item.title, prio: item.priority === 'high' ? 'high' : item.priority === 'low' ? 'low' : 'med', assignee: 'You' });
    setFiled(f => ({ ...f, [i]: true }));
    if (window.cortexxToast) window.cortexxToast(`${item.type.toUpperCase()} created`, 'success');
  };

  const TYPE_C = { task: accent, snag: T.amber, rfi: T.purple };
  return (
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 16px 24px' }}>
        ...
        {result && (
          <div style={{ marginTop: 14 }}>
            {result.summary && <div style={{ background: `${accent}11`, border: `0.5px solid ${accent}33`, borderRadius: 12, padding: 12, fontFamily: SF, fontSize: 13, color: T.t1, lineHeight: 1.5, marginBottom: 12 }}>{React.cloneElement(Ic.spark, { size: 12, color: accent })} {result.summary}</div>}
            {(result.items || []).length === 0 ? (
              <div style={{ textAlign: 'center', fontFamily: SF, fontSize: 13, color: T.t3, padding: 20 }}>Nothing actionable spotted.</div>
            ) : (result.items || []).map((it, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, background: T.bg2, border: `0.5px solid ${T.hair}`, borderRadius: 12, padding: 12, marginBottom: 8 }}>
                <Pill c={TYPE_C[it.type] || accent} size="xs">{it.type}</Pill>
                <div style={{ flex: 1 }}>
...
// INBOX TRIAGE SHEET
// ═══════════════════════════════════════════════════════════
function InboxTriageSheet({ onClose, accent }) {
  ...
  const CAT_C = { lead: T.green, enquiry: T.green, invoice: T.amber, supplier: T.blue, complaint: T.red, other: T.t2 };
  return (
    <Sheet onClose={onClose}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 16px 10px' }}>
        <button onClick={onClose} style={{ background: 'none', border: 'none', color: accent, fontFamily: SF, fontSize: 16, cursor: 'pointer' }}>Close</button>
        ...
        {result && (
          <div style={{ marginTop: 16, background: T.bg2, border: `0.5px solid ${T.hair}`, borderRadius: 14, padding: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              <Pill c={CAT_C[result.category] || T.t2} size="sm">{result.category}</Pill>
              <span style={{ fontFamily: SFMono, fontSize: 11, color: T.t3 }}>{Math.round((result.confidence || 0) * 100)}% confident</span>
            </div>
            <div style={{ fontFamily: SF, fontSize: 14, color: T.t1, lineHeight: 1.5, marginBottom: 8 }}>{result.summary}</div>
            <div style={{ fontFamily: SF, fontSize: 12, color: T.t2, marginBottom: 12 }}>→ {result.suggestedAction}</div>
            {result.extract && (result.extract.name || result.extract.value) && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
                {result.extract.name && <Pill c={T.bg3} size="xs">{result.extract.name}</Pill>}
                {result.extract.value && <Pill c={T.bg3} size="xs">£{result.extract.value}</Pill>}
                {result.extract.due && <Pill c={T.bg3} size="xs">due {result.extract.due}</Pill>}
              </div>
            )}
            {filed ? (
...
Object.assign(window, { PhotoMentionSheet, InboxTriageSheet });
