...
const colorForActor = (actorType: string) => {
  ...
  if (actorType === 'system') return 'var(--t3)'
  return '#2563eb'
}
...
export default function Timeline({ accent = '#f59e0b', data }: TimelineProps) {
  ...
  if (events.length === 0) {
    return (
      <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--t3)', fontFamily: 'var(--font-system)', fontSize: 14 }}>
    ...
    <div style={{ padding: '8px 20px 100px' }}>
      <div style={{ padding: '8px 0 18px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
          <div style={{ fontFamily: 'var(--font-system)', fontSize: 22, fontWeight: 700, color: 'var(--t1)', letterSpacing: '-0.03em' }}>Today</div>
        ...
        <div style={{ fontFamily: 'var(--font-system)', fontSize: 13, color: 'var(--t2)', marginTop: 2 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
          {events.map((ev, i) => (
            <div key={i} style={{ display: 'flex', gap: 14, paddingBottom: 20, opacity: ev.done && !ev.isNow ? 0.45 : 1, position: 'relative' }}>
              <span style={{ width: 42, fontSize: 11, fontWeight: 600, color: ev.isNow ? accent : 'var(--t3)', fontFamily: 'var(--font-system)', flexShrink: 0, paddingTop: 8 }}>
                {ev.time}
              </span>
              <div style={{ width: 12, height: 12, borderRadius: '50%', background: ev.isNow ? accent : ev.done ? 'var(--bg3)' : 'rgba(255,255,255,0.15)', border: `2px solid ${ev.isNow ? accent : 'rgba(255,255,255,0.2)'}`, flexShrink: 0, marginTop: 10, position: 'relative', zIndex: 1 }}>
                {ev.isNow && (
              ...
              <div style={{ flex: 1, padding: '8px 14px', borderRadius: 14, background: ev.isNow ? `${accent}15` : 'rgba(255,255,255,0.03)', border: ev.isNow ? `1px solid ${accent}44` : '1px solid rgba(255,255,255,0.06)' }}>
                ...
                <p style={{ fontSize: 13, fontWeight: 600, color: ev.isNow ? 'var(--t1)' : ev.done ? 'var(--t2)' : 'var(--t1)', fontFamily: 'var(--font-system)', textDecoration: ev.done && !ev.isNow ? 'line-through' : 'none', marginTop: ev.isNow ? 2 : 0 }}>
                  {ev.label}
                </p>
                <p style={{ fontSize: 11, color: 'var(--t3)', marginTop: 2, fontFamily: 'var(--font-system)' }}>{ev.sub}</p>
              </div>
            </div>
          ))}
        </div>
