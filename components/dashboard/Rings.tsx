    ...
    <div style={{ padding: '0 0 100px' }}>
      <div style={{ padding: '20px 20px 8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <h2 style={{ fontSize: 26, fontWeight: 700, color: 'var(--t1)', letterSpacing: '-0.03em', fontFamily: 'var(--font-system)' }}>This week</h2>
      ...
      <div style={{ padding: '8px 0 14px', display: 'flex', justifyContent: 'center' }}>
        <svg width={ringSize} height={ringSize} viewBox={`0 0 ${ringSize} ${ringSize}`}>
          ...
          <text x={ringSize / 2} y={ringSize / 2 - 6} textAnchor="middle" fill="var(--t1)" fontSize="28" fontWeight="700" fontFamily="ui-monospace, monospace">{avgCompletion}%</text>
          <text x={ringSize / 2} y={ringSize / 2 + 18} textAnchor="middle" fill="var(--t3)" fontSize="11" fontFamily="-apple-system, system-ui">avg completion</text>
        </svg>
      </div>
      ...
      <div style={{ padding: '0 20px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        {rings.map((r, i) => (
          <div key={i} style={{ background: 'var(--surface-raised)', borderRadius: 12, padding: '12px 14px', border: '0.5px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 10, height: 10, borderRadius: 5, background: r.c, boxShadow: `0 0 8px ${r.c}88`, flexShrink: 0 }} />
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: 'var(--font-system)', fontSize: 13, color: 'var(--t2)', fontWeight: 600 }}>{r.l}</div>
              <div style={{ fontFamily: 'ui-monospace, monospace', fontSize: 18, color: 'var(--t1)', fontWeight: 700, marginTop: 1, letterSpacing: -0.4 }}>
                {r.raw}{r.u} <span style={{ color: 'var(--t3)', fontSize: 13 }}>/ {r.max}{r.u}</span>
              </div>
            </div>
      ...
      <div style={{ padding: '0 20px' }}>
        <div style={{ background: 'linear-gradient(135deg, rgba(245,158,11,0.13), rgba(239,68,68,0.13))', border: '0.5px solid rgba(245,158,11,0.3)', borderRadius: 14, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontFamily: 'var(--font-system)', fontSize: 14, fontWeight: 600, color: 'var(--t1)' }}>{activeSites} active sites</div>
            <div style={{ fontFamily: 'var(--font-system)', fontSize: 11, color: 'var(--t2)' }}>{hoursThisWeek}h billed this week across all sites</div>
          </div>
