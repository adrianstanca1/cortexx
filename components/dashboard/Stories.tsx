...
const statusColors: Record<string, string> = {
  active: '#f59e0b', snagging: '#10b981', quoting: '#06b6d4', complete: 'var(--t3)'
}
  ...
  return (
    <div style={{ padding: '0 0 100px' }}>
      <div style={{ padding: '20px 20px 12px' }}>
        <h2 style={{ fontSize: 26, fontWeight: 700, color: 'var(--t1)', letterSpacing: '-0.03em', fontFamily: 'var(--font-system)' }}>Sites</h2>
        <p style={{ fontSize: 12, color: 'var(--t3)', marginTop: 2, fontFamily: 'var(--font-system)' }}>Swipe through · {activeCount} active</p>
      </div>
      ...
      <div style={{ padding: '4px 20px 16px', display: 'flex', gap: 14, overflowX: 'auto' }}>
        {projects.map((project, i) => {
          ...
          return (
            <div key={project.id} onClick={() => router.push(`/projects/${project.id}`)} style={{ flexShrink: 0, textAlign: 'center', width: 64, cursor: 'pointer' }}>
              ...
              <div style={{ fontFamily: 'var(--font-system)', fontSize: 11, color: 'var(--t1)', fontWeight: 500, marginTop: 5 }}>{project.name.split(' ')[0]}</div>
              <div style={{ fontFamily: 'ui-monospace, monospace', fontSize: 9, color: 'var(--t3)' }}>{project.progress}%</div>
            </div>
          )
        })}
        <div style={{ flexShrink: 0, textAlign: 'center', width: 64, cursor: 'pointer' }} onClick={() => router.push('/projects')}>
          ...
          <div style={{ fontFamily: 'var(--font-system)', fontSize: 11, color: 'var(--t3)', fontWeight: 500, marginTop: 5 }}>All</div>
          <div style={{ fontFamily: 'ui-monospace, monospace', fontSize: 9, color: 'var(--t3)' }}>{projects.length}</div>
        </div>
      </div>
          ...
          <div style={{ background: `linear-gradient(160deg, ${accent}28, #152641 60%)`, borderRadius: 18, padding: 16, border: `0.5px solid ${accent}44`, position: 'relative', overflow: 'hidden' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                <div style={{ fontFamily: 'var(--font-system)', fontSize: 20, fontWeight: 700, color: 'var(--t1)', marginTop: 8, letterSpacing: -0.4 }}>{featured.name}</div>
                <div style={{ fontFamily: 'var(--font-system)', fontSize: 12, color: 'var(--t2)', marginTop: 2 }}>{featured.clientName} · {featured.postcode}</div>
              </div>
              <div style={{ fontFamily: 'ui-monospace, monospace', fontSize: 28, fontWeight: 700, color: accent, letterSpacing: -1 }}>{featured.progress}<span style={{ fontSize: 16, color: 'var(--t2)' }}>%</span></div>
            </div>
            ...
            {(() => {
              ...
              return (
                <>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: 'ui-monospace, monospace', fontSize: 9, color: 'var(--t3)', marginTop: 6 }}>
      ...
      <div style={{ padding: '0 20px 8px', display: 'flex', alignItems: 'center', gap: 8 }}>
        <p style={{ fontFamily: 'var(--font-system)', fontSize: 11, fontWeight: 700, color: 'var(--t2)', textTransform: 'uppercase', letterSpacing: 0.6 }}>Activity feed</p>
        <span title={connected ? 'Live' : 'Reconnecting…'} style={{ width: 6, height: 6, borderRadius: '50%', background: connected ? '#10b981' : 'var(--t3)', boxShadow: connected ? '0 0 6px #10b98166' : 'none' }} />
      </div>
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        {activities.map((a) => {
          const color = a.actorType === 'ai' ? '#8b5cf6' : a.actorType === 'system' ? 'var(--t3)' : '#2563eb'
          const relTime = (() => {
            ...
            <div key={a.id} style={{ background: 'var(--surface-raised)', borderRadius: 12, padding: '10px 12px', border: '0.5px solid rgba(255,255,255,0.07)', display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: 'var(--font-system)', fontSize: 13, color: 'var(--t1)', lineHeight: 1.3 }}>
                  <span style={{ fontWeight: 600 }}>{a.actorName}</span> <span style={{ color: 'var(--t2)' }}>{a.action}</span>
                </div>
                <div style={{ fontFamily: 'var(--font-system)', fontSize: 11, color: 'var(--t3)', marginTop: 1 }}>
        ...
        {activities.length === 0 && (
          <p style={{ fontSize: 13, color: 'var(--t3)', fontFamily: 'var(--font-system)', textAlign: 'center', padding: '20px 0' }}>No recent activity</p>
        )}
      </div>
    </div>
  )
