...
export default function DocumentTemplatePage() {
  ...
  const create = async () => {
    const value = window.prompt('New templates — short label')
    if (!value) return
  ...
  return (
    <ModuleShell
      title="Templates"
      tagline="Document templates (RAMS / report / quote)"
      ...
      {loading ? (
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {rows.map(r => (
            ...
            >
              <div>{[r.name, r.category, r.body].filter(Boolean).join(' · ') || r.id}</div>
