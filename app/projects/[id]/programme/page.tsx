...
const statusColor: Record<string,string> = { not_started: 'var(--t3)', in_progress: '#2563eb', complete: '#10b981', blocked: '#ef4444' }
  ...
  return (
    <Shell>
      <header style={headerStyle}>
        <Link href={`/projects/${id}`} aria-label="Back to project" style={{ width: 36, height: 36, borderRadius: 9, background: 'var(--surface-raised)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><IcChevL size={18} color="var(--t2)" /></Link>
      ...
      <main style={{ padding: '14px 16px 96px', maxWidth: 1100, margin: '0 auto' }}>
        ...
        <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(120px,1fr))', gap: 8 }}>
          ...
          <Kpi label="Activities" value={String(data.summary.totalActivities)} color="var(--t1)" />
          ...
          <Kpi label="Overdue" value={String(data.summary.overdue)} color={data.summary.overdue ? '#ef4444' : 'var(--t3)'} />
          <Kpi label="Blocked" value={String(data.summary.blocked)} color={data.summary.blocked ? '#ef4444' : 'var(--t3)'} />
        </section>

        <section style={panelStyle}>
          <div style={sectionHeader}>
            <div><div style={eyebrow}>LOOK-AHEAD</div><div style={{ color: 'var(--t1)', fontWeight: 700, marginTop: 2 }}>Upcoming site work</div></div>
        ...
        <section style={panelStyle}>
          <div style={sectionHeader}>
            <div><div style={eyebrow}>CHANGE CONTROL</div><div style={{ color: 'var(--t1)', fontWeight: 700, marginTop: 2 }}>{data.latestBaseline ? `Baseline Rev ${data.latestBaseline.revision}` : 'No committed baseline'} · {data.delays.filter(d => d.status === 'open').length} open delays</div></div>
          ...
          {data.latestBaseline && <div style={{ padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}><b style={{ color: 'var(--t1)', fontSize: 12 }}>Rev {data.latestBaseline.revision}{data.latestBaseline.label ? ` · ${data.latestBaseline.label}` : ''}</b><span style={{ color: 'var(--t3)', fontSize: 10 }}>{prettyDate(data.latestBaseline.effectiveAt)}</span></div><div style={{ color: 'var(--t2)', fontSize: 11, marginTop: 4 }}>{data.latestBaseline.reason}</div></div>}
          {data.baselines.length > 1 && <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', padding: '9px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>{data.baselines.slice(1, 5).map(b => <span key={b.id} style={{ ...codeBadge, color: b.status === 'active' ? '#10b981' : 'var(--t2)' }}>Rev {b.revision} · {b.status}</span>)}</div>}
          {data.delays.length === 0 ? <Empty text="No programme delay events recorded." /> : <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8 }}>{data.delays.slice(0, 12).map(delay => <div key={delay.id} style={{ background: '#0d1b2d', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: 10 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'flex-start' }}><div style={{ minWidth: 0 }}><div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}><b style={{ color: 'var(--t1)', fontSize: 12 }}>{delay.title}</b><span style={codeBadge}>{delay.category}</span><span style={{ ...codeBadge, color: delay.status === 'open' ? '#f59e0b' : delay.status === 'accepted' ? '#10b981' : delay.status === 'rejected' ? '#ef4444' : 'var(--t2)' }}>{delay.status}</span></div><div style={{ color: 'var(--t3)', fontSize: 10, marginTop: 4 }}>{prettyDate(delay.startDate)}{delay.endDate ? ` → ${prettyDate(delay.endDate)}` : ''} · {delay.delayDays}d{delay.activity?.title ? ` · ${delay.activity.title}` : ''}</div>{delay.impact && <div style={{ color: 'var(--t2)', fontSize: 11, marginTop: 4 }}>{delay.impact}</div>}{delay.decisionNotes && <div style={{ color: 'var(--t2)', fontSize: 10, marginTop: 4 }}>Decision: {delay.decisionNotes}</div>}</div>{data.permissions.plan && <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', justifyContent: 'flex-end' }}>{delay.status === 'open' && <><button onClick={() => transitionDelay(delay, 'accepted')} style={miniButton}>Accept</button><button onClick={() => transitionDelay(delay, 'rejected')} style={{ ...miniButton, color: '#ef4444' }}>Reject</button><button onClick={() => removeDelay(delay)} aria-label={`Delete ${delay.title}`} style={iconButton}><IcTrash size={13} color="#ef4444" /></button></>}{delay.status === 'accepted' && <button onClick={() => transitionDelay(delay, 'closed')} style={miniButton}>Close</button>}</div>}</div></div>)}</div>}
        </section>

        <section style={panelStyle}>
          <div style={sectionHeader}>
            <div><div style={eyebrow}>MASTER PROGRAMME</div><div style={{ color: 'var(--t1)', fontWeight: 700, marginTop: 2 }}>{data.activities.length} activities · {data.summary.criticalPath.projectDurationDays} network days</div></div>
          ...
          {data.activities.length === 0 ? <Empty text="Build the project programme by adding the first activity." /> : <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {data.activities.map(activity => {
              ...
              const sc = statusColor[activity.status] || 'var(--t3)'
              return <article key={activity.id} style={{ background: '#0d1b2d', borderRadius: 12, padding: 12, border: `1px solid ${critical.has(activity.id) ? 'rgba(245,158,11,0.35)' : 'rgba(255,255,255,0.06)'}` }}>
                <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', gap: 7, alignItems: 'center', flexWrap: 'wrap' }}>
                      ...
                      {cpm && !cpm.critical && <span style={{ fontSize: 10, color: 'var(--t3)', fontFamily: 'var(--font-system)' }}>{Math.round(cpm.totalFloatDays)}d float</span>}
                    </div>
                    <div style={{ color: 'var(--t1)', fontWeight: 700, fontSize: 14, marginTop: 6, fontFamily: 'var(--font-system)' }}>{activity.title}</div>
                    <div style={{ color: 'var(--t2)', fontSize: 11, marginTop: 4, fontFamily: 'var(--font-system)' }}>{prettyDate(activity.plannedStart)} → {prettyDate(activity.plannedEnd)}{activity.responsibleMember?.name ? ` · ${activity.responsibleMember.name}` : ''}{activity.location ? ` · ${activity.location}` : ''}</div>
                  </div>
                ...
                <div style={{ marginTop: 10, display: 'grid', gridTemplateColumns: 'minmax(120px,1fr) auto', gap: 10, alignItems: 'center' }}>
                  <div><div style={{ height: 7, borderRadius: 5, background: 'rgba(255,255,255,0.07)', overflow: 'hidden' }}><div style={{ width: `${activity.progress}%`, height: '100%', background: sc }} /></div><div style={{ marginTop: 3, fontSize: 10, color: 'var(--t3)', fontFamily: 'var(--font-system)' }}>{activity.progress}% complete</div></div>
        ...
        {data.dependencies.length > 0 && <section style={panelStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 8 }}>{data.dependencies.map(dep => <div key={dep.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.05)', fontFamily: 'var(--font-system)', fontSize: 12 }}><span style={{ flex: 1, color: 'var(--t2)' }}><b style={{ color: 'var(--t1)' }}>{byId.get(dep.predecessorId)?.title || 'Activity'}</b> → <b style={{ color: 'var(--t1)' }}>{byId.get(dep.successorId)?.title || 'Activity'}</b> · {dep.type}{dep.lagDays ? ` ${dep.lagDays > 0 ? '+' : ''}${dep.lagDays}d` : ''}</span>{data.permissions.plan && <button onClick={() => removeDependency(dep)} aria-label="Remove dependency" style={{ background: 'none', border: 0, cursor: 'pointer' }}><IcX size={13} color="#ef4444" /></button>}</div>)}</div>
        </section>}
      </main>
      <TabBar />
...
function Kpi({ label, value, color }: { label: string; value: string; color: string }) { return <div style={{ background: 'var(--surface-raised)', borderRadius: 12, padding: '11px 12px', border: '1px solid rgba(255,255,255,0.06)' }}><div style={{ ...eyebrow, fontSize: 9 }}>{label}</div><div style={{ color, fontSize: 21, fontWeight: 800, marginTop: 3, fontFamily: 'ui-monospace, monospace' }}>{value}</div></div> }
function CompactActivity({ activity, critical }: { activity: Activity; critical: boolean }) { return <div style={{ display: 'flex', gap: 9, alignItems: 'center', padding: '9px 0', borderBottom: '1px solid rgba(255,255,255,0.05)' }}><div style={{ width: 7, height: 7, borderRadius: 7, background: critical ? '#f59e0b' : statusColor[activity.status] || 'var(--t3)' }} /><div style={{ flex: 1, minWidth: 0 }}><div style={{ color: 'var(--t1)', fontSize: 12, fontWeight: 650, fontFamily: 'var(--font-system)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{activity.title}</div><div style={{ color: 'var(--t3)', fontSize: 10, marginTop: 2, fontFamily: 'var(--font-system)' }}>{prettyDate(activity.plannedStart)} → {prettyDate(activity.plannedEnd)} · {activity.progress}%</div></div></div> }
function Empty({ text }: { text: string }) { return <div style={{ padding: '24px 8px', color: 'var(--t3)', fontSize: 12, textAlign: 'center', fontFamily: 'var(--font-system)' }}>{text}</div> }
...
const panelStyle: React.CSSProperties = { background: 'var(--surface-raised)', borderRadius: 14, padding: 14, border: '1px solid rgba(255,255,255,0.07)', marginTop: 12 }
...
const eyebrow: React.CSSProperties = { color: 'var(--t3)', fontSize: 10, fontWeight: 800, letterSpacing: 0.7, fontFamily: 'var(--font-system)' }
...
const inputStyle: React.CSSProperties = { width: '100%', boxSizing: 'border-box', background: 'var(--bg0)', color: 'var(--t1)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 9, padding: '10px 11px', fontFamily: 'var(--font-system)', fontSize: 13 }
...
const codeBadge: React.CSSProperties = { borderRadius: 5, background: 'rgba(255,255,255,0.06)', color: 'var(--t2)', padding: '2px 5px', fontSize: 9, fontWeight: 800, fontFamily: 'ui-monospace, monospace' }
const twoCol: React.CSSProperties = { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9 }
const centerStyle: React.CSSProperties = { minHeight: '70dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'var(--t3)', fontFamily: 'var(--font-system)' }
