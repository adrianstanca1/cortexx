...
const STATUS_COLOR: Record<Drawing['status'], string> = { draft: 'var(--t3)', approved: '#22c55e', superseded: '#f59e0b', archived: '#6b7280' }
...
export default function DrawingsPage() {
  ...
  return (
    <div className="module-page" style={{ background: 'var(--bg0)', minHeight: '100dvh', paddingBottom: 100 }}>
      ...
      <div className="module-header" data-kicker="Drawings command" style={{ padding: '20px 20px 12px 60px', position: 'sticky', top: 0, zIndex: 50, background: 'rgba(6,16,30,0.95)', backdropFilter: 'blur(12px)', borderBottom: '0.5px solid rgba(255,255,255,0.07)' }}>
        ...
        <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          {Object.values(grouped).map(g => (
            <div key={g.project?.id || 'unassigned'}>
              <div style={{ fontFamily: SF, fontSize: 11, color: 'var(--t3)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6, paddingLeft: 4 }}>{g.project?.name || 'Unassigned'} · {g.items.length}</div>
                    ...
                    <button key={d.id} onClick={() => openDetail(d)} style={{ display: 'flex', alignItems: 'center', gap: 10, width: '100%', padding: '11px 14px', background: 'transparent', border: 'none', borderBottom: i < g.items.length - 1 ? '0.5px solid rgba(255,255,255,0.04)' : 'none', textAlign: 'left', cursor: 'pointer' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                          <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: 10, color: 'var(--t3)', fontWeight: 700 }}>{d.number}</span>
                          <span style={{ fontFamily: SF, fontSize: 13, color: 'var(--t1)', fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.title}</span>
                        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
          ...
          <div style={{ position: 'relative', background: 'var(--surface-raised)', borderRadius: '20px 20px 0 0', padding: '24px 20px 40px', display: 'flex', flexDirection: 'column', gap: 12, maxHeight: '92dvh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                <div style={{ fontFamily: 'ui-monospace, monospace', fontSize: 11, color: 'var(--t3)', fontWeight: 700 }}>{activeDwg.number}</div>
            ...
            <div style={{ background: 'var(--bg3)', borderRadius: 10, padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div style={{ fontFamily: SF, fontSize: 11, color: 'var(--t3)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 }}>Upload new revision</div>
            ...
            <div>
              ...
              {activeDwg.revisions.length === 0 ? (
                <div style={{ padding: 12, fontFamily: SF, fontSize: 12, color: 'var(--t3)', textAlign: 'center', background: 'var(--bg3)', borderRadius: 8 }}>No revisions uploaded yet</div>
              ) : (
                  ...
                  {activeDwg.revisions.map((r, i) => {
                    ...
                    return (
                      <div key={r.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderBottom: i < activeDwg.revisions.length - 1 ? '0.5px solid rgba(255,255,255,0.04)' : 'none', background: isSel ? 'rgba(139,92,246,0.10)' : 'transparent' }}>
                        <div style={{ flexShrink: 0, width: 32, height: 32, borderRadius: 8, background: i === 0 ? '#22c55e22' : '#52749a22', color: i === 0 ? '#22c55e' : 'var(--t3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'ui-monospace, monospace', fontSize: 11, fontWeight: 700 }}>{r.revision}</div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontFamily: SF, fontSize: 12, color: 'var(--t1)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.fileName || 'No file'}</div>
                          <div style={{ fontFamily: SF, fontSize: 10, color: 'var(--t3)', marginTop: 1 }}>
