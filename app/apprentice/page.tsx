...
export default function ApprenticeshipPage() {
  ...
  const create = async () => {
    const value = window.prompt('New apprentices — short label')
    if (!value) return
  ...
  const del = async (id: string) => {
    ...
    const res = await fetch(`/api/apprentice/${id}`, { method: 'DELETE' })
  ...
  return (
    <ModuleShell
      title="Apprentices"
      tagline="Apprenticeship tracking"
          ...
          {rows.map(r => (
            <li key={r.id} style={{ background: '#152641', borderRadius: 10, padding: '12px 14px', border: '0.5px solid rgba(255,255,255,0.07)', fontFamily: 'var(--font-system)', fontSize: 13, color: '#eef3fa', display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div>{[r.memberId, r.trade, r.nvqLevel].filter(Boolean).join(' · ') || r.id}</div>
