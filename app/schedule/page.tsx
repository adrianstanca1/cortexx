...
export default function SchedulePage() {
  ...
  return (
    <div className="module-page" style={{ background: 'var(--bg0)', minHeight: '100dvh', paddingBottom: 100 }}>
      ...
      <div className="module-header" data-kicker="Schedule command" style={{ padding: '20px 20px 12px 60px', position: 'sticky', top: 0, zIndex: 50, background: 'rgba(6,16,30,0.95)', backdropFilter: 'blur(12px)', borderBottom: '0.5px solid rgba(255,255,255,0.07)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--t1)', letterSpacing: -0.4, fontFamily: SF }}>Schedule</h1>
      ...
      {loading ? (
        ...
        <div style={{ overflowX: 'auto', paddingBottom: 20 }}>
          <div style={{ display: 'inline-block', minWidth: '100%' }}>
            ...
            <div style={{ display: 'flex', position: 'sticky', top: 0, background: 'var(--bg0)', zIndex: 5, borderBottom: '0.5px solid rgba(255,255,255,0.1)' }}>
              <div style={{ width: labelWidth, flexShrink: 0, padding: '6px 8px', fontFamily: SF, fontSize: 10, color: 'var(--t3)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5 }}>Project</div>
              {days.map((d, i) => {
                const isMonday = d.getDay() === 1
                const isToday = dayKey(d) === today
                const isWeekend = d.getDay() === 0 || d.getDay() === 6
                return (
                  <div key={i} style={{ width: colWidth, flexShrink: 0, padding: '2px 0', borderLeft: isMonday ? '1px solid rgba(255,255,255,0.12)' : '0.5px solid rgba(255,255,255,0.04)', background: isToday ? 'rgba(6,182,212,0.12)' : isWeekend ? 'rgba(255,255,255,0.02)' : 'transparent', textAlign: 'center', fontFamily: 'ui-monospace, monospace', fontSize: 9, color: isToday ? '#06b6d4' : isWeekend ? 'var(--t3)' : 'var(--t2)' }}>
                    {d.getDate()}
                  </div>
                )
              })}
            ...
            {rows.map(row => (
              <div key={row.project?.id || '_'} style={{ display: 'flex', flexDirection: 'column', borderBottom: '0.5px solid rgba(255,255,255,0.05)' }}>
                <div style={{ display: 'flex', alignItems: 'center', minHeight: 36 }}>
                  <div style={{ width: labelWidth, flexShrink: 0, padding: '6px 10px', fontFamily: SF, fontSize: 12, fontWeight: 600, color: 'var(--t1)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', position: 'sticky', left: 0, background: 'var(--bg0)', zIndex: 4, borderRight: '0.5px solid rgba(255,255,255,0.07)' }}>
                    {row.project?.name || 'Unassigned'}
                    <div style={{ fontFamily: SF, fontSize: 10, color: 'var(--t3)', marginTop: 1 }}>{row.tasks.length} task{row.tasks.length === 1 ? '' : 's'}</div>
                  </div>
                  <div style={{ position: 'relative', flex: 1, minHeight: 36 }}>
                    ...
                    <div style={{ position: 'relative', padding: '4px 0', display: 'flex', flexDirection: 'column', gap: 3 }}>
                      {row.tasks.map((t, ti) => {
                        ...
                        return (
                          <Link key={t.id} href={`/tasks?focus=${t.id}`} style={{ display: 'block', textDecoration: 'none', marginLeft: left, marginRight: 4, padding: '4px 8px', borderRadius: 6, background: bg, borderLeft: `3px solid ${fg}`, color: 'var(--t1)', fontFamily: SF, fontSize: 10, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: Math.max(colWidth * 4, 140) }}>
                            {t.title}
                          </Link>
                        )
                      })}
          ...
          <div style={{ padding: '16px 20px', display: 'flex', gap: 12, flexWrap: 'wrap', fontFamily: SF, fontSize: 11, color: 'var(--t3)' }}>
