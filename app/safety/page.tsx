...
const SEVERITY_COLOR: Record<Severity, string> = { near_miss: '#52749a', low: '#10b981', medium: '#f59e0b', high: '#ef4444', critical: '#dc2626' }
...
export default function SafetyPage() {
  ...
  return <div style={{ background: '#06101e', minHeight: '100dvh', paddingBottom: 100 }}>
    {toast && <Toast message={toast.msg} type={toast.type} onDone={() => setToast(null)} />}
    <div style={{ padding: '20px 20px 12px 60px', position: 'sticky', top: 0, zIndex: 50, background: 'rgba(6,16,30,0.95)', backdropFilter: 'blur(12px)', borderBottom: '0.5px solid rgba(255,255,255,0.07)' }}>
      <Link href="/apps" style={{ display: 'flex', alignItems: 'center', gap: 4, textDecoration: 'none', marginBottom: 10 }}><IcChevL size={18} color="#52749a" /><span style={{ fontFamily: SF, fontSize: 13, color: '#52749a' }}>Apps</span></Link>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}><div><h1 style={{ fontSize: 22, fontWeight: 700, color: '#eef3fa', fontFamily: SF }}>Safety</h1><p style={{ fontSize: 12, color: '#52749a', marginTop: 2, fontFamily: SF }}>Incident · investigation · RIDDOR · closeout</p></div><button onClick={() => setShowAdd(true)} style={primaryBtn('#ef4444')}><IcPlus size={12} color="#fff" /> Log incident</button></div>
    </div>
    ...
    {loading ? <State text="Loading…" /> : error ? <State text={error} color="#ef4444" /> : incidents.length === 0 ? <State text="No incidents in this view" /> : <div style={{ padding: '0 16px', display: 'grid', gap: 8 }}>
      {incidents.map(i => <button key={i.id} onClick={() => openIncident(i)} style={{ background: '#152641', borderRadius: 12, padding: 12, border: `0.5px solid ${i.riddorStatus === 'reportable' && i.status !== 'closed' ? '#f59e0b55' : 'rgba(255,255,255,0.07)'}`, display: 'flex', gap: 10, alignItems: 'flex-start', cursor: 'pointer', textAlign: 'left' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 6 }}><span style={{ fontFamily: SF, fontSize: 14, fontWeight: 700, color: '#eef3fa', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{i.title}</span><Badge text={STATUS_LABEL[i.status]} color={STATUS_COLOR[i.status]} /></div><div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 4 }}><span style={{ fontFamily: SF, fontSize: 10, fontWeight: 800, color: SEVERITY_COLOR[i.severity], textTransform: 'uppercase' }}>{i.severity.replace('_', ' ')}</span><span style={{ color: '#52749a' }}>·</span><span style={{ fontFamily: SF, fontSize: 10, color: '#8ea8c5' }}>{TYPE_LABEL[i.type]}</span>{i.riddorStatus !== 'not_assessed' && <Badge text={RIDDOR_LABEL[i.riddorStatus]} color="#f59e0b" />}</div><div style={{ fontFamily: SF, fontSize: 10, color: '#52749a', marginTop: 4 }}>{i.project?.name || 'No project'} · {new Date(i.occurredAt).toLocaleString('en-GB', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</div></div>
      </button>)}
    </div>}
    <TabBar />
    ...
    {active && <Modal onClose={() => setActive(null)}>
      ...
      {detailLoading ? <State text="Loading incident…" /> : <>
        {active.description && <p style={{ fontFamily: SF, fontSize: 13, color: '#eef3fa', lineHeight: 1.5 }}>{active.description}</p>}
        ...
        <div style={{ display: 'grid', gap: 7 }}>{(active.correctiveActions || []).length === 0 ? <div style={{ fontFamily: SF, fontSize: 11, color: '#52749a' }}>No corrective actions recorded.</div> : active.correctiveActions!.map(action => <div key={action.id} style={cardStyle}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}><div><div style={{ fontFamily: SF, fontSize: 12, fontWeight: 800, color: '#eef3fa' }}>{action.title}</div><div style={{ fontFamily: SF, fontSize: 10, color: '#8ea8c5', marginTop: 2 }}>{action.ownerName || 'Unassigned'}{action.dueDate ? ` · due ${new Date(action.dueDate).toLocaleDateString('en-GB')}` : ''}</div></div><Badge text={action.status.replace('_', ' ')} color={action.status === 'complete' ? '#10b981' : action.status === 'in_progress' ? '#f59e0b' : '#ef4444'} /></div>{action.description && <div style={{ fontFamily: SF, fontSize: 10, color: '#8ea8c5', marginTop: 6 }}>{action.description}</div>}{active.status !== 'closed' && <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>{action.status === 'open' && <button onClick={() => updateCorrectiveAction(action, 'in_progress')} style={miniBtn('#f59e0b')}>Start</button>}{action.status !== 'complete' && <button onClick={() => updateCorrectiveAction(action, 'complete')} style={miniBtn('#10b981')}>Complete</button>}</div>}</div>)}</div>
...
function Modal({ children, onClose }: { children: React.ReactNode; onClose: () => void }) { return <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}><div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.68)', backdropFilter: 'blur(4px)' }} /><div style={{ position: 'relative', background: '#152641', borderRadius: '20px 20px 0 0', padding: '22px 20px 38px', display: 'grid', gap: 11, maxHeight: '92dvh', overflowY: 'auto' }}>{children}</div></div> }
function Header({ title, subtitle, onClose }: { title: string; subtitle?: string; onClose: () => void }) { return <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'flex-start' }}><div><h2 style={{ fontFamily: SF, fontSize: 19, color: '#eef3fa' }}>{title}</h2>{subtitle && <div style={{ fontFamily: SF, fontSize: 11, color: '#8ea8c5', marginTop: 2 }}>{subtitle}</div>}</div><button onClick={onClose} aria-label="Close" style={{ background: 'transparent', border: 0, cursor: 'pointer' }}><IcX size={19} color="#8ea8c5" /></button></div> }
function KPI({ icon, label, value, color }: { icon: React.ReactNode; label: string; value: string; color: string }) { return <div style={cardStyle}><div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>{icon}<span style={{ fontFamily: SF, fontSize: 9, color: '#8ea8c5', fontWeight: 800, textTransform: 'uppercase' }}>{label}</span></div><div style={{ fontFamily: 'ui-monospace, monospace', fontSize: 18, color, fontWeight: 800, marginTop: 2 }}>{value}</div></div> }
function DetailRow({ label, value }: { label: string; value: string }) { return <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, fontFamily: SF, fontSize: 11 }}><span style={{ color: '#8ea8c5' }}>{label}</span><span style={{ color: '#eef3fa', textAlign: 'right' }}>{value}</span></div> }
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label style={{ display: 'grid', gap: 5 }}><span style={{ fontFamily: SF, fontSize: 9, color: '#52749a', fontWeight: 800, textTransform: 'uppercase' }}>{label}</span>{children}</label> }
function SectionTitle({ text }: { text: string }) { return <div style={{ fontFamily: SF, fontSize: 10, color: '#8ea8c5', fontWeight: 900, textTransform: 'uppercase', letterSpacing: 0.6, marginTop: 4 }}>{text}</div> }
...
function State({ text, color = '#52749a' }: { text: string; color?: string }) { return <div style={{ padding: 40, textAlign: 'center', color, fontFamily: SF, fontSize: 13 }}>{text}</div> }
function chipBtn(active: boolean): React.CSSProperties { return { padding: '6px 12px', borderRadius: 99, background: active ? '#152641' : 'rgba(255,255,255,0.04)', border: `0.5px solid ${active ? 'rgba(239,68,68,0.4)' : 'rgba(255,255,255,0.07)'}`, color: active ? '#eef3fa' : '#52749a', fontFamily: SF, fontSize: 10, fontWeight: 800, textTransform: 'uppercase', cursor: 'pointer', flexShrink: 0 } }
function selectBtn(active: boolean, color: string): React.CSSProperties { return { padding: '7px 9px', borderRadius: 8, background: active ? color + '22' : 'rgba(255,255,255,0.04)', border: `0.5px solid ${active ? color : 'rgba(255,255,255,0.10)'}`, color: active ? color : '#8ea8c5', fontFamily: SF, fontSize: 10, fontWeight: 800, cursor: 'pointer', textTransform: 'uppercase' } }
