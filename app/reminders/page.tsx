...
export default function ReminderPage() {
  ...
  const create = async () => {
    const value = window.prompt('New reminders — short label')
    if (!value) return
  ...
  return (
    <ModuleShell
      title="Reminders"
      tagline="Quick reminders separate from project tasks"
          ...
          {rows.map(r => (
            <li key={r.id} style={{ background: '#152641', borderRadius: 10, padding: '12px 14px', border: '0.5px solid rgba(255,255,255,0.07)', fontFamily: 'var(--font-system)', fontSize: 13, color: '#eef3fa', display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div>{[r.title, r.dueAt, r.memberId].filter(Boolean).join(' · ') || r.id}</div>
