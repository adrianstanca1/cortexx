...
const STATUS_COLOR: Record<BankTransaction['status'], string> = { unmatched: '#f59e0b', partial: '#3b82f6', reconciled: '#10b981', ignored: '#52749a' }
  ...
  return <div style={{ minHeight: '100dvh', background: '#06101e', paddingBottom: 96 }}>
    ...
    <header style={{ padding: '18px 20px 12px 60px', position: 'sticky', top: 0, zIndex: 50, background: 'rgba(6,16,30,0.95)', backdropFilter: 'blur(12px)', borderBottom: '0.5px solid rgba(255,255,255,0.07)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
        <div><h1 style={{ fontFamily: SF, fontSize: 22, fontWeight: 800, color: '#eef3fa' }}>Bank reconciliation</h1><p style={{ fontFamily: SF, fontSize: 12, color: '#8ea8c5', marginTop: 2 }}>Bank feed → invoices, valuations and subcontract payments</p></div>
        <button type="button" onClick={() => setShowAdd(true)} style={roundAction}><IcPlus size={18} color="#fff" /></button>
      </div>
      <div style={{ display: 'flex', gap: 6, marginTop: 10, flexWrap: 'wrap' }}>
        <button type="button" onClick={connectBank} disabled={connecting} style={pillBtn(bankingConfigured ? '#2563eb' : '#52749a')}>{connecting ? 'Connecting…' : bankingConfigured ? 'Connect bank' : 'Bank feed not configured'}</button>
    ...
    <section style={{ display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 8, padding: '14px 16px 0' }}>
      ...
      <Kpi label="To allocate" value={money(summary.unallocatedValue)} color="#eef3fa" />
    </section>

    <div style={{ display: 'flex', gap: 6, overflowX: 'auto', padding: '12px 16px 4px' }}>{(['all', 'unmatched', 'partial', 'reconciled', 'ignored'] as const).map(s => <button key={s} type="button" onClick={() => setFilter(s)} style={{ ...pillBtn(filter === s ? '#2563eb' : '#334b68'), color: filter === s ? '#fff' : '#8ea8c5' }}>{s === 'all' ? 'All' : s[0].toUpperCase() + s.slice(1)}</button>)}</div>
    ...
    {loading ? <div style={emptyStyle}>Loading bank transactions…</div> : visible.length === 0 ? <div style={emptyStyle}>No transactions in this view.</div> : <main style={{ padding: '10px 16px', display: 'grid', gap: 8 }}>
      {visible.map(item => {
        ...
        return <button key={item.id} type="button" onClick={() => openTransaction(item)} style={{ background: '#152641', border: '0.5px solid rgba(255,255,255,0.07)', borderRadius: 13, padding: 13, textAlign: 'left', cursor: 'pointer' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}><div style={{ minWidth: 0 }}><div style={{ fontFamily: SF, fontSize: 14, color: '#eef3fa', fontWeight: 750, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{item.description || item.reference || item.accountName || 'Bank transaction'}</div><div style={{ fontFamily: SF, fontSize: 10, color: '#8ea8c5', marginTop: 3 }}>{item.occurredAt ? new Date(item.occurredAt).toLocaleDateString('en-GB') : 'No date'} · {item.accountName || item.source}</div></div><div style={{ textAlign: 'right', flexShrink: 0 }}><div style={{ fontFamily: 'ui-monospace, monospace', fontSize: 15, fontWeight: 850, color: credit ? '#10b981' : '#eef3fa' }}>{credit ? '+' : '−'}{money(Math.abs(Number(item.amount) || 0), item.currency)}</div><span style={{ display: 'inline-block', marginTop: 4, borderRadius: 99, padding: '2px 7px', background: STATUS_COLOR[item.status] + '20', color: STATUS_COLOR[item.status], fontFamily: SF, fontSize: 8, fontWeight: 900, textTransform: 'uppercase' }}>{item.status}</span></div></div>
          {item.status === 'partial' && <div style={{ marginTop: 7, fontFamily: SF, fontSize: 10, color: '#8ea8c5' }}>{money(item.reconciliation.allocated)} allocated · {money(item.reconciliation.remaining)} remaining</div>}
        </button>
      })}
    </main>}
    <TabBar />
    ...
    {selected && <Modal onClose={() => setSelected(null)} title="Reconcile transaction">
      <div style={{ background: '#1a2f4e', borderRadius: 11, padding: 12 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}><div><div style={{ fontFamily: SF, fontSize: 13, fontWeight: 800, color: '#eef3fa' }}>{selected.description || selected.reference || 'Bank transaction'}</div><div style={{ fontFamily: SF, fontSize: 10, color: '#8ea8c5', marginTop: 3 }}>{selected.occurredAt ? new Date(selected.occurredAt).toLocaleDateString('en-GB') : ''} · {selected.accountName || selected.source}</div></div><strong style={{ fontFamily: 'ui-monospace, monospace', color: Number(selected.amount) > 0 ? '#10b981' : '#eef3fa' }}>{money(Number(selected.amount) || 0, selected.currency)}</strong></div><div style={{ fontFamily: SF, fontSize: 10, color: '#8ea8c5', marginTop: 8 }}>{money(selected.reconciliation.allocated)} allocated · {money(selected.reconciliation.remaining)} remaining</div></div>

      {allocations.length > 0 && <div><div style={sectionLabel}>Current matches</div><div style={{ display: 'grid', gap: 6 }}>{allocations.map(a => <div key={a.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', background: '#0f1d31', borderRadius: 9, padding: '8px 10px' }}><div><div style={{ fontFamily: SF, fontSize: 11, color: '#eef3fa', fontWeight: 700 }}>{a.targetLabel || a.targetId}</div><div style={{ fontFamily: SF, fontSize: 9, color: '#8ea8c5' }}>{a.targetType.replaceAll('_', ' ')} · {money(a.amount)}</div></div><button type="button" onClick={() => removeAllocation(a)} style={smallDanger}>Remove</button></div>)}</div></div>}

      {selected.status !== 'ignored' && selected.reconciliation.remaining > 0.009 && <div><div style={sectionLabel}>Available matches</div>{candidateLoading ? <div style={{ fontFamily: SF, fontSize: 11, color: '#8ea8c5' }}>Finding candidates…</div> : candidates.length === 0 ? <div style={{ fontFamily: SF, fontSize: 11, color: '#8ea8c5' }}>No outstanding matching targets found.</div> : <div style={{ display: 'grid', gap: 6, maxHeight: 280, overflowY: 'auto' }}>{candidates.slice(0, 40).map(c => <button key={`${c.targetType}:${c.targetId}`} type="button" disabled={saving} onClick={() => matchCandidate(c)} style={{ background: '#0f1d31', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 9, padding: '9px 10px', textAlign: 'left', cursor: 'pointer' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}><div><div style={{ fontFamily: SF, fontSize: 11, fontWeight: 750, color: '#eef3fa' }}>{c.label}</div><div style={{ fontFamily: SF, fontSize: 9, color: '#8ea8c5', marginTop: 2 }}>{c.project || 'No project'}{c.purchaseOrder ? ` · ${c.purchaseOrder}` : ''}{c.costCode ? ` · ${c.costCode}` : ''}</div></div><strong style={{ fontFamily: 'ui-monospace, monospace', fontSize: 11, color: '#f59e0b' }}>{money(c.outstanding)}</strong></div></button>)}</div>}</div>}
...
function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) { return <div style={{ position: 'fixed', inset: 0, zIndex: 220, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}><div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.68)' }} /><div style={{ position: 'relative', background: '#152641', borderRadius: '20px 20px 0 0', padding: '22px 20px 34px', maxHeight: '92dvh', overflowY: 'auto', display: 'grid', gap: 11 }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><h2 style={{ fontFamily: SF, fontSize: 19, color: '#eef3fa' }}>{title}</h2><button type="button" onClick={onClose} aria-label="Close" style={{ background: 'transparent', border: 0, cursor: 'pointer' }}><IcX size={19} color="#8ea8c5" /></button></div>{children}</div></div> }
function Kpi({ label, value, color }: { label: string; value: string; color: string }) { return <div style={{ background: '#152641', border: '0.5px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: 11 }}><div style={{ fontFamily: SF, fontSize: 8, fontWeight: 900, color: '#52749a', textTransform: 'uppercase' }}>{label}</div><div style={{ fontFamily: 'ui-monospace, monospace', fontSize: 15, fontWeight: 800, color, marginTop: 3 }}>{value}</div></div> }
...
const emptyStyle: React.CSSProperties = { padding: 44, textAlign: 'center', color: '#52749a', fontFamily: SF, fontSize: 13 }
...
const secondaryBtn: React.CSSProperties = { minHeight: 40, border: '1px solid rgba(255,255,255,0.12)', borderRadius: 10, background: 'rgba(255,255,255,0.04)', color: '#8ea8c5', fontFamily: SF, fontSize: 11, fontWeight: 800, cursor: 'pointer' }
...
const sectionLabel: React.CSSProperties = { fontFamily: SF, fontSize: 9, color: '#8ea8c5', fontWeight: 900, textTransform: 'uppercase', marginBottom: 6 }
