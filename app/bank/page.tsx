...
export default function BankTransactionPage() {
  ...
  const create = async () => {
    const title = window.prompt('New bank reconciliation — short label')
    if (!title) return
    const res = await fetch('/api/bank', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ accountName: title }),
    })
    if (!res.ok) {
  ...
  return (
    <ModuleShell
      ...
      tagline="Reconcile bank statements against invoices"
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {rows.map(r => (
            <li key={r.id} style={{ background: '#152641', borderRadius: 10, padding: '12px 14px', border: '0.5px solid rgba(255,255,255,0.07)', fontFamily: 'var(--font-system)', fontSize: 13, color: '#eef3fa' }}>
              <div>{[r.accountName, r.occurredAt, r.amount].filter(Boolean).join(' · ') || r.id}</div>
