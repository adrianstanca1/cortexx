...
export default function LiveStatusPage() {
  ...
  return (
    <div className="module-page" style={{ background: 'var(--bg0)', minHeight: '100dvh', paddingBottom: 100 }}>
      <div className="module-header" data-kicker="Live Status command" style={{ padding: '20px 20px 12px 60px', position: 'sticky', top: 0, zIndex: 50, background: 'rgba(6,16,30,0.95)', backdropFilter: 'blur(12px)', borderBottom: '0.5px solid rgba(255,255,255,0.07)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: 'var(--t1)', letterSpacing: -0.4, fontFamily: SF }}>Live status</h1>
            <p style={{ fontSize: 12, color: 'var(--t3)', marginTop: 2, fontFamily: SF }}>Auto-refreshes every 30s</p>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6 }}>
          ...
          <Kpi label="Off site" value={totals.offSite} color="var(--t3)" />
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              {sitesOccupied.map(g => (
                <div key={g.project.id} style={{ padding: '12px 14px', borderBottom: '0.5px solid rgba(255,255,255,0.04)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                    ...
                    <Link href={`/projects/${g.project.id}`} style={{ fontFamily: SF, fontSize: 14, fontWeight: 600, color: 'var(--t1)', textDecoration: 'none' }}>{g.project.name}</Link>
                    <span style={{ marginLeft: 'auto', fontFamily: 'ui-monospace, monospace', fontSize: 11, color: 'var(--t3)' }}>{g.checkins.length} on site</span>
                  </div>
                    ...
                    {g.checkins.map(ci => {
                      ...
                      return (
                        <div key={ci.id} title={`${ci.member?.name} · ${hrs.toFixed(1)}h on site`} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '3px 8px 3px 3px', background: (ci.member?.avatarColor || '#2563eb') + '15', borderRadius: 99, border: `0.5px solid ${(ci.member?.avatarColor || '#2563eb')}44` }}>
                          ...
                          <span style={{ fontFamily: SF, fontSize: 11, color: 'var(--t1)', fontWeight: 500 }}>{ci.member?.name.split(' ')[0]}</span>
                          <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: 10, color: 'var(--t3)', marginLeft: 2 }}>{hrs.toFixed(1)}h</span>
                        </div>
                      )
                    })}
                  </div>
              ...
              {sitesEmpty.map(g => (
                <Link key={g.project.id} href={`/projects/${g.project.id}`} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', borderBottom: '0.5px solid rgba(255,255,255,0.04)', textDecoration: 'none' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontFamily: SF, fontSize: 13, color: 'var(--t1)' }}>{g.project.name}</div>
                    {g.project.address && <div style={{ fontFamily: SF, fontSize: 11, color: 'var(--t3)' }}>{g.project.address}{g.project.postcode ? `, ${g.project.postcode}` : ''}</div>}
                  </div>
                </Link>
              ))}
            </Section>
          ...
          {offSite.length > 0 && (
            <Section title={`Off site · ${offSite.length}`} color="var(--t3)">
              <div style={{ padding: 10, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                {offSite.map(m => (
                  <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '3px 8px 3px 3px', background: 'rgba(255,255,255,0.03)', borderRadius: 99, border: '0.5px solid rgba(255,255,255,0.07)' }}>
                    <div style={{ width: 20, height: 20, borderRadius: 10, background: m.avatarColor || 'var(--t3)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: SF, fontSize: 9, fontWeight: 700 }}>
                      {m.name.slice(0, 2).toUpperCase()}
                    </div>
                    <span style={{ fontFamily: SF, fontSize: 11, color: 'var(--t2)' }}>{m.name.split(' ')[0]}</span>
                  </div>
                ))}
              </div>
            </Section>
...
function Kpi({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div style={{ background: 'var(--surface-raised)', borderRadius: 10, padding: '8px 10px', border: '0.5px solid rgba(255,255,255,0.07)', textAlign: 'center' }}>
      <div style={{ fontFamily: 'ui-monospace, monospace', fontSize: 20, color, fontWeight: 700 }}>{value}</div>
      <div style={{ fontFamily: SF, fontSize: 9, color: 'var(--t3)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, marginTop: 1 }}>{label}</div>
    </div>
  )
}
