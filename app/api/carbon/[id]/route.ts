...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const item = await prisma.carbonEntry.findUnique({ where: { id } })
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const item = await prisma.carbonEntry.update({
      where: { id },
      data: {
      ...
      ...(typeof body.co2eKg === 'number' ? { co2eKg: body.co2eKg } : {}),
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await auditLog({
      ...
      action: 'carbon.delete',
      resourceType: 'carbonEntry',
      resourceId: id,
    })
