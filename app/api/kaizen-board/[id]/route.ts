...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const item = await prisma.kaizenCard.findUnique({ where: { id } })
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const item = await prisma.kaizenCard.update({
      where: { id },
      data: {
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  try {
    await prisma.kaizenCard.delete({ where: { id } })
    await auditLog({
      userId: (auth.user as { id?: string }).id,
      action: 'kaizen-board.delete',
      resourceType: 'kaizenCard',
      resourceId: id,
    })
