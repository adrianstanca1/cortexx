...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const items = await prisma.appraisal.findMany({
      orderBy: { createdAt: 'desc' },
      take, skip,
    })
    const total = await prisma.appraisal.count()
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const item = await prisma.appraisal.create({
      data: {
      ...(typeof body.memberId === 'string' ? { memberId: body.memberId.trim() } : {}),
      ...(typeof body.reviewerId === 'string' ? { reviewerId: body.reviewerId.trim() } : {}),
      ...(typeof body.periodLabel === 'string' ? { periodLabel: body.periodLabel.trim() } : {}),
      ...(typeof body.rating === 'number' ? { rating: body.rating } : {}),
      ...(typeof body.strengths === 'string' ? { strengths: body.strengths.trim() } : {}),
      ...(typeof body.improvements === 'string' ? { improvements: body.improvements.trim() } : {}),
