...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const item = await prisma.improvement.findUnique({ where: { id } })
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const item = await prisma.improvement.update({
      ...
      ...(typeof body.raisedBy === 'string' ? { raisedBy: body.raisedBy.trim() } : {}),
      ...(typeof body.status === 'string' ? { status: body.status.trim() } : {}),
      ...(typeof body.impact === 'string' ? { impact: body.impact.trim() } : {}),
      ...(typeof body.effort === 'string' ? { effort: body.effort.trim() } : {}),
      },
    })
    return NextResponse.json({ item })
  } catch (error) {
