...
async function runSeed(): Promise<NextResponse> {
  try {
    ...
    const camden = await prisma.project.create({ data: { name: 'Camden Mews Refurb', address: '12 Camden Mews', postcode: 'NW1 8AF', status: 'active', progress: 68, clientName: 'J. Patterson', budget: 85000, spent: 57800, lat: 51.5388, lng: -0.1426, onSiteCount: 4, startDate: new Date('2024-02-01'), endDate: new Date('2024-05-10') } })
    const hackney = await prisma.project.create({ data: { name: 'Hackney Loft Conversion', address: '47 Dalston Lane', postcode: 'E8 3DF', status: 'active', progress: 22, clientName: 'E. Lin', budget: 65000, spent: 14300, lat: 51.5477, lng: -0.0758, onSiteCount: 2, startDate: new Date('2024-03-15'), endDate: new Date('2024-07-30') } })
    const brixton = await prisma.project.create({ data: { name: 'Brixton Shopfront', address: '221 Coldharbour Lane', postcode: 'SW9 8RU', status: 'snagging', progress: 90, clientName: 'Tonic Café Ltd', budget: 32000, spent: 29500, lat: 51.4613, lng: -0.1169, onSiteCount: 3, startDate: new Date('2024-01-10'), endDate: new Date('2024-04-28') } })
    ...
    const wk = Math.ceil((now.getTime() - new Date(now.getFullYear(),0,1).getTime()) / (7*86400000))
    await prisma.timeEntry.createMany({ data: [
      { memberId: tom.id, projectId: camden.id, date: weekStart, hours: 42.5, week: wk, year: now.getFullYear(), approved: false },
      { memberId: aisha.id, projectId: camden.id, date: weekStart, hours: 38, week: wk, year: now.getFullYear(), approved: true },
      { memberId: jack.id, projectId: camden.id, date: weekStart, hours: 32, week: wk, year: now.getFullYear(), approved: false },
      { memberId: sara.id, projectId: camden.id, date: weekStart, hours: 40, week: wk, year: now.getFullYear(), approved: false },
      { memberId: mike.id, projectId: hackney.id, date: weekStart, hours: 28, week: wk, year: now.getFullYear(), approved: true },
      { memberId: priya.id, projectId: islington.id, date: weekStart, hours: 12, week: wk, year: now.getFullYear(), approved: true },
    ]})
