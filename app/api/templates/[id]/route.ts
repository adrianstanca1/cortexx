...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const item = await prisma.documentTemplate.findUnique({ where: { id } })
...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const item = await prisma.documentTemplate.update({
      where: { id },
      data: {
      ...(typeof body.name === 'string' ? { name: body.name.trim() } : {}),
      ...(typeof body.category === 'string' ? { category: body.category.trim() } : {}),
      ...(typeof body.body === 'string' ? { body: body.body.trim() } : {}),
      ...(typeof body.placeholders === 'string' ? { placeholders: body.placeholders.trim() } : {}),
