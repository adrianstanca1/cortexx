...
export default function WasteEntryPage() {
  ...
  const create = async () => {
    const title = window.prompt('New waste log — short label')
    if (!title) return
  ...
  return (
    <ModuleShell
      title="Waste log"
      tagline="DEFRA-compliant waste tickets"
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {rows.map(r => (
            <li key={r.id} style={{ background: '#152641', borderRadius: 10, padding: '12px 14px', border: '0.5px solid rgba(255,255,255,0.07)', fontFamily: 'var(--font-system)', fontSize: 13, color: '#eef3fa' }}>
              <div>{[r.projectId, r.wasteType, r.weightKg].filter(Boolean).join(' · ') || r.id}</div>
