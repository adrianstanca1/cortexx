...
export default function PhotosPage() {
  ...
  return (
    <div className="module-page" style={{ background: 'var(--bg0)', minHeight: '100dvh', paddingBottom: 100 }}>
      ...
      <div className="module-header" data-kicker="Photos command" style={{ padding: '20px 20px 12px 60px', position: 'sticky', top: 0, zIndex: 50, background: 'rgba(6,16,30,0.95)', backdropFilter: 'blur(12px)', borderBottom: '0.5px solid rgba(255,255,255,0.07)' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(6,16,30,0.95)', display: 'flex', flexDirection: 'column', overflow: 'auto' }}>
          ...
          <div style={{ padding: '20px 16px 16px', maxWidth: 800, margin: '0 auto', width: '100%' }}>
            ...
            <div style={{ fontFamily: SF, fontSize: 13, color: 'var(--t1)', marginBottom: 14 }}>
              <span style={{ padding: '3px 8px', borderRadius: 99, background: compareResult.progress === 'progressed' ? 'rgba(16,185,129,0.2)' : compareResult.progress === 'reversed' ? 'rgba(239,68,68,0.2)' : compareResult.progress === 'stalled' ? 'rgba(245,158,11,0.2)' : 'rgba(82,116,154,0.2)', color: compareResult.progress === 'progressed' ? '#10b981' : compareResult.progress === 'reversed' ? '#ef4444' : compareResult.progress === 'stalled' ? '#f59e0b' : 'var(--t2)', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, marginRight: 6 }}>{compareResult.progress}</span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
              {[compareResult.earlier, compareResult.later].map((photo, idx) => (
                <div key={photo.id}>
                  <div style={{ aspectRatio: '1 / 1', borderRadius: 10, overflow: 'hidden', background: 'var(--bg1)' }}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={photo.url || ''} alt={photo.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  </div>
                  <div style={{ fontFamily: SF, fontSize: 10, color: 'var(--t3)', marginTop: 4, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.4 }}>
            ...
            {compareResult.changes.length > 0 && (
              <div>
                <div style={{ fontFamily: SF, fontSize: 11, color: 'var(--t2)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 8 }}>What changed</div>
                <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {compareResult.changes.map((c, i) => (
                    <li key={i} style={{ display: 'flex', gap: 8, padding: '8px 10px', background: 'rgba(139,92,246,0.06)', borderRadius: 8, borderLeft: '2px solid #a78bfa' }}>
                      ...
                      <span style={{ fontFamily: SF, fontSize: 12, color: 'var(--t1)', lineHeight: 1.45 }}>{c}</span>
                    </li>
                  ))}
                </ul>
              </div>
            ...
            {compareResult.notes && (
              <div style={{ fontFamily: SF, fontSize: 11, color: 'var(--t2)', fontStyle: 'italic', marginTop: 10, padding: '8px 10px', background: 'rgba(255,255,255,0.03)', borderRadius: 6 }}>Note: {compareResult.notes}</div>
            )}
          </div>
        </div>
      )}
      ...
      {activePhoto && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(6,16,30,0.95)', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ background: 'var(--surface-raised)', padding: '16px 20px 24px', borderTop: '0.5px solid rgba(255,255,255,0.08)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12, marginBottom: 12 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontFamily: SF, fontSize: 14, fontWeight: 700, color: 'var(--t1)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{activePhoto.name}</div>
            ...
            {photoTags[activePhoto.id] && !photoTags[activePhoto.id].loading && (
              <div>
                {photoTags[activePhoto.id].error ? (
                  ...
                  <>
                    <div style={{ fontFamily: SF, fontSize: 11, color: 'var(--t2)', marginBottom: 6 }}>
            ...
            <a href={activePhoto.url!} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', marginTop: 10, fontFamily: SF, fontSize: 11, color: 'var(--t3)', textDecoration: 'underline' }}>Open original ↗</a>
          </div>
        </div>
      )}
