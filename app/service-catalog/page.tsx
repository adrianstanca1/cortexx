...
export default function ServiceCatalogItemPage() {
  ...
  const create = async () => {
    const value = window.prompt('New service catalog — short label')
    if (!value) return
  ...
  return (
    <ModuleShell
      ...
      tagline="Reusable service / package definitions"
      ...
      {loading ? (
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {rows.map(r => (
            ...
            >
              <div>{[r.name, r.category, r.unitPrice].filter(Boolean).join(' · ') || r.id}</div>
