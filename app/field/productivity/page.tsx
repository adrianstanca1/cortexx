    ...
    <div style={{ minHeight: '100dvh', background: '#06101e', paddingBottom: 100 }}>
      ...
      <main style={{ padding: '14px 16px 0' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 14 }}>
          <Metric label="Plan" value={summary ? fmt(summary.plannedQty) : '—'} color="#8ea8c5" />
                ...
                return (
                  <div key={area.label} style={{ background: '#0b1a30', borderRadius: 10, padding: 10 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}>
                      <div style={{ color: '#eef3fa', fontFamily: SF, fontSize: 11.5, fontWeight: 900 }}>{area.label}</div>
                      <div style={{ color: tone, fontFamily: SF, fontSize: 11, fontWeight: 900 }}>{area.pct == null ? 'No plan' : `${area.pct}%`}</div>
                    </div>
                    <div style={{ height: 6, background: '#152641', borderRadius: 99, marginTop: 7, overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${Math.min(100, Math.max(0,pct))}%`, background: tone }} />
                    </div>
                    <div style={{ color: '#8ea8c5', fontFamily: SF, fontSize: 9.8, marginTop: 6 }}>
          ...
          <section style={panelStyle}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 9 }}>
              {logs.slice(0,50).map(row => {
                ...
                const tone = pct == null ? '#8ea8c5' : pct >= 100 ? '#10b981' : pct >= 80 ? '#f59e0b' : '#ef4444'
                return (
                  <div key={row.id} style={{ background: '#0b1a30', borderRadius: 10, padding: 10, borderLeft: `3px solid ${tone}` }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}>
                      <div>
                        <div style={{ color: '#eef3fa', fontFamily: SF, fontSize: 11.5, fontWeight: 900 }}>{row.activity}</div>
                        <div style={{ color: '#8ea8c5', fontFamily: SF, fontSize: 9.8, marginTop: 3 }}>{row.area}{row.elevation ? ` · ${row.elevation}` : ''} · {new Date(row.date).toLocaleDateString('en-GB')}</div>
                      </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 6, marginTop: 8 }}>
                      ...
                      <Mini icon={<IcLayers size={11} color="#8ea8c5" />} label="Plan" value={`${fmt(row.plannedQty)} ${row.unit}`} />
...
function Mini({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div style={{ background: '#102039', borderRadius: 8, padding: 7 }}><div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>{icon}<span style={{ color: '#52749a', fontFamily: SF, fontSize: 8.8, textTransform: 'uppercase', fontWeight: 900 }}>{label}</span></div><div style={{ color: '#eef3fa', fontFamily: SF, fontSize: 10.5, fontWeight: 800, marginTop: 3 }}>{value}</div></div>
}
...
const metricStyle: React.CSSProperties = { borderRadius: 11, background: '#102039', border: '1px solid rgba(255,255,255,.07)', padding: 9 }
const metricLabel: React.CSSProperties = { color: '#8ea8c5', fontFamily: SF, fontSize: 8.8, fontWeight: 800, textTransform: 'uppercase', marginTop: 2 }
const sectionTitle: React.CSSProperties = { color: '#8ea8c5', fontFamily: SF, fontSize: 10.5, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '.07em' }
