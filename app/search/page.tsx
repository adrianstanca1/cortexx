...
const invStatusColor: Record<string, string> = { draft: 'var(--t3)', sent: '#f59e0b', paid: '#10b981', overdue: '#ef4444' }
  ...
  return (
    <div className="module-page" style={{ background: 'var(--bg0)', minHeight: '100dvh', paddingBottom: 100 }}>
      ...
      <div style={{ padding: '16px 56px 12px 60px', position: 'sticky', top: 0, zIndex: 10, background: 'rgba(6,16,30,0.95)', backdropFilter: 'blur(12px)', borderBottom: '0.5px solid rgba(255,255,255,0.07)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'var(--surface-raised)', border: '0.5px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '12px 14px' }}>
          <IcSearch size={18} color="var(--t3)" />
          <input
            ref={inputRef}
            value={q}
            onChange={e => setQ(e.target.value)}
            placeholder="Search projects, tasks, team, invoices, snags, RFIs, docs…"
            style={{ background: 'none', border: 'none', outline: 'none', color: 'var(--t1)', fontFamily: 'var(--font-system)', fontSize: 15, flex: 1 }}
          />
        ...
        {results && (
          <p style={{ fontFamily: 'var(--font-system)', fontSize: 11, color: 'var(--t3)', marginTop: 8 }}>
      ...
      <div style={{ padding: '16px 20px' }}>
        {!q && (
          <p style={{ padding: '40px 0', textAlign: 'center', color: 'var(--t3)', fontFamily: 'var(--font-system)', fontSize: 14 }}>
            Type at least 2 characters to search across the workspace.
          </p>
        )}

        {q && results && results.total === 0 && !loading && (
          <div style={{ padding: '60px 20px', textAlign: 'center', color: 'var(--t3)', fontFamily: 'var(--font-system)' }}>
            <div style={{ fontSize: 48, marginBottom: 8 }}>🔍</div>
            <p style={{ fontSize: 14, color: 'var(--t1)' }}>No matches for &ldquo;{q}&rdquo;</p>
          ...
          <Section title="Projects" count={results.projects.length}>
            {results.projects.map(p => (
              <button key={p.id} onClick={() => router.push(`/projects/${p.id}`)} style={cardStyle}>
                ...
                <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: 12, color: 'var(--t2)', fontWeight: 700 }}>{p.progress}%</span>
              </button>
            ))}
          </Section>
        )}
          ...
          <Section title="Invoices" count={results.invoices.length}>
            {results.invoices.map(i => (
              <button key={i.id} onClick={() => router.push(i.projectId ? `/projects/${i.projectId}` : '/projects')} style={cardStyle}>
                ...
                <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: 12, color: 'var(--t1)', fontWeight: 700 }}>£{i.amount.toLocaleString()}</span>
              </button>
            ))}
          </Section>
        )}
            ...
            {results.goals.map(g => (
              <button key={g.id} onClick={() => router.push('/goals')} style={cardStyle}>
                ...
                {typeof g.progress === 'number' && (
                  <span style={{ fontFamily: 'ui-monospace, monospace', fontSize: 12, color: 'var(--t2)', fontWeight: 700 }}>{g.progress}%</span>
                )}
              </button>
            ))}
          </Section>
  ...
  return (
    <section style={{ marginBottom: 16 }}>
      <p style={{ fontFamily: 'var(--font-system)', fontSize: 11, fontWeight: 700, color: 'var(--t3)', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8 }}>
        {title} <span style={{ color: 'var(--t2)' }}>· {count}</span>
      </p>
