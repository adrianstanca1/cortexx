...
export default function SecurityPage() {
  ...
  return (
    <div style={{ background: 'var(--bg0)', minHeight: '100dvh', padding: '20px 20px 100px 60px' }}>
      <Link href="/settings" style={{ display: 'inline-block', marginBottom: 12, fontFamily: 'var(--font-system)', fontSize: 13, color: 'var(--t3)', textDecoration: 'none' }}>
        ← Settings
      </Link>
      ...
      <p style={{ fontSize: 13, color: 'var(--t2)', fontFamily: 'var(--font-system)', marginBottom: 24, lineHeight: 1.5 }}>
      ...
      {stage === 'idle' && !enrolled && (
        <section style={sectionStyle}>
          <div style={{ fontFamily: 'var(--font-system)', fontSize: 14, color: 'var(--t1)', marginBottom: 14 }}>
      ...
      {stage === 'enrolling' && setupData && (
        <section style={sectionStyle}>
          <div style={{ fontFamily: 'var(--font-system)', fontSize: 13, color: 'var(--t2)', marginBottom: 14, lineHeight: 1.5 }}>
            1. Open your authenticator app and tap <strong style={{ color: 'var(--t1)' }}>Add account → Scan QR</strong>.<br />
            2. Scan this code:
          </div>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={setupData.qrPng} alt="TOTP QR code" style={{ display: 'block', margin: '12px auto', borderRadius: 12, background: 'var(--t1)', padding: 8 }} />
          <details style={{ fontFamily: 'var(--font-system)', fontSize: 12, color: 'var(--t3)', marginTop: 8 }}>
            <summary style={{ cursor: 'pointer' }}>Can&apos;t scan? Enter the secret manually</summary>
            <code style={{ display: 'block', marginTop: 8, padding: 10, background: 'var(--bg3)', borderRadius: 8, color: 'var(--t1)', fontSize: 13, wordBreak: 'break-all' }}>
      ...
      {stage === 'enabled' && enableResult && (
        <section style={{ ...sectionStyle, borderColor: 'rgba(16,185,129,0.4)' }}>
          ...
          <p style={{ fontFamily: 'var(--font-system)', fontSize: 13, color: 'var(--t2)', lineHeight: 1.5, marginBottom: 14 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8, fontFamily: 'ui-monospace, monospace', fontSize: 13 }}>
            {enableResult.backupCodes.map(c => (
              <div key={c} style={{ padding: '8px 10px', background: 'var(--bg3)', borderRadius: 6, color: 'var(--t1)', textAlign: 'center', letterSpacing: 1 }}>
                {c}
              </div>
            ))}
          </div>
