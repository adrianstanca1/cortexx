...
export default function SubPortalSessionPage() {
  ...
  const create = async () => {
    const value = window.prompt('New subcontractor portal — short label')
    if (!value) return
    const res = await fetch('/api/sub-portal', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ subcontractorId: value }),
    })
    if (!res.ok) {
  ...
  const del = async (id: string) => {
    ...
    const res = await fetch(`/api/sub-portal/${id}`, { method: 'DELETE' })
  ...
  return (
    <ModuleShell
      ...
      tagline="Sub-only landing for assignments + payments"
          ...
          {rows.map(r => (
            <li key={r.id} style={{ background: '#152641', borderRadius: 10, padding: '12px 14px', border: '0.5px solid rgba(255,255,255,0.07)', fontFamily: 'var(--font-system)', fontSize: 13, color: '#eef3fa', display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div>{[r.subcontractorId, r.token, r.expiresAt].filter(Boolean).join(' · ') || r.id}</div>
