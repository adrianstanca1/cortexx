...
export default function InfraSnapshotPage() {
  ...
  const create = async () => {
    const value = window.prompt('New infrastructure — short label')
    if (!value) return
    const res = await fetch('/api/infrastructure', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ recordedAt: value }),
    })
    if (!res.ok) {
  ...
  const del = async (id: string) => {
    ...
    const res = await fetch(`/api/infrastructure/${id}`, { method: 'DELETE' })
  ...
  return (
    <ModuleShell
      ...
      tagline="System / capacity dashboard for admins"
          ...
          {rows.map(r => (
            <li key={r.id} style={{ background: '#152641', borderRadius: 10, padding: '12px 14px', border: '0.5px solid rgba(255,255,255,0.07)', fontFamily: 'var(--font-system)', fontSize: 13, color: '#eef3fa', display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div>{[r.recordedAt, r.cpuPercent, r.memMb].filter(Boolean).join(' · ') || r.id}</div>
