...
export default function InsuranceClaimPage() {
  ...
  const create = async () => {
    const value = window.prompt('New insurance claims — short label')
    if (!value) return
  ...
  return (
    <ModuleShell
      ...
      tagline="Public-liability / damage claims register"
      ...
      {loading ? (
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {rows.map(r => (
            ...
            >
              <div>{[r.projectId, r.incidentDate, r.policy].filter(Boolean).join(' · ') || r.id}</div>
