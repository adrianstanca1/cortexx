...
export default function AppraisalPage() {
  ...
  const create = async () => {
    const value = window.prompt('New performance reviews — short label')
    if (!value) return
  ...
  return (
    <ModuleShell
      ...
      tagline="Staff appraisal cycles + 1:1s"
      ...
      {loading ? (
        ...
        <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 8 }}>
          {rows.map(r => (
            ...
            >
              <div>{[r.memberId, r.reviewerId, r.periodLabel].filter(Boolean).join(' · ') || r.id}</div>
